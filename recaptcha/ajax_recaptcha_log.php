...
if(isset($_POST["captha"]) && isset($_SESSION["img_code"])) {
			if($_POST["captha"] == $ncapth) {
						...
						if(strtolower($username) != strtolower($row["username"])) {
						    ...
						    if( isset($row["wmid"]) && preg_match("|^[\d]{12}$|", trim($row["wmid"])) ) {
								...
								if(isset($_RES_WM_11["wmids"]) && count($_RES_WM_11["wmids"])>0) {
									...
									$_ALL_WMID_TAB = ( isset($_ALL_WMID_TAB) && is_array($_ALL_WMID_TAB) ) ? implode("; ", $_ALL_WMID_TAB) : $row["wmid"];
									$_ATTESTAT = isset($_RES_WM_11["att"]) ? $_RES_WM_11["att"] : $row["attestat"];
/*
									if($_ATTESTAT<=100) {
										$message_text = "Авторизация c аттестатом псевдонима запрещена! Для авторизации на проекте получите как минимум формальный аттестат!";
										$js_result = ($ajax_json=="json") ? json_encode_cp1251(array("result" => "ERROR", "message" => iconv("CP1251", "UTF-8", $message_text))) : $message_text;
										exit($js_result);
									}
									*/
							...
							if(mysql_num_rows($sql_b)>0) {
								...
								$message_text = "Ваш аккаунт заблокирован!<br><u>Причина блокировки</u>: ".$row_b["why"]."<br><u>Дата блокировки</u>: ".$row_b["date"]."";
								$js_result = ($ajax_json=="json") ? json_encode_cp1251(array("result" => "ERROR", "message" => iconv("CP1251", "UTF-8", $message_text))) : $message_text;
								exit($js_result);
							}
							...
							mysql_query("UPDATE `tb_users` SET 
								...
								`lastiplog`='$laip', `agent`='".$_SERVER["HTTP_USER_AGENT"]."', 
								`kol_log`=`kol_log`+'1' 
							...
							if($my_referer_1!=false && $statusref==0 && $my_lastlogdate==0 && $my_ban_date==0) {
								...
								if($konk_complex_status==1) {
									...
									if($my_joindate>=$konk_complex_date_start && $konk_complex_date_end>=time() && $konk_complex_date_start<=time()) {
										mysql_query("UPDATE `tb_users` SET `konkurs_complex`=`konkurs_complex`+'$konk_complex_point' WHERE `username`='$my_referer_1'") or die(($ajax_json=="json") ? json_encode_cp1251(array("result" => "ERROR", "message" => iconv("CP1251", "UTF-8", mysql_error()))) : mysql_error());
									}
								}
							}
							### АДМИН-КОНКУРС КОМПЛЕКСНЫЙ NEW ###
							...
							if($_COOKIE_ID != false) {
								...
								if(mysql_num_rows($sql_ban)>0) {
									...
									if($_USER_T_ID != $_COOKIE_ID) {
										$sql_ch1 = mysql_query("SELECT `id` FROM `tb_ban_users` WHERE `name`='$_COOKIE_NAME'") or die(($ajax_json=="json") ? json_encode_cp1251(array("result" => "ERROR", "message" => iconv("CP1251", "UTF-8", mysql_error()))) : mysql_error());
										...
										$sql_ch2 = mysql_query("SELECT `id` FROM `tb_ban_users` WHERE `name`='$username'") or die(($ajax_json=="json") ? json_encode_cp1251(array("result" => "ERROR", "message" => iconv("CP1251", "UTF-8", mysql_error()))) : mysql_error());
									...
									if(mysql_num_rows($sql_referer_1)>0) {
										...
										if(mysql_num_rows($sql_r_b_1)>0) {
											...
											if($money_ref_1>=$money_ureferera_nado) {
												...
												VALUES('$username','$partnerid','".DATE("d.m.Y H:i")."','".time()."','".$row_r_b_stat_1["money"]."','Реф-Бонус от реферера $my_referer_1 за регистрацию на проекте','Зачислено','rashod')") or die(($ajax_json=="json") ? json_encode_cp1251(array("result" => "ERROR", "message" => iconv("CP1251", "UTF-8", mysql_error()))) : mysql_error());
										...
										if(mysql_num_rows($sql_referer_2)>0) {
											...
											if(mysql_num_rows($sql_r_b_2)>0) {
												...
												if($money_ref_2>=$money_ureferera_nado) {
													...
													mysql_query("INSERT INTO `tb_history` (`user`,`user_id`,`date`,`time`,`amount`,`method`,`status`,`tipo`) 
													VALUES('$my_referer_1','$id_rb_ref_1','".DATE("d.m.Y H:i")."','".time()."','".$row_r_b_stat_2["money"]."','Реф-Бонус от реферера $my_referer_2 за привлечение реферала (ID:$partnerid)','Зачислено','rashod')") or die(($ajax_json=="json") ? json_encode_cp1251(array("result" => "ERROR", "message" => iconv("CP1251", "UTF-8", mysql_error()))) : mysql_error());

													mysql_query("INSERT INTO `tb_history` (`user`,`user_id`,`date`,`time`,`amount`,`method`,`status`,`tipo`) 
													VALUES('$my_referer_2','$id_rb_ref_2','".DATE("d.m.Y H:i")."','".time()."','$money_ureferera_nado','Реф-Бонус рефералу $my_referer_1 за привлечение реферала (ID:$partnerid)','Списано','rashod')") or die(($ajax_json=="json") ? json_encode_cp1251(array("result" => "ERROR", "message" => iconv("CP1251", "UTF-8", mysql_error()))) : mysql_error());

													if(trim($row_r_b_2["description"])!=false) {
														mysql_query("INSERT INTO `tb_mail_in`(`namein`,`nameout`,`subject`,`message`,`status`,`date`,`ip`) 
														VALUES('$my_referer_1','Система','Реф-Бонус от реферера $my_referer_2 за привлечение реферала (ID:$partnerid)','".$row_r_b_2["description"]."','0','".time()."','0.0.0.0')");
													}
												}else{
