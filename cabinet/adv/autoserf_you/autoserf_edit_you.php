<?php
if(!DEFINED("YOUAUTOSERF_EDIT")) {die ("Hacking attempt!");}

	echo '<a name="goto"></a>';
	echo '<h5 class="sp" style="margin-top:0px; font-size:14px; font-weight:bold; text-align:center;">Редактирование видеоролика в авто-серфинге <span style="color: #3F3F3F;">You</span><span style="border-radius: 5px; background-color: #E62117; padding: 2px; color: #FFFFFF;">Tube</span> № '.$id.'</h5>';

	$sql = mysql_query("SELECT * FROM `tb_ads_autoyoutube` WHERE `id`='$id' AND ytflag='1' AND `username`='$username'");
...
$cena_timer_aserf = mysql_result($sql,0,0);
		...
		echo '<div align="center"><span onClick="save_ads('.$row["id"].', \'youautoserf\', '.$status.');" class="proc-btn" style="float:none; width:160px;">Сохранить</span></div>';
