...
if($option == "pay_adv_up") {
	...
	if(mysql_num_rows($sql)>0) {
		...
		if($money_user_rb>=$youtube_cena_uplist && ($position>1 | $position==0)) {
			...
			VALUES('$username','".DATE("d.m.Yг. H:i")."','$youtube_cena_uplist','Поднятие ссылки в серфинге ID:$id','Списано','rashod')") or die(mysql_error());
	...
	/*	}elseif(($type_serf==2 | $type_serf==4) && is_url_img($description, "cabinet")!="true") {
			...
			echo 'Для VIP серфинга время просмотра должно быть в пределах от 20 сек. до '.$youtube_timer_do.' сек.';
			exit("");
		}elseif($limit_d<0) {
			exit("Ограничение количества показов в сутки должно быть положительным");
		}elseif($limit_d!=0 && $limit_d<$youtube_min_hits) {
			exit("Ограничение количества показов в сутки должно быть не менее $youtube_min_hits либо 0-без ограничений");
		}elseif($limit_h<0) {
			exit("Ограничение количества показов в час должно быть положительным");
		}elseif($limit_h!=0 && $limit_h<$youtube_min_hits) {
			exit("Ограничение количества показов в час должно быть не менее $youtube_min_hits либо 0-без ограничений");
			...
			if($status == 0 | $status == 3) {
				...
				if($timer<$youtube_timer_ot | $timer>$youtube_timer_do) {
					exit("Время просмотра должно быть в пределах от $youtube_timer_ot до $youtube_timer_do сек.");
				}
	...
	if(mysql_num_rows($sql)>0) {
		...
		if($money_pay>$money_user_rb) {
			...
			if($status=="0") {
				...
				mysql_query("INSERT INTO `tb_history` (`user`,`date`,`amount`,`method`,`status`,`tipo`) 
				VALUES('$username','".DATE("d.m.Yг. H:i")."','$money_pay','Пополнение баланса серфинга YouTube ID:$id','Списано','rashod')") or die(mysql_error());
