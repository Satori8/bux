...
if(isset($ads) && isset($id) && isset($op) && $ads=="statlink" && $op=="edit" && $id>0) {
	if(!DEFINED("STATLINK_EDIT")) DEFINE("STATLINK_EDIT", true);
	include("statlink_edit.php");
...
$slink_min = mysql_result($sql,0,0);
...
function pay_adv(id, type) {
	...
	if (plan == '') {
		...
		gebi("imfo-msg-addmoney"+id).style.display = 'block';
		setTimeout(function() {$("#imfo-msg-addmoney"+id).fadeOut('slow')}, 2000); clearTimeout();
	} else if (rplan < <?=$slink_min;?>) {
		gebi("imfo-msg-addmoney"+id).innerHTML = "<span class='msg-error'>Количество дней должно быть не менее <?=$slink_min;?></span>";
		gebi("imfo-msg-addmoney"+id).style.display = 'block';
		setTimeout(function() {$("#imfo-msg-addmoney"+id).fadeOut('slow')}, 2000); clearTimeout();
	} else {
...
$sql = mysql_query("SELECT * FROM `tb_ads_slink` WHERE `username`='$username' ORDER BY `id` ASC");
	...
	while ($row = mysql_fetch_array($sql)) {
		...
		echo '<tr id="adv_dell'.$row["id"].'">';
			...
			echo '<td align="left" class="noborder2">';
				...
				echo '</span>';
				echo '<span class="adv-dell" title="Удалить ссылку" onClick="alert_delete('.$row["id"].', \'statlink\');"></span>';
		...
		echo '<tr id="adv_addmoney'.$row["id"].'" style="display: none">';
			...
			function obsch<?php echo $row["id"];?>(){
				...
				plan<?php echo $row["id"];?> = str_replace("-", "", plan<?php echo $row["id"];?>);
				var cena_link<?php echo $row["id"];?> = <?php echo ($cena_slink + $row["color"] * $cena_slink_color);?>;
			...
			echo '<td align="center" colspan="3" class="ext-text">';
				...
				echo 'Укажите количество дней, которые вы хотите внести в бюджет рекламной площадки<br>(Минимум '.count_text($slink_min, "дней", "день", "дня", "").')';
				...
				echo '<center><span onClick="pay_adv('.$row["id"].', \'statlink\');" class="sub-green" style="float:none;" title="Пополнить бюджет площадки">Оплатить</span></center>';
...
echo '<div align="center"><span class="proc-btn" onClick="document.location.href=\'/advertise.php?ads=stat_links\'">Разместить ссылку</span></div>';
?>