<?php
if(!DEFINED("STATKAT_AJAX")) {die ("Hacking attempt!");}

if($option == "delete") {
	$sql = mysql_query("SELECT * FROM `tb_ads_kat` WHERE `id`='$id' AND `username`='$username'");
	if(mysql_num_rows($sql)>0) {
		...
		if($status == 0 | $status == 3) {
			mysql_query("DELETE FROM `tb_ads_kat` WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
			cache_stat_kat();
			exit("OK");
	...
	if(mysql_num_rows($sql)>0) {
		...
		if(mysql_num_rows($sql_bl)>0 && $black_url!=false) {
			...
			if($status == 0 | $status == 3) {
				...
				mysql_query("UPDATE `tb_ads_kat` SET `color`='$color', `url`='$url', `description`='$description', `ip`='$laip'  WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
				cache_stat_kat();
				...
				mysql_query("UPDATE `tb_ads_kat` SET `color`='$color_tab', `url`='$url', `description`='$description', `ip`='$laip'  WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
				cache_stat_kat();
	...
	if(mysql_num_rows($sql)>0) {
		...
		$cena = $plan * ($color * $cena_kat_color + $cena_kat) * (100-$cab_skidka)/100;
		...
		if($plan<$kat_min) {
			...
			if($status=="0" | $status=="3") {
				...
				mysql_query("UPDATE `tb_ads_kat` SET `status`='1', `method_pay`='10', `plan`='$plan', `date`='".time()."', `date_end`='".(time()+$plan*24*60*60)."', `ip`='$laip', `money`='$money_pay'  WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
				mysql_query("INSERT INTO `tb_history` (`user`,`date`,`amount`,`method`,`status`,`tipo`) 
				VALUES('$username','".DATE("d.m.Yг. H:i")."','$money_pay','Пополнение баланса индексируемой ссылки ID:$id','Списано','rashod')") or die(mysql_error());

				mysql_query("UPDATE `tb_ads_kat` SET `plan`=(`date_end`-`date`)/86400  WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());

				@stat_pay('statkat', $money_pay);
				@ads_wmid($wmid_user, $wmr_user, $username, $money_pay);
				@konkurs_rek($wmid_user, $wmr_user, $username, $money_pay);
				...
				mysql_query("UPDATE `tb_users` SET `reiting`=`reiting`+'$reit_add_1', `money_rb`=`money_rb`-'$money_pay', `money_rek`=`money_rek`+'$money_pay'  WHERE `username`='$username'") or die(mysql_error());
				mysql_query("UPDATE `tb_ads_kat` SET `status`='1', `method_pay`='10', `plan`=`plan`+'$plan', `date_end`=`date_end`+'".($plan*24*60*60)."', `ip`='$laip', `money`=`money`+'$money_pay'  WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
				mysql_query("INSERT INTO `tb_history` (`user`,`date`,`amount`,`method`,`status`,`tipo`) 
				VALUES('$username','".DATE("d.m.Yг. H:i")."','$money_pay','Пополнение баланса индексируемой ссылки ID:$id','Списано','rashod')") or die(mysql_error());
