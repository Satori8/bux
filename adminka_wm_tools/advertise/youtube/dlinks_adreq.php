...
if(count($_GET)>0) {
	...
	if($option=="add") {
		$sql = mysql_query("SELECT * FROM `tb_ads_youtube` WHERE `id`='$id' AND `status`='0'");
...
$sql = mysql_query("SELECT * FROM `tb_ads_youtube` WHERE `status`='0' AND `type_serf`!='10' ORDER BY `id` ASC");
if(mysql_num_rows($sql)>0) {
