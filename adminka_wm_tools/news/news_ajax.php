...
*/
		...
		if($option == "AddNews") {
			...
			if($title==false) {
				...
				mysql_query("INSERT INTO `tb_news` (`title`,`description`,`link_forum`,`status_comments`,`time`) 
				VALUES('$title','$description','$link_forum','$status_comments','".time()."')") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));

				if($re_not == 1) mysql_query("UPDATE `tb_users` SET `read_news`='0' WHERE `read_news`='1'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
			...
			$count = mysql_numrows(mysql_query("SELECT `id` FROM `tb_news`")) or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
			$pages_count = ceil($count / $perpage);
			$start_pos = intval($page * $perpage);

			$sql = mysql_query("SELECT * FROM `tb_news` ORDER BY `id` DESC LIMIT $start_pos, $perpage") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
			...
			$sql = mysql_query("SELECT `id` FROM `tb_news` WHERE `id`='$id'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
			if(mysql_num_rows($sql)>0) {
				mysql_query("DELETE FROM `tb_news` WHERE `id`='$id'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
				mysql_query("DELETE FROM `tb_news_comments` WHERE `ident`='$id'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
			...
			$sql = mysql_query("SELECT * FROM `tb_news` WHERE `id`='$id'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
			if(mysql_num_rows($sql)>0) {
			...
			if(mysql_num_rows($sql)>0) {
				...
				if($title==false) {
					...
					mysql_query("UPDATE `tb_news` SET `title`='$title', `description`='$description', `link_forum`='$link_forum', `status_comments`='$status_comments' WHERE `id`='$id'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
			...
			if(mysql_num_rows($sql)>0) {
				...
				mysql_query("UPDATE `tb_news` SET `status_comments`='$lock' WHERE `id`='$id'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
