...
if(isset($option) && $option=="edit" && isset($id) && $id>0) {
	...
	$sql = mysql_query("SELECT * FROM `tb_ads_tests` WHERE `id`='$id' AND `status`>'0'") or die(mysql_error());
	...
	if(mysql_num_rows($sql)>0) {
		...
		$perpage = 25;
		$sql_p = mysql_query("SELECT `id` FROM `tb_ads_tests_visits` WHERE `ident`='$id' AND `money`>'0'") or die(mysql_error());
		...
		$sql = mysql_query("SELECT * FROM `tb_ads_tests_visits` WHERE `ident`='$id' ORDER BY `id` DESC LIMIT $start_pos, $perpage") or die(mysql_error());
		if(mysql_num_rows($sql)>0) {
...
require("navigator/navigator.php");
$perpage = 25;
$sql_p = mysql_query("SELECT `id` FROM `tb_ads_tests` WHERE `status`>'0' $WHERE_ADD") or die(mysql_error());
$count = mysql_numrows($sql_p);
$pages_count = ceil($count / $perpage);
$page = (isset($_GET["page"]) && preg_match("|^[0-9\-]{1,11}$|", trim($_GET["page"]))) ? intval(trim($_GET["page"])) : "1";
if ($page > $pages_count | $page<0) $page = $pages_count;
...
$sql_tests = mysql_query("SELECT * FROM `tb_ads_tests` WHERE `status`>'0' $WHERE_ADD ORDER BY `id` DESC LIMIT $start_pos, $perpage") or die(mysql_error());
$all_tests = mysql_num_rows($sql_tests);
...
if(mysql_num_rows($sql_tests)>0) {
	while ($row = mysql_fetch_assoc($sql_tests)) {
