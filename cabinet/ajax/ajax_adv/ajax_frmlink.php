<?php
if(!DEFINED("FRMLINK_AJAX")) {die ("Hacking attempt!");}

if($option == "delete") {
	$sql = mysql_query("SELECT * FROM `tb_ads_frm` WHERE `id`='$id' AND `username`='$username'");
	if(mysql_num_rows($sql)>0) {
		...
		if($status == 0) {
			mysql_query("DELETE FROM `tb_ads_frm` WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
			cache_frm_links();
			exit("OK");
	...
	if(mysql_num_rows($sql)>0) {
		...
		if(mysql_num_rows($sql_bl)>0 && $black_url!=false) {
			...
			mysql_query("UPDATE `tb_ads_frm` SET `url`='$url', `description`='$description', `ip`='$laip'  WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
			cache_frm_links();
			exit("OK");
		}
	}else{
	...
	if(mysql_num_rows($sql)>0) {
		...
		$cena = $plan * $cena_frm_links * (100-$cab_skidka)/100;
		$money_pay = number_format($cena, 2, ".", "");

		if($plan<$min_frm_links) {
			exit("ERROR1");
			...
			if($status=="0" | $status=="3") {
				...
				mysql_query("UPDATE `tb_ads_frm` SET `status`='1', `method_pay`='10', `plan`='$plan', `date`='".time()."', `date_end`='".(time()+$plan*24*60*60)."', `ip`='$laip', `money`='$money_pay'  WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
				mysql_query("INSERT INTO `tb_history` (`user`,`date`,`amount`,`method`,`status`,`tipo`) 
				VALUES('$username','".DATE("d.m.Yг. H:i")."','$money_pay','Пополнение баланса ссылки во фрейме ID:$id','Списано','rashod')") or die(mysql_error());

				mysql_query("UPDATE `tb_ads_frm` SET `plan`=(`date_end`-`date`)/86400  WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
				...
				mysql_query("UPDATE `tb_users` SET `reiting`=`reiting`+'$reit_add_1', `money_rb`=`money_rb`-'$money_pay', `money_rek`=`money_rek`+'$money_pay'  WHERE `username`='$username'") or die(mysql_error());
				mysql_query("UPDATE `tb_ads_frm` SET `status`='1', `method_pay`='10', `plan`=`plan`+'$plan', `date_end`=`date_end`+'".($plan*24*60*60)."', `ip`='$laip', `money`=`money`+'$money_pay'  WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
				mysql_query("INSERT INTO `tb_history` (`user`,`date`,`amount`,`method`,`status`,`tipo`) 
				VALUES('$username','".DATE("d.m.Yг. H:i")."','$money_pay','Пополнение баланса ссылки во фрейме ID:$id','Списано','rashod')") or die(mysql_error());
