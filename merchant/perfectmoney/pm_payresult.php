			...
			/////////////////////	
				if($bon_pay_status==1){
					if($merch_amount1>=$bon_pay_summa){
						...
						mysql_query("UPDATE `tb_users` SET `money_rb`=`money_rb`+'$merch_amount1', `money_in`=`money_in`+'$merch_amount' WHERE `username`='$username'") or die(mysql_error());
						mysql_query("INSERT INTO `tb_history` (`user`, `amount`, `date`, `method`, `status`, `tipo`) 
						VALUES('$username', '$merch_amount', '".DATE("d.m.Yг. H:i")."', 'Пополнение рекламного баланса через PerfectMoney','Зачислено', 'popoln')") or die(mysql_error());
			...
			/////////////////////	
				if($bon_inv_status==1){
					if($merch_amount>=$bon_inv_summa){
						...
						mysql_query("INSERT INTO `tb_history` (`user`, `amount`, `date`, `method`, `status`, `tipo`) 
						VALUES('$username', '$merch_amount', '".DATE("d.m.Yг. H:i")."', 'Пополнение баланса инвестора через PerfectMoney','Зачислено', 'popoln')") or die(mysql_error());
