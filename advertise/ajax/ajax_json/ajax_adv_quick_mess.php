...
$message_text = "";
	$sql = mysql_query("SELECT `price` FROM `tb_config` WHERE `item`='quick_mess_сena' AND `howmany`='1'");
	...
	if($user_name==false) {
		...
		if(mysql_num_rows($sql_id)>0) {
			...
			if($user_money_rb >= $money_pay) {
				...
				VALUES('1','$user_name','$user_id','".DATE("d.m.Y H:i")."','".time()."','$money_pay','Оплата рекламы (Быстрые сообщения, ID:$id)','Оплачено','reklama')") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
