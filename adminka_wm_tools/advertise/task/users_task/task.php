...
if(!isset($_SESSION["userLog"]) && !isset($_SESSION["userPas"])) {
	...
	function format_table($id, $country_targ, $name, $url, $reiting, $comments, $cena, $re, $good, $bad, $wait, $date){
	    
	    $sql = mysql_query("SELECT * FROM `tb_ads_task` WHERE `id`='$id' ");
if(mysql_num_rows($sql)>0) {
    
    while ($row = mysql_fetch_array($sql)) {
        echo '<tr id="adv_dell'.$row["id"].'" align="riht">';
	    ...
	    /*echo '<td align="center" width="30" style="border-right:none;">';
				if($row["status"]=="wait") {
					echo '<span onClick=\'alert("Для запуска задания, необходимо пополнить баланс");\' style="cursor: pointer;"><img src="/img/adv/icon-play.png" alt="" title="Запустить задание" border="0" aling="absmiddle" /></span>';
				}elseif($row["status"]=="pay") {
					echo '<a href="'.$_SERVER["PHP_SELF"].'?option=pause_task&rid='.$row["id"].'">';
						echo '<img src="/img/adv/icon-pause.png" alt="" border="0" aling="absmiddle" title="Остановить задание" />';
					echo '</a>';
				}elseif($row["status"]=="pause") {
					echo '<a href="'.$_SERVER["PHP_SELF"].'?option=active_task&rid='.$row["id"].'">';
						echo '<img src="/img/adv/icon-play.png" alt="" border="0" aling="absmiddle" title="Запустить задание" />';
					echo '</a>';
				}elseif($row["status"]=="end") {
					echo '<span onClick=\'alert("Баланс исчерпан, необходимо пополнить баланс");\' style="cursor: pointer;"><img src="/img/adv/icon-play.png" alt="" title="Запустить задание" border="0" aling="absmiddle" /></span>';
				}
			echo '</td>';*/
		echo '<td align="center">'.$id.'</td>';
		echo '<td>';
			if($date > (time()-24*60*60)) {echo '<img src="../../img/new_rek.gif" border="0" alt="" align="middle" />&nbsp;';}
			echo '<a href="'.$_SERVER["PHP_SELF"].'?op='.limpiarez($_GET["op"]).'&amp;option=view&amp;rid='.$id.'">'.$name.'</a><br>Сайт: <b>http://'.GET_DOMEN_T($url).'/</b><br>Рейтинг: <b>'.round($reiting,2).'</b>&nbsp;&nbsp;&nbsp;Комментариев: <a href="'.$_SERVER["PHP_SELF"].'?op='.limpiarez($_GET["op"]).'&amp;view=comments&amp;rid='.$id.'">'.$comments.'</a>';
		echo '</td>';
	...
	$count = mysql_num_rows(mysql_query("SELECT `id` FROM `tb_ads_task` WHERE $WHERE `status`='pay' $type_tab"));
	...
	$tabla = mysql_query("SELECT * FROM `tb_ads_task` WHERE $WHERE `status`!='0' $type_tab ORDER BY $ORDER LIMIT $start_pos,$perpage");
	...
	echo '<table align="center" border="1" width="100%" cellspacing="3" cellpadding="3" style="border-collapse: collapse; border: 1px solid #000; line-height: 1.4em;">';
		...
		echo '</tr>';
		echo '<tr bgcolor="#7EC0EE">';
		echo '<th align="left" style="border: 1px solid #000;">Тип заданий: ';
			...
			echo '<a  href="'.$_SERVER['PHP_SELF'].'?op='.limpiarez($_GET["op"]).'&sort='.$sort.'&sort_z='.$sort_z.'&type=1&task_search='.$task_search.'&task_name='.$task_name.'&task_auto='.$task_auto.'&task_price='.$task_price.'&s='.$page.'"'; if($type==1){echo ' class="b"';} echo '>Клики</a>&nbsp;| ';
			echo '<a  href="'.$_SERVER['PHP_SELF'].'?op='.limpiarez($_GET["op"]).'&sort='.$sort.'&sort_z='.$sort_z.'&type=2&task_search='.$task_search.'&task_name='.$task_name.'&task_auto='.$task_auto.'&task_price='.$task_price.'&s='.$page.'"'; if($type==2){echo ' class="b"';} echo '>Рег. без активности</a>&nbsp;| ';
			echo '<a  href="'.$_SERVER['PHP_SELF'].'?op='.limpiarez($_GET["op"]).'&sort='.$sort.'&sort_z='.$sort_z.'&type=3&task_search='.$task_search.'&task_name='.$task_name.'&task_auto='.$task_auto.'&task_price='.$task_price.'&s='.$page.'"'; if($type==3){echo ' class="b"';} echo '>Рег. с активностью</a>&nbsp;| ';
			echo '<a  href="'.$_SERVER['PHP_SELF'].'?op='.limpiarez($_GET["op"]).'&sort='.$sort.'&sort_z='.$sort_z.'&type=4&task_search='.$task_search.'&task_name='.$task_name.'&task_auto='.$task_auto.'&task_price='.$task_price.'&s='.$page.'"'; if($type==4){echo ' class="b"';} echo '>Постинг в форум</a>&nbsp;| ';
			echo '<a  href="'.$_SERVER['PHP_SELF'].'?op='.limpiarez($_GET["op"]).'&sort='.$sort.'&sort_z='.$sort_z.'&type=5&task_search='.$task_search.'&task_name='.$task_name.'&task_auto='.$task_auto.'&task_price='.$task_price.'&s='.$page.'"'; if($type==5){echo ' class="b"';} echo '>Постинг в блоги</a>&nbsp;| ';
			echo '<a  href="'.$_SERVER['PHP_SELF'].'?op='.limpiarez($_GET["op"]).'&sort='.$sort.'&sort_z='.$sort_z.'&type=6&task_search='.$task_search.'&task_name='.$task_name.'&task_auto='.$task_auto.'&task_price='.$task_price.'&s='.$page.'"'; if($type==6){echo ' class="b"';} echo '>Голосование</a>&nbsp;| ';
			echo '<a  href="'.$_SERVER['PHP_SELF'].'?op='.limpiarez($_GET["op"]).'&sort='.$sort.'&sort_z='.$sort_z.'&type=7&task_search='.$task_search.'&task_name='.$task_name.'&task_auto='.$task_auto.'&task_price='.$task_price.'&s='.$page.'"'; if($type==7){echo ' class="b"';} echo '>Загрузка файлов</a>&nbsp;| ';
			echo '<a  href="'.$_SERVER['PHP_SELF'].'?op='.limpiarez($_GET["op"]).'&sort='.$sort.'&sort_z='.$sort_z.'&type=9&task_search='.$task_search.'&task_name='.$task_name.'&task_auto='.$task_auto.'&task_price='.$task_price.'&s='.$page.'"'; if($type==9){echo ' class="b"';} echo '>Социальные сети</a>&nbsp;| ';
			echo '<a  href="'.$_SERVER['PHP_SELF'].'?op='.limpiarez($_GET["op"]).'&sort='.$sort.'&sort_z='.$sort_z.'&type=10&task_search='.$task_search.'&task_name='.$task_name.'&task_auto='.$task_auto.'&task_price='.$task_price.'&s='.$page.'"'; if($type==10){echo ' class="b"';} echo '>YouTube</a>&nbsp;| ';
			echo '<a  href="'.$_SERVER['PHP_SELF'].'?op='.limpiarez($_GET["op"]).'&sort='.$sort.'&sort_z='.$sort_z.'&type=11&task_search='.$task_search.'&task_name='.$task_name.'&task_auto='.$task_auto.'&task_price='.$task_price.'&s='.$page.'"'; if($type==11){echo ' class="b"';} echo '>Работа с капчей</a>&nbsp;| ';
			echo '<a  href="'.$_SERVER['PHP_SELF'].'?op='.limpiarez($_GET["op"]).'&sort='.$sort.'&sort_z='.$sort_z.'&type=8&task_search='.$task_search.'&task_name='.$task_name.'&task_auto='.$task_auto.'&task_price='.$task_price.'&s='.$page.'"'; if($type==8){echo ' class="b"';} echo '>Прочее</a>';
		echo '</th>';
