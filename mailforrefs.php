<?php
$pagetitle="Рассылка сообщения рефералам I уровня";
include('header.php');
...
if(!isset($_SESSION["userLog"]) && !isset($_SESSION["userPas"])) {
	...
	$sql = mysql_query("SELECT `id` FROM `tb_users` WHERE `referer`='$username'");
	$all_referals = mysql_num_rows($sql);

	$sql = mysql_query("SELECT `id` FROM `tb_users` WHERE `referer`='$username' AND `lastlogdate2`>='".strtotime(DATE("d.m.Y", time()-14*24*60*60))."'");
	...
	if(count($_POST)>0 && isset($_POST["message"])) {
		...
		if($types==1) {
			...
			$sql = mysql_query("SELECT `username` FROM `tb_users` WHERE `referer`='$username'");

		}elseif($types==2) {
			$cena = $cena_2;
			$types_txt="активным";

			$sql = mysql_query("SELECT `username` FROM `tb_users` WHERE `referer`='$username' AND `lastlogdate2`>='".strtotime(DATE("d.m.Y", time()-14*24*60*60))."'");
		}else{
	...
	<?php
	echo '<br>';
	echo '<b>Платная рассылка вашим рефералам I уровня:</b><br>цена за 1 письмо <b>'.$cena_mail_refs.' руб.</b><br>Письмо приходит через внутреннюю почту системы.<br><br>';
	...
	echo '<tr>';
		...
		echo '<td><input type="text" id="subject" name="subject" value="Сообщение от реферера I уровня '.$username.'" maxlength="255" onChange="messchange();" onKeyUp="messchange();" class="ok"><div align="right" id="count1" style="float:right; display:block; color:#696969; padding:0px; margin:0px;"></div></td>';
	echo '</tr>';
	echo '<tr>';
