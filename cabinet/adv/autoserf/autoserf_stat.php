<?php
if(!DEFINED("AUTOSERF_STAT")) {die ("Hacking attempt!");}

echo '<a name="goto"></a>';
echo '<h5 class="sp" style="margin-top:0px; font-size:14px; font-weight:bold; text-align:center;">Статистика просмотров авто-серфинга № '.$id.' за последние 24 часа</h5>';
...
if(mysql_num_rows($sql)>0) {
	...
	$perpage = 30;
	$sql_p = mysql_query("SELECT `id` FROM `tb_ads_auto_visits` WHERE `ident`='$id' AND `money`>'0'");
	...
	$sql = mysql_query("SELECT * FROM `tb_ads_auto_visits` WHERE `ident`='$id' AND `type`='visit' AND `money`>'0' ORDER BY `date` ASC LIMIT $start_pos,$perpage");
	if(mysql_num_rows($sql)>0) {
