...
if(count($_POST)>0 && isset($_POST["id_pay"])) {
	if(!isset($_SESSION["userLog"]) && !isset($_SESSION["userPas"])) {
		...
		$sql_id = mysql_query("SELECT * FROM `tb_ads_youtube` WHERE `id`='$id_pay' AND `status`='0' AND `username`='$username' ORDER BY `id` DESC LIMIT 1");
		if(mysql_num_rows($sql_id)>0) {
			...
			if($money_user>=$money_pay) {
			   ...
			   mysql_query("UPDATE `tb_users` SET `reiting`=`reiting`+'$reit_add_1'+'$reit_add_3', `money_rb`=`money_rb`-'$money_pay', `money_rek`=`money_rek`+'$money_pay' $vip_serf_tab WHERE `username`='$username'") or die(mysql_error());
				...
				VALUES('$username', '".DATE("d.m.Y H:i")."', '$money_pay', 'Оплата рекламы YouTube серфинг(vip), ID:#$id_pay', 'Оплачено', 'reklama')") or die(mysql_error());
			}else{
				mysql_query("INSERT INTO `tb_history` (`user`, `date`, `amount`, `method`, `status`, `tipo`) 
				VALUES('$username', '".DATE("d.m.Y H:i")."', '$money_pay', 'Оплата рекламы [YouTube], ID:#$id_pay', 'Оплачено', 'reklama')") or die(mysql_error());
			}
	...
	echo '<div id="adv-block1" style="display:none;">';
		...
		echo '<tbody>';
			...
			echo '<tr>';
				...
				//echo '<td style="height:31px;"><div id="bl5"><select name="active" id="active" onChange="PlanChange();" onClick="PlanChange();"><option value="1">Да</option></select></div><div id="bl51" style="text-align:left;"></div></td>';
