...
if(count($_POST)>0) {
	if(isset($_POST["option"]) && limpiarez($_POST["option"])=="add_ban") {
		...
		if($ip_block==false) {
			...
			$sql_с = mysql_query("SELECT `id` FROM `tb_black_ip` WHERE `ip_block`='$ip_block'");
...
if(count($_POST)>0) {
	if(isset($_POST["option"]) && limpiarez($_POST["option"])=="del_ban") {
		...
		$sql_с = mysql_query("SELECT `ip_block` FROM `tb_black_ip` WHERE `id`='$id'");
		if(mysql_num_rows($sql_с)>0) {
			$row_c = mysql_fetch_assoc($sql_с);
