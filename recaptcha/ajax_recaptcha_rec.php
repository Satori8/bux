...
if(isset($_POST["captha"]) && isset($_SESSION["img_code"]) && ($_POST["captha"] == $ncapth)) {
	    ...
	    $msgtext = str_replace($var, $zamena, $email_temp['recc_2']);
							...
							if( $send_wmid==1 && isset($row["wmid"]) && preg_match("|^[\d]{12}$|", trim($row["wmid"])) ) {
								...
								$message_wm.= "Вы запросили данные для авторизации на проекте https://".$_SERVER["HTTP_HOST"]."/\n";
								...
								$message_wm.= "---------------------------\n";
								$message_wm.= "С уважением, автоматическая служба уведомлений https://".$_SERVER["HTTP_HOST"]."/";
								...
								if($_RES_WM["retval"]==0) {
									...
									if(!$mail_error = $mail_out->send($email, $row["username"], iconv("CP1251", "UTF-8", 'Восстановление данных для авторизации '.$_SERVER["HTTP_HOST"]), iconv("CP1251", "UTF-8", $msgtext))) {
				...
				$message_text = ( isset($_ERR_CODE) && htmlspecialchars(trim($_ERR_CODE))!=false ) ? "Необходимо подтвердить, что Вы не робот!" : "Каптча введена не верно!";
				$js_result = ($ajax_json=="json") ? json_encode_cp1251(array("result" => "ERROR", "message" => iconv("CP1251", "UTF-8", $message_text))) : $message_text;
				exit($js_result);
			}
		}
