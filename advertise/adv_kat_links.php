...
if(count($_POST)>0 && isset($_POST["id_pay"])) {
	if(!isset($_SESSION["userLog"]) && !isset($_SESSION["userPas"])) {
		...
		$sql_id = mysql_query("SELECT * FROM `tb_ads_kat` WHERE `id`='$id_pay' AND `status`='0' AND `username`='$username' ORDER BY `id` DESC LIMIT 1");
		if(mysql_num_rows($sql_id)>0) {
			...
			if($money_user>=$money_pay) {
				...
				mysql_query("UPDATE `tb_ads_kat` SET `status`='1', `date`='".time()."', `date_end`='".(time()+$plan*24*60*60)."', `wmid`='$wmid_user' WHERE `id`='$id_pay' AND `status`='0' AND `username`='$username' ORDER BY `id` DESC LIMIT 1") or die(mysql_error());
				mysql_query("INSERT INTO `tb_history` (`user`,`date`,`amount`,`method`,`status`,`tipo`) VALUES('$username','".DATE("d.m.Yг. H:i")."','$money_pay',  'Оплата рекламы: статическая ссылка ($plan дн.)','Списано','rashod')") or die(mysql_error());
...
$system_pay[21] = "AdvCash";
		...
		mysql_query("DELETE FROM `tb_ads_kat` WHERE `status`='0' AND `date`<'".(time()-(24*60*60))."'") or die(mysql_error());
		...
		$check_wmid = mysql_query("SELECT `id` FROM `tb_ads_kat` WHERE `status`='0' AND `session_ident`='".session_id()."' ORDER BY `id` DESC LIMIT 1");
		if(mysql_num_rows($check_wmid)>0) {
			mysql_query("UPDATE `tb_ads_kat` SET `merch_tran_id`='$merch_tran_id', `method_pay`='$method_pay', `wmid`='$wmid_user', `username`='$username', `date`='".time()."', `date_end`='".(time()+$plan*24*60*60)."', `plan`='$plan', `url`='$url', `description`='$description', `color`='$color', `ip`='$laip', `money`='$summa' WHERE `status`='0' AND `session_ident`='".session_id()."'") or die(mysql_error());
		}else{
			mysql_query("INSERT INTO `tb_ads_kat` (`status`,`session_ident`,`merch_tran_id`,`method_pay`,`wmid`,`username`,`date`,`date_end`,`plan`,`url`,`description`,`color`,`ip`,`money`) 
		...
		$sql_id = mysql_query("SELECT `id` FROM `tb_ads_kat` WHERE `status`='0' AND `session_ident`='".session_id()."' ORDER BY `id` DESC LIMIT 1");
	...
	echo '<div id="InfoAds" style="display:block; align:justify; margin-bottom:15px;">';
		...
		echo 'На данный момент показатели <b style="color:#3A5FCD">'.strtoupper(array_shift(explode(".", $_SERVER["HTTP_HOST"]))).'</b> ИКС =';
