...
require($_SERVER["DOCUMENT_ROOT"]."/merchant/megakassa/mk_config.php");
define('PATH_TO_LOG', dirname(__FILE__).'/');

$uid = ( isset($_REQUEST["uid"]) && preg_match("|^[\d]{1,11}$|", htmlspecialchars(trim($_REQUEST["uid"]))) ) ? intval(htmlspecialchars(trim($_REQUEST["uid"]))) : false;
$merch_amount = (isset($_REQUEST["amount"])) ? floatval(htmlspecialchars(trim($_REQUEST["amount"]))) : false;
$amount_shop = (isset($_REQUEST["amount_shop"])) ? floatval(htmlspecialchars(trim($_REQUEST["amount_shop"]))) : false;
$amount_client = (isset($_REQUEST["amount_client"])) ? floatval(htmlspecialchars(trim($_REQUEST["amount_client"]))) : false;
...
$signature = (isset($_REQUEST["signature"])) ? strtoupper(htmlspecialchars(trim($_REQUEST["signature"]))) : false;
//$my_signature = strtoupper(md5("$uid:$merch_amount:$amount_shop:$amount_client:$currency:$order_id:$payment_method_id:$payment_method_title:$client_email:$mk_secret_key"));
//$my_signature = "$uid:$merch_amount:$amount_shop:$amount_client:$currency:$order_id:$payment_method_id:$payment_method_title:$client_email:$mk_secret_key";
$debug = (isset($mk_debug) && $mk_debug==1) ? "1" : "0";
$my_signature = strtoupper(md5(join(':', array($uid, $merch_amount, $amount_shop, $amount_client, $currency, $order_id, $payment_method_id, $payment_method_title, $creation_time, $payment_time, $client_email, $status, $debug, $mk_secret_key))));
...
$mk_post = false;
if(count($_REQUEST) > 0) {
	foreach($_REQUEST as $key => $val) $mk_post.="$key: $val; ";
	$mk_post.="my_signature: $my_signature";
	$mk_post = trim($mk_post);
}
$f = @fopen(PATH_TO_LOG."mk_order.txt", "a+");
@fputs($f,"$mk_post\n");
fclose($f);
### запись в файл информации о проведенной операции ###
...
if($my_signature != $signature) {
	...
	if($shp_item!=false && array_key_exists($shp_item, $TABLE_ARR)) {
		...
		if(mysql_num_rows($sql)) {
			...
			/////////////////////	
				if($bon_pay_status==1){
					...
					VALUES('$username', '$merch_amount1', '".DATE("d.m.Yг. H:i")."', 'Пополнение рекламного баланса через MEGAKASSA','Зачислено', 'popoln')") or die(mysql_error());
	    ...
	    $mail_error = $mail_out->send($email,$username,iconv("CP1251", "UTF-8", 'Пополнение рекламного баланса на Scorpionbux.info'), iconv("CP1251", "UTF-8", $msgtext));
		/////////////////////////////////////////////////////
			...
			/////////////////////	
				if($bon_inv_status==1){
					...
					mysql_query("INSERT INTO `tb_history` (`user`, `amount`, `date`, `method`, `status`, `tipo`) 
					VALUES('$username', '$merch_amount', '".DATE("d.m.Yг. H:i")."', 'Пополнение баланса инвестора через MegaKassa','Зачислено', 'popoln')") or die(mysql_error());
