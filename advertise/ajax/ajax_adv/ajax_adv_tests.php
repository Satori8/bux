		...
		echo '<tr align="center">';
			echo '<td align="left" width="160" style="border-right:none;">';
				if($method_pay==10 && $username!=false) {
					echo '<span onClick="PayAds(\''.$id_zakaz.'\');" class="proc-btn" style="float:left;">Оплатить</span>';
	...
	if($username==false) {
		...
		if(mysql_num_rows($sql_id)>0) {
			...
			if($money_user_rb>=$money_pay) {
				...
				VALUES('$username', '".DATE("d.m.Y H:i")."', '$money_pay', 'Пополнение баланса рекламной площадки (Тесты, ID:$id)', 'Оплачено', 'reklama')") or die(mysql_error());
