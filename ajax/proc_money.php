		...
		if($check_hs1==1 && $cnt==$check_hs2) {
			...
			if(DATE("d.m.Y")!=DATE("d.m.Y", $bonus_act_usr)) {
				mysql_query("UPDATE `tb_users` SET `money_rb`=`money_rb`+'$bonus', `bonus_act`='".time()."' WHERE `username`='$username'") or die(mysql_error());
                               mysql_query("INSERT INTO `tb_history` (`user`, `date`, `amount`, `method`, `status`, `tipo`) VALUES('$username', '".DATE("d.m.Y H:i")."', '$bonus', 'Ежедневный бонус за посещение аккаунта', 'Зачислено', 'popoln')") or die(mysql_error());
