...
$param_arr = array(
	...
	"username" 	=> 	"Логин", 
	"password" 	=> 	"Пароль", 
	"referer1" 	=> 	"Реферер I уровня", 
	"referer2" 	=> 	"Реферер II уровня", 
	"referer3" 	=> 	"Реферер III уровня", 
	"referer4" 	=> 	"Реферер IV уровня", 
	"referer5" 	=> 	"Реферер V уровня", 
	"wmid" 		=> 	"WMID", 
...
if(isset($_GET["option"]) && limpiar($_GET["option"])=="edit") {
	...
	if(count($_POST)>0) {
		...
		if(mysql_num_rows($sql_e)<1) {
			...
			echo '<span class="msg-error">Номер телефона введен некорректно!</span>';
		}else{
			$_ERROR = 0;
	...
	if(mysql_num_rows($sql_e)>0) {
		...
		echo '<form method="post" action="" id="newform">';
			...
			echo '<tr><td valign="top" style="margin:0px; padding:0px;" width="50%">';
				...
				echo '<tr><th>Параметр</th><th width="250">Значение</th></tr>';
				...
				echo '<tr><td><b>E-mail подтвержден?</b></td><td class="select"><select name="email_ok">';
				...
				echo '<tr><td><b>WMID подтвержден?</b></td><td class="select"><select name="wm_verif">';
					echo '<option value="0" '.( ($row["wm_verif"]==0 | $row["wmid"]==false | $row["wm_purse"]==false) ? 'selected="selected"' : false).'>Нет</option>';
					echo '<option value="1" '.($row["wm_verif"]==1 && ($row["wmid"]!=false && $row["wm_purse"]!=false)? 'selected="selected"' : false).'>Да</option>';
				echo '</select></td></tr>';
				echo '<tr>';
				...
				echo '<tr><td><img src="/img/sber16x16.png" alt="" title="" border="0" align="absmiddle" style="margin:0px; padding:3px 6px 3px 0px;" /><span style="color:green;">СберБанк</span></td><td><input type="text" class="ok" name="sb_purse" value="'.$row["sb_purse"].'" placeholder="Только карты СберБанка России"></td></tr>';

				echo '<tr><td><b>Реферер Iур.:</b></td><td><input type="text" class="ok" name="referer1" value="'.$row["referer1"].'"></td></tr>';
				echo '<tr><td><b>Реферер IIур.:</b></td><td><input type="text" class="ok" name="referer2" value="'.$row["referer2"].'"></td></tr>';
				echo '<tr><td><b>Реферер IIIур.:</b></td><td><input type="text" class="ok" name="referer3" value="'.$row["referer3"].'"></td></tr>';
				echo '<tr><td><b>Реферер IVур.:</b></td><td><input type="text" class="ok" name="referer4" value="'.$row["referer4"].'"></td></tr>';
				echo '<tr><td><b>Реферер Vур.:</b></td><td><input type="text" class="ok" name="referer5" value="'.$row["referer5"].'"></td></tr>';

				echo '<tr><td><b>Вход в аккаунт через WmLogin:</b></td><td class="select"><select name="block_wmid" class="ok">';
				...
				echo '<tr><td><b>Дополнительная защита аккаунта по IP/браузеру:</b></td><td class="select"><select name="block_agent" class="ok">';
			...
			echo '<td valign="top" style="margin:0px; padding:0px;" width="50%">';
				echo '<table class="tables" style="margin:0px; padding:0px;">';
				echo '<tr><th>Параметр</th><th width="150">Значение</th></tr>';
				echo '<tr><td>Рейтинг:</td><td><input type="text" class="ok12" name="reiting" value="'.$row["reiting"].'"></td></tr>';
				echo '<tr><td>Рефералы Iур:</td><td align="center"><input type="text" class="ok" name="referals1" value="'.$row["referals1"].'"></td></tr>';
				echo '<tr><td>Рефералы IIур:</td><td align="center"><input type="text" class="ok" name="referals2" value="'.$row["referals2"].'"></td></tr>';
				echo '<tr><td>Рефералы IIIур:</td><td align="center"><input type="text" class="ok" name="referals3" value="'.$row["referals3"].'"></td></tr>';
				echo '<tr><td>Рефералы IVур:</td><td align="center"><input type="text" class="ok" name="referals4" value="'.$row["referals4"].'"></td></tr>';
				echo '<tr><td>Рефералы Vур:</td><td align="center"><input type="text" class="ok" name="referals5" value="'.$row["referals5"].'"></td></tr>';
				echo '<tr><td>Заработали рефералы Iур.:</td><td align="center"><input type="text" class="ok" name="refmoney1" value="'.$row["refmoney1"].'"></td></tr>';
				echo '<tr><td>Заработали рефералы IIур.:</td><td align="center"><input type="text" class="ok" name="refmoney2" value="'.$row["refmoney2"].'"></td></tr>';
				echo '<tr><td>Заработали рефералы IIIур.:</td><td align="center"><input type="text" class="ok" name="refmoney3" value="'.$row["refmoney3"].'"></td></tr>';
				echo '<tr><td>Заработали рефералы IVур.:</td><td align="center"><input type="text" class="ok" name="refmoney4" value="'.$row["refmoney4"].'"></td></tr>';
				echo '<tr><td>Заработали рефералы Vур.:</td><td align="center"><input type="text" class="ok" name="refmoney5" value="'.$row["refmoney5"].'"></td></tr>';

				echo '<tr><td>Просмотрено ссылок в серфинге:</td><td align="center"><input type="text" class="ok" name="visits" value="'.$row["visits"].'"></td></tr>';

				echo '<tr><td>Просмотрено ссылок в авто-серфинге:</td><td align="center"><input type="text" class="ok" name="visits_a" value="'.$row["visits_a"].'"></td></tr>';
				echo '<tr><td>Заработано на авто-серфинге:</td><td align="center"><input type="text" class="ok" name="money_a" value="'.$row["money_a"].'"></td></tr>';

				echo '<tr><td>Просмотрено писем:</td><td align="center"><input type="text" class="ok" name="visits_m" value="'.$row["visits_m"].'"></td></tr>';
				echo '<tr><td>Заработано на письмах:</td><td align="center"><input type="text" class="ok" name="money_m" value="'.$row["money_m"].'"></td></tr>';

				echo '<tr><td>Выполнено заданий:</td><td align="center"><input type="text" class="ok" name="visits_t" value="'.$row["visits_t"].'"></td></tr>';
				echo '<tr><td>Заработано на заданиях:</td><td align="center"><input type="text" class="ok" name="money_t" value="'.$row["money_t"].'"></td></tr>';

				echo '<tr><td>Пройдено тестов:</td><td align="center"><input type="text" class="ok" name="visits_tests" value="'.$row["visits_tests"].'"></td></tr>';
				echo '<tr><td>Заработано на тестах:</td><td align="center"><input type="text" class="ok" name="money_tests" value="'.$row["money_tests"].'"></td></tr>';

				echo '<tr><td>Пополнение баланса:</td><td align="center"><input type="text" class="ok" name="money_in" value="'.$row["money_in"].'"></td></tr>';
				echo '<tr><td>Потрачено на рекламу:</td><td align="center"><input type="text" class="ok" name="money_rek" value="'.$row["money_rek"].'"></td></tr>';

				echo '<tr><td>На основном счету:</td><td align="center"><input type="text" class="ok" name="money" value="'.$row["money"].'"></td></tr>';
				echo '<tr><td>На рекламном счету:</td><td align="center"><input type="text" class="ok" name="money_rb" value="'.$row["money_rb"].'"></td></tr>';
				echo '<tr><td>Выплачено:</td><td align="center"><input type="text" class="ok" name="money_out" value="'.$row["money_out"].'"></td></tr>';
				echo '</table>';
			echo '</td>';
			echo '</tr>';
			...
			echo '</td>';
			echo '</tr>';
			echo '<tr><td colspan="3" align="center"><input type="submit" value="Cохранить" class="sub-blue160"></td></tr>';
			echo '</table>';
			echo '</form><br><br>';
			...
			if(mysql_num_rows($sql)>0) {
				while ($row = mysql_fetch_assoc($sql)) {
					echo '<tr align="center">';
						...
						echo '<td>'.((strtolower($row["method"])=="webmoney" | strtolower($row["method"])=="yandexmoney" | strtolower($row["method"])=="payeer" | strtolower($row["method"])=="perfectmoney" | strtolower($row["method"])=="qiwi" | strtolower($row["method"])=="sberbank") ? "Выплата на платежную систему <b>".str_replace("SberBank", "СБЕРБАНК", $row["method"])."</b>" : $row["method"]).'</td>';
						echo '<td>';
