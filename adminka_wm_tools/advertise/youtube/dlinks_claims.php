...
$perpage = 20;
$sql_p = mysql_query("SELECT `id` FROM `tb_ads_youtube` WHERE `status`>'0' AND `id` IN (SELECT `ident` FROM `tb_ads_claims` WHERE `type`='youtube') ");
...
$sql = mysql_query("SELECT * FROM `tb_ads_youtube` WHERE `status`>'0' AND `id` IN (SELECT `ident` FROM `tb_ads_claims` WHERE `type`='youtube') ORDER BY `id` DESC LIMIT $start_pos,$perpage ");
