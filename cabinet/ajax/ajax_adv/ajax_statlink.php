<?php
if(!DEFINED("STATLINK_AJAX")) {die ("Hacking attempt!");}

if($option == "delete") {
	$sql = mysql_query("SELECT * FROM `tb_ads_slink` WHERE `id`='$id' AND `username`='$username'");
	if(mysql_num_rows($sql)>0) {
		...
		if($status == 0) {
			mysql_query("DELETE FROM `tb_ads_slink` WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
			cache_stat_links();
			exit("OK");
	...
	if(mysql_num_rows($sql)>0) {
		...
		if(mysql_num_rows($sql_bl)>0 && $black_url!=false) {
			...
			if($status == 0 | $status == 3) {
				...
				mysql_query("UPDATE `tb_ads_slink` SET `color`='$color', `url`='$url', `description`='$description', `ip`='$laip'  WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
				cache_stat_links();
				...
				mysql_query("UPDATE `tb_ads_slink` SET `color`='$color_tab', `url`='$url', `description`='$description', `ip`='$laip'  WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
				cache_stat_links();
	...
	if(mysql_num_rows($sql)>0) {
		...
		$cena = $plan * ($color * $cena_slink_color + $cena_slink) * (100-$cab_skidka)/100;
		...
		if($plan<$slink_min) {
			...
			if($status=="0" | $status=="3") {
				...
				mysql_query("UPDATE `tb_ads_slink` SET `status`='1', `method_pay`='10', `plan`='$plan', `date`='".time()."', `date_end`='".(time()+$plan*24*60*60)."', `ip`='$laip', `money`='$money_pay'  WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
				mysql_query("INSERT INTO `tb_history` (`user`,`date`,`amount`,`method`,`status`,`tipo`) 
				VALUES('$username','".DATE("d.m.Yг. H:i")."','$money_pay','Пополнение баланса статической ссылки ID:$id','Списано','rashod')") or die(mysql_error());

				mysql_query("UPDATE `tb_ads_slink` SET `plan`=(`date_end`-`date`)/86400  WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
				...
				mysql_query("UPDATE `tb_users` SET `reiting`=`reiting`+'$reit_add_1', `money_rb`=`money_rb`-'$money_pay', `money_rek`=`money_rek`+'$money_pay'  WHERE `username`='$username'") or die(mysql_error());
				mysql_query("UPDATE `tb_ads_slink` SET `status`='1', `method_pay`='10', `plan`=`plan`+'$plan', `date_end`=`date_end`+'".($plan*24*60*60)."', `ip`='$laip', `money`=`money`+'$money_pay'  WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
				mysql_query("INSERT INTO `tb_history` (`user`,`date`,`amount`,`method`,`status`,`tipo`) 
				VALUES('$username','".DATE("d.m.Yг. H:i")."','$money_pay','Пополнение баланса статической ссылки ID:$id','Списано','rashod')") or die(mysql_error());
