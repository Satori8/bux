	...
	if(isset($_SESSION["userLog_a"]) && isset($_SESSION["userPas_a"])) {
		...
		if($option=="AddDelivery") {
			...
			if($title==false) {
				...
				$count_sent = 0;
				$sql = mysql_query("SELECT `id`,`username` FROM `tb_users` WHERE `investor`='1'") or die(mysql_error());
				if(mysql_num_rows($sql)>0) {
					while($row = mysql_fetch_assoc($sql)) {
						$count_sent++;

						mysql_query("INSERT INTO `tb_mail_in` (`namein`,`nameout`,`subject`,`message`,`status`,`date`,`ip`) 
						VALUES('".$row["username"]."','Система','$title','$description','0','".time()."','$laip')") or die(mysql_error());
