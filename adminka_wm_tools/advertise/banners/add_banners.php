...
if(count($_POST)>0 && isset($_POST["id_pay"])) {
	...
	$sql_id = mysql_query("SELECT `id`,`money` FROM `tb_ads_banner` WHERE `id`='$id_pay' AND `status`='0' AND `username`='$username' ORDER BY `id` DESC LIMIT 1");
	if(mysql_num_rows($sql_id)>0) {
		mysql_query("UPDATE `tb_ads_banner` SET `status`='1', `date`='".time()."', `date_end`=`date`+`plan`*'".(24*60*60)."' WHERE `id`='$id_pay' AND `status`='0' AND `username`='$username'  ORDER BY `id` DESC LIMIT 1") or die(mysql_error());
...
if(count($_POST)>0) {
	...
	if(array_key_exists($type_banner, $type_banner_arr) === false) {
		...
		if(mysql_num_rows($check_wmid)>0) {
			mysql_query("UPDATE `tb_ads_banner` SET `merch_tran_id`='0',`method_pay`='$method_pay',`wmid`='$wmid_user',`username`='$username',`type`='$type_banner',`plan`='$plan',`date`='".time()."',`date_end`='".(time()+$plan*24*60*60)."',`url`='$url',`urlbanner`='$urlbanner',`urlbanner_load`='$urlbanner_load',`ip`='$laip' WHERE `status`='0' AND `type`='$type_banner' AND `session_ident`='".session_id()."'") or die(mysql_error());
		}else{
			mysql_query("INSERT INTO `tb_ads_banner` (`status`,`session_ident`,`merch_tran_id`,`method_pay`,`wmid`,`username`,`type`,`plan`,`date`,`date_end`,`url`,`urlbanner`,`urlbanner_load`,`ip`) 
			VALUES('0','".session_id()."','0','$method_pay','$wmid_user','$username','$type_banner','$plan','".time()."','".(time()+$plan*24*60*60)."','$url','$urlbanner','$urlbanner_load','$laip')") or die(mysql_error());
		}
