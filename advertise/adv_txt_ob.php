...
if(isset($_SESSION["userLog"]) && isset($_SESSION["userPas"])) {
	...
	if(isset($_SESSION["userLog"]) && isset($_SESSION["userPas"]) && isset($my_referer)) {
		if($my_referer!=false) {
			$sql_p = mysql_query("SELECT `p_txt`,`discount_partner` FROM `tb_users_partner` WHERE `username`='$my_referer' AND `discount_partner`>'0'");
...
if(count($_POST)>0 && isset($_POST["id_pay"])) {
	if(!isset($_SESSION["userLog"]) && !isset($_SESSION["userPas"])) {
		...
		$sql_id = mysql_query("SELECT * FROM `tb_ads_txt` WHERE `id`='$id_pay' AND `status`='0' AND `username`='$username' ORDER BY `id` DESC LIMIT 1");
		if(mysql_num_rows($sql_id)>0) {
			...
			if($money_user>=$money_pay) {
				...
				mysql_query("UPDATE `tb_ads_txt` SET `status`='1', `date`='".time()."', `date_end`='".(time()+$plan*24*60*60)."', `wmid`='$wmid_user' WHERE `id`='$id_pay' AND `status`='0' AND `username`='$username' ORDER BY `id` DESC LIMIT 1") or die(mysql_error());
				mysql_query("INSERT INTO `tb_history` (`user`,`date`,`amount`,`method`,`status`,`tipo`) 
				VALUES('$username','".DATE("d.m.Yг. H:i")."','$money_pay',  'Оплата рекламы: Ттекстовое объявление($plan дн.)','Списано','rashod')") or die(mysql_error());
				...
				mysql_query("UPDATE `tb_users` SET `p_txt`='$add_percent_user' WHERE `username`='$username'") or die(mysql_error());

				if($my_referer!=false) {
					$sql_up = mysql_query("SELECT `p_txt` FROM `tb_users` WHERE `username`='$my_referer' AND `p_txt`>'0'");
					if(mysql_num_rows($sql_up)>0) {
						...
						mysql_query("INSERT INTO `tb_partner` (`time`, `username`, `referer`, `type`, `money`, `percent`) VALUES('".time()."','$username','$my_referer','p_txt','$money_add_referer', '$ref_p_txt')") or die("Error".mysql_error());
					}
				}
...
$system_pay[21] = "AdvCash";
		...
		mysql_query("DELETE FROM `tb_ads_txt` WHERE `status`='0' AND `date`<'".(time()-(24*60*60))."'") or die(mysql_error());
		...
		$check_wmid = mysql_query("SELECT `id` FROM `tb_ads_txt` WHERE `status`='0' AND `session_ident`='".session_id()."' ORDER BY `id` DESC LIMIT 1");
		if(mysql_num_rows($check_wmid)>0) {
			mysql_query("UPDATE `tb_ads_txt` SET `merch_tran_id`='$merch_tran_id', `method_pay`='$method_pay', `wmid`='$wmid_user', `username`='$username', `date`='".time()."', `date_end`='".(time()+$plan*24*60*60)."', `plan`='$plan', `url`='$url', `description`='$description', `ip`='$laip', `money`='$summa' WHERE `status`='0' AND `session_ident`='".session_id()."'") or die(mysql_error());
		}else{
			mysql_query("INSERT INTO `tb_ads_txt` (`status`,`session_ident`,`merch_tran_id`,`method_pay`,`wmid`,`username`,`date`,`date_end`,`plan`,`url`,`description`,`ip`,`money`) 
		...
		$sql_id = mysql_query("SELECT `id` FROM `tb_ads_txt` WHERE `status`='0' AND `session_ident`='".session_id()."' ORDER BY `id` DESC LIMIT 1");
