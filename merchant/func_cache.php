...
function cache_notification(){
	$sql = mysql_query("SELECT * FROM `tb_notification` WHERE `status`='1' ORDER BY `id` DESC");
	if(mysql_num_rows($sql)>0) {
	...
	mysql_query("ALTER TABLE `tb_notification` ORDER BY `id` ASC");
	mysql_query("OPTIMIZE TABLE `tb_notification`");
}
...
function cache_news(){
	...
	mysql_query("ALTER TABLE `tb_news` ORDER BY `id` ASC");
	mysql_query("OPTIMIZE TABLE `tb_news`");
}

function cache_hints(){
	$sql = mysql_query("SELECT * FROM `tb_hint_tips` ORDER BY `id` DESC");
	if(mysql_num_rows($sql)>0) {
		while($row = mysql_fetch_array($sql)) {
			$hints_arr[] = array(
				'id_hint' => $row["id"], 
	...
	mysql_query("ALTER TABLE `tb_hint_tips` ORDER BY `id` ASC");
	mysql_query("OPTIMIZE TABLE `tb_hint_tips`");
}

function cache_articles(){
	$sql = mysql_query("SELECT `id`,`username`,`date`,`title`,`url`,`desc_min`,`desc_big`,`views` FROM `tb_ads_articles` WHERE `status`='1' ORDER BY `up_list` DESC, `id` DESC");
	if(mysql_num_rows($sql)>0) {
		while($row = mysql_fetch_assoc($sql)) {
	...
	mysql_query("ALTER TABLE `tb_ads_articles` ORDER BY `id` ASC");
	mysql_query("OPTIMIZE TABLE `tb_ads_articles`");
}
