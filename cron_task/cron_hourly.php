		...
		if(mysql_num_rows($sql)>0) {
			...
			while($row = mysql_fetch_array($sql)) {
				...
				if($_ADD_PRIZES_USERS==1) {
					...
					VALUES('1','".$row["username"]."','".$row["id"]."','".DATE("d.m.Y H:i")."','".time()."','$_ADD_MONEY','Приз за <b>$_MESTO</b> место в конкурсе активных рефералов <b>№$k_nomer</b><br>$_Prizes_Txt_Tab','Зачислено','prihod','1')") or die(mysql_error());
		...
		if(mysql_num_rows($sql)>0) {
			...
			while($row = mysql_fetch_array($sql)) {
				...
				if($_ADD_PRIZES_USERS==1) {
					...
					mysql_query("INSERT INTO `tb_history` (`status_pay`,`user`,`user_id`,`date`,`time`,`amount`,`method`,`status`,`tipo`,`type`) 
					VALUES('1','".$row["username"]."','".$row["id"]."','".DATE("d.m.Y H:i")."','".time()."','$_ADD_MONEY','Приз за <b>$_MESTO</b> место в конкурсе рекламодателей <b>№$k_nomer</b><br>$_Prizes_Txt_Tab','зачислено','prihod','1')") or die(mysql_error());
		...
		if(mysql_num_rows($sql)>0) {
			...
			while($row = mysql_fetch_assoc($sql)) {
				...
				if($_ADD_USERS_TAB!==false) {
					...
					mysql_query("INSERT INTO `tb_history` (`status_pay`,`user`,`user_id`,`date`,`time`,`amount`,`method`,`status`,`tipo`,`type`) 
					VALUES('1','".$row["username"]."','".$row["id"]."','".DATE("d.m.Y H:i")."','".time()."','".($_ADD_MONEY+$_ADD_MONEY_RB)."','Приз за <b>$_MESTO</b> место в конкурсе рекламодателей[2] <b>№$k_nomer</b><br>$_Prizes_Txt_Tab','Зачислено','prihod','1')") or die(mysql_error());
				}
			}
		}
		...
		if(mysql_num_rows($sql)>0) {
			...
			while($row = mysql_fetch_array($sql)) {
				...
				if($_ADD_PRIZES_USERS==1) {
					...
					mysql_query("INSERT INTO `tb_history` (`status_pay`,`user`,`user_id`,`date`,`time`,`amount`,`method`,`status`,`tipo`,`type`) 
					VALUES('1','".$row["username"]."','".$row["id"]."','".DATE("d.m.Y H:i")."','".time()."','$_ADD_MONEY','Приз за <b>$_MESTO</b> место в конкурсе кликеров <b>№$k_nomer</b><br>$_Prizes_Txt_Tab','Зачислено','prihod','1')") or die(mysql_error());
		...
		if(mysql_num_rows($sql)>0) {
			...
			while($row = mysql_fetch_array($sql)) {
				...
				if($_ADD_PRIZES_USERS==1) {
					...
					mysql_query("INSERT INTO `tb_history` (`status_pay`,`user`,`user_id`,`date`,`time`,`amount`,`method`,`status`,`tipo`,`type`) 
					VALUES('1','".$row["username"]."','".$row["id"]."','".DATE("d.m.Y H:i")."','".time()."','$_ADD_MONEY','Приз за <b>$_MESTO</b> место в конкурсе по прохождению тестов <b>№$k_nomer</b><br>$_Prizes_Txt_Tab','Зачислено','prihod','1')") or die(mysql_error());
		...
		if(mysql_num_rows($sql)>0) {
			...
			while($row = mysql_fetch_array($sql)) {
				...
				if($_ADD_PRIZES_USERS==1) {
					...
					mysql_query("INSERT INTO `tb_history` (`status_pay`,`user`,`user_id`,`date`,`time`,`amount`,`method`,`status`,`tipo`,`type`) 
					VALUES('1','".$row["username"]."','".$row["id"]."','".DATE("d.m.Y H:i")."','".time()."','$_ADD_MONEY','Приз за <b>$_MESTO</b> место в конкурсе YouTube <b>№$k_nomer</b><br>$_Prizes_Txt_Tab','Зачислено','prihod','1')") or die(mysql_error());
		...
		if(mysql_num_rows($sql)>0) {
			...
			while($row = mysql_fetch_array($sql)) {
				...
				if($_ADD_PRIZES_USERS==1) {
					...
					mysql_query("INSERT INTO `tb_history` (`status_pay`,`user`,`user_id`,`date`,`time`,`amount`,`method`,`status`,`tipo`,`type`) 
					VALUES('1','".$row["username"]."','".$row["id"]."','".DATE("d.m.Y H:i")."','".time()."','$_ADD_MONEY','Приз за <b>$_MESTO</b> место в конкурсе по выполнению заданий <b>№$k_nomer</b><br>$_Prizes_Txt_Tab','Зачислено','prihod','1')") or die(mysql_error());
		...
		if(mysql_num_rows($sql)>0) {
			...
			while($row = mysql_fetch_assoc($sql)) {
				...
				if($_ADD_USERS_TAB!==false) {
					...
					mysql_query("INSERT INTO `tb_history` (`status_pay`,`user`,`user_id`,`date`,`time`,`amount`,`method`,`status`,`tipo`,`type`) 
					VALUES('1','".$row["username"]."','".$row["id"]."','".DATE("d.m.Y H:i")."','".time()."','".($_ADD_MONEY+$_ADD_MONEY_RB)."','Приз за <b>$_MESTO</b> место в комплексном конкурсе <b>№$k_nomer</b><br>$_Prizes_Txt_Tab','Зачислено','prihod','1')") or die(mysql_error());
		...
		if(mysql_num_rows($sql)>0) {
			...
			while($row = mysql_fetch_assoc($sql)) {
				...
				if($_ADD_USERS_TAB!==false) {
					...
					mysql_query("INSERT INTO `tb_history` (`status_pay`,`user`,`user_id`,`date`,`time`,`amount`,`method`,`status`,`tipo`,`type`) 
					VALUES('1','".$row["username"]."','".$row["id"]."','".DATE("d.m.Y H:i")."','".time()."','$_ADD_MONEY','Приз за <b>$_MESTO</b> место в конкурсе по размещению ссылок в серфинге <b>№$k_nomer</b><br>$_Prizes_Txt_Tab','Зачислено','prihod','1')") or die(mysql_error());
	...
	while ($row_t = mysql_fetch_array($sql_t)) {
		...
		if($count_s>0) {
			while ($row_s = mysql_fetch_array($sql_s)) {
				...
				VALUES('".$row_s["username"]."', '".DATE("d.m.Yг. H:i")."', '$p[$m]',  'Приз за $m место в конкурсе № ".$row_t["id"]." от реферера ".$row_t["username"]."','Зачислено','prihod','2')") or die(mysql_error());
			}
		}
		...
		if($count_s<$row_t["count_kon"]) {
			...
			VALUES('".$row_t["username"]."', '".DATE("d.m.Yг. H:i")."', '$money_back',  'Возврат средств за конкурс для рефералов','Зачислено','prihod')") or die(mysql_error());
		}
	}
}
### КОНКУРСЫ ОТ РЕФЕРЕРОВ ###
