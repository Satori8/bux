<?php
if(!DEFINED("REKCEP_EDIT")) {die ("Hacking attempt!");}

	echo '<a name="goto"></a>';
	echo '<h5 class="sp" style="margin-top:0px; font-size:14px; font-weight:bold; text-align:center;">Редактирование рекламной цепочки № '.$id.'</h5>';
	...
	if(mysql_num_rows($sql)>0) {
		...
		function obsch(){
			...
			var cena_link = <?php echo $cena_rek_cep;?>;
				...
				echo '<td>';
					if($row["status"]=="0" || $row["status"]=="3") {
						echo '<select id="color" onChange="obsch();" onClick="obsch();">';
							...
							echo '<option value="1" '.("".$row["color"]."" == "1" ? 'selected="selected"' : false).'>Да (+'.number_format($cena_color_rek_cep,2,".","'").' руб./сутки)</option>';
		...
		echo '<div align="center"><span onClick="save_ads('.$row["id"].', \'rekcep\');" class="proc-btn" style="float:none; width:160px;">Сохранить</span></div>';
