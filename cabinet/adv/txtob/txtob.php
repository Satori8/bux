...
if(isset($ads) && isset($id) && isset($op) && $ads=="txtob" && $op=="edit" && $id>0) {
	if(!DEFINED("TXTOB_EDIT")) DEFINE("TXTOB_EDIT", true);
	include("txtob_edit.php");
	include(DOC_ROOT."/footer.php");
	exit();
}

echo '<a name="goto"></a>';
echo '<h5 class="sp" style="margin-top:0px; font-size:14px; font-weight:bold; text-align:center;">Мои текстовые объявления</h5>';
...
function pay_adv(id, type) {
	...
	if (plan == '') {
		...
		gebi("imfo-msg-addmoney"+id).style.display = 'block';
		setTimeout(function() {$("#imfo-msg-addmoney"+id).fadeOut('slow')}, 2000); clearTimeout();
	} else if (rplan < <?=$min_txt_ob;?>) {
		gebi("imfo-msg-addmoney"+id).innerHTML = "<span class='msg-error'>Количество дней должно быть не менее <?=$min_txt_ob;?></span>";
		gebi("imfo-msg-addmoney"+id).style.display = 'block';
		setTimeout(function() {$("#imfo-msg-addmoney"+id).fadeOut('slow')}, 2000); clearTimeout();
	} else {
...
$sql = mysql_query("SELECT * FROM `tb_ads_txt` WHERE `username`='$username' ORDER BY `id` ASC");
if(mysql_num_rows($sql)>0) {
	...
	while ($row = mysql_fetch_array($sql)) {
		...
		echo '<tr id="adv_dell'.$row["id"].'">';
			...
			echo '<td align="left" class="noborder2">';
				...
				echo '</span>';
				echo '<span class="adv-dell" title="Удалить ссылку" onClick="alert_delete('.$row["id"].', \'txtob\');"></span>';
		...
		echo '<tr id="adv_addmoney'.$row["id"].'" style="display: none">';
			...
			function obsch<?php echo $row["id"];?>(){
				...
				plan<?php echo $row["id"];?> = str_replace("-", "", plan<?php echo $row["id"];?>);
				var cena_link<?php echo $row["id"];?> = <?php echo $cena_txt_ob;?>;
			...
			echo '<td align="center" colspan="3" class="ext-text">';
				...
				echo 'Укажите количество дней, которые вы хотите внести в бюджет рекламной площадки<br>(Минимум '.count_text($min_txt_ob, "дней", "день", "дня", "").')';
				...
				echo '<center><span onClick="pay_adv('.$row["id"].', \'txtob\');" class="sub-green" style="float:none;" title="Пополнить бюджет площадки">Оплатить</span></center>';
	...
	echo '<span class="msg-w">У вас нет своих размещённых текстовых объявлений</span>';
}

echo '<div align="center"><span class="proc-btn" onClick="document.location.href=\'/advertise.php?ads=txt_ob\'">Разместить ссылку</span></div>';
?>