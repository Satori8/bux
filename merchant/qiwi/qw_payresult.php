...
require($_SERVER["DOCUMENT_ROOT"]."/merchant/qiwi/qw_config.php");
include($_SERVER["DOCUMENT_ROOT"]."/merchant/qiwi/IShopServerWSService.php");

define('TRACE', 1);
$service = new IShopServerWSService($_SERVER["DOCUMENT_ROOT"].'/merchant/qiwi/IShopServerWS.wsdl', array('location' => 'http://ishop.qiwi.ru/services/ishop', 'trace' => TRACE));

function checkBill($txn_id) {
	global $service;
	$params = new checkBill();
	$params->login = API_ID_QW;
	$params->password = API_PASSWORD_QW;
	$params->txn = $txn_id;
	
	$res = $service->checkBill($params);

	$check["user"] = trim($res->user);
	$check["amount"] = trim($res->amount);
	$check["date"] = trim($res->date);
	$check["lifetime"] = trim($res->lifetime);
	$check["status"] = trim($res->status);

	return $check;
}

$s = new SoapServer($_SERVER["DOCUMENT_ROOT"].'/merchant/qiwi/IShopClientWS.wsdl', array('classmap' => array('tns:updateBill' => 'Param', 'tns:updateBillResponse' => 'Response')));
...
class TestServer {
	...
	function updateBill($param) {
		$pay_tran_id = trim($param->txn);
		$pay_status = intval(trim($param->status));
		$pay_login = intval(trim($param->login));
		$pay_password = trim($param->password);
		$pay_hash = strtoupper(md5($pay_tran_id.strtoupper(md5(API_PASSWORD_QW))));

		if($pay_login == HTTP_LOGIN_QW && $pay_password == $pay_hash) {
			if($pay_tran_id!=false) {
				$exp = explode(":", $pay_tran_id);
				$shp_item = $exp[0];
				$merch_tran_id = $exp[1];
			}else{
				$merch_tran_id = false;
			...
			if($pay_status == 60) {
				...
				if($shp_item!=false && array_key_exists($shp_item, $TABLE_ARR)) {
					...
					if(mysql_num_rows($sql)) {
						...
						$check_money = checkBill($pay_tran_id);
						$check_money = $check_money["amount"];

						if(floatval($check_money)!=floatval($merch_amount)) {
							mysql_query("UPDATE `".$TABLE_ARR[$shp_item]."` SET `status`='-100' WHERE `status`='0' AND `merch_tran_id`='$merch_tran_id' ORDER BY `id` DESC LIMIT 1") or die(mysql_error());
				...
				if($bon_pay_status==1){
					if($merch_amount1>=$bon_pay_summa){
						mysql_query("UPDATE `tb_users` SET `money_rb`=`money_rb`+'".$bon_pay_money."', `reiting`=`reiting`+'".$bon_pay_reiting."' WHERE `username`='$username'") or die(mysql_error());
								...
								VALUES('$username', '$merch_amount1', '".DATE("d.m.Yг. H:i")."', 'Пополнение рекламного баланса через Qiwi','Зачислено', 'popoln')") or die(mysql_error());
				...
				if($bon_inv_status==1){
					if($merch_amount>=$bon_inv_summa){
						mysql_query("UPDATE `tb_users` SET `money_inv`=`money_inv`+'".$bon_inv_money."', `reiting`=`reiting`+'".$bon_inv_reiting."' WHERE `username`='$username'") or die(mysql_error());
								...
								mysql_query("INSERT INTO `tb_history` (`user`, `amount`, `date`, `method`, `status`, `tipo`) 
								VALUES('$username', '$merch_amount', '".DATE("d.m.Yг. H:i")."', 'Пополнение баланса инвестора через Qiwi','Зачислено', 'popoln')") or die(mysql_error());
