...
$perpage = 25;
$sql_p = mysql_query("SELECT `id` FROM `tb_ads_psevdo` WHERE `status`>'0' $WHERE_ADD");
...
if(isset($_GET["option"])) {
	...
	if($option=="edit") {
		...
		if(count($_POST)>0) {
			...
			if(mysql_num_rows($sql_bl)>0 && $black_url!=false) {
				...
				$save = "ok";
				mysql_query("UPDATE `tb_ads_psevdo` SET `wmid`='$wmid', `username`='$username', `date_end`=`date`+'".($plan*24*60*60)."', `plan`='$plan', `url`='$url', `description`='$description', `color`='$color' WHERE `id`='$id'") or die(mysql_error());
		...
		if(!isset($save)) {
			...
			$sql = mysql_query("SELECT * FROM `tb_ads_psevdo` WHERE `id`='$id' AND `status`='1' ORDER BY `id` ASC");
	...
	if($option=="dell") {
		mysql_query("DELETE FROM `tb_ads_psevdo` WHERE `id`='$id'") or die(mysql_error());
...
$sql_ads = mysql_query("SELECT * FROM `tb_ads_psevdo` WHERE `status`>'0' $WHERE_ADD ORDER BY `id` DESC LIMIT $start_pos, $perpage") or die(mysql_error());
