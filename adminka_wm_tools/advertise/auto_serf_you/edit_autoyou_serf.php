...
mysql_query("UPDATE `tb_ads_autoyoutube` SET `status`='3', `date`='".time()."' WHERE `status`>'0' AND `status`<'3' AND `totals`<'1' ") or die(mysql_error());
...
$perpage = 20;
$sql_p = mysql_query("SELECT `id` FROM `tb_ads_autoyoutube` WHERE `status`>'0' $WHERE_ADD");
...
if(isset($_GET["option"])) {
	...
	if($country_to!=false) {$country_to="$country_to";}else{$country_to="НЕТ";}
	    ...
	    $ytflag=1;
				...
				$sql_totals = mysql_query("SELECT `plan`,`totals` FROM `tb_ads_autoyoutube` WHERE `id`='$id'");
				if(mysql_num_rows($sql_totals)>0) {
					...
					mysql_query("UPDATE `tb_ads_autoyoutube` SET `wmid`='$wmid',`username`='$username',`timer`='$timer',`plan`='$new_plan',`totals`='$new_totals',`limits`='$limits_table',`limits_now`='$limits_table',`url`='$url',`description`='$description' WHERE `id`='$id'") or die(mysql_error());
				}
		...
		if(!isset($save)) {
			...
			$sql = mysql_query("SELECT * FROM `tb_ads_autoyoutube` WHERE `id`='$id' ORDER BY `id` ASC");
	...
	if($option=="dell") {
		$sql = mysql_query("SELECT * FROM `tb_ads_autoyoutube` WHERE `id`='$id' AND `status`>'0'");
		if(mysql_num_rows($sql)>0) {
			mysql_query("DELETE FROM `tb_ads_autoyoutube` WHERE `id`='$id'") or die(mysql_error());
...
$sql_ads = mysql_query("SELECT * FROM `tb_ads_autoyoutube` WHERE `status`>'0' $WHERE_ADD ORDER BY `id` DESC LIMIT $start_pos, $perpage") or die(mysql_error());
