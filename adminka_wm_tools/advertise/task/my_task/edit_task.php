...
if(count($_POST) > 0) {
	...
	elseif($zdcheck==2 && ($zdquest==false | $zdotv==false | strlen($zdquest) < 4 | strlen($zdotv) < 2) ) {
		...
		if(mysql_num_rows($sql)>0) {
			...
			mysql_query("UPDATE `tb_ads_task` SET `country_targ`='$zdcountry',`user_id`='$partnerid',`zdname`='$zdname',`zdtext`='$zdtext',`zdurl`='$zdurl',`zdtype`='$zdtype',`zdre`='$zdre',`zdquest`='$zdquest',`zdotv`='$zdotv',`zdprice`='$zdprice',`zdreit_us`='$zdreit',`ip`='$ip' WHERE  `id`='$rid' AND `username`='$username'") or die(mysql_error());
...
echo '<table align="center" border="0" width="100%" cellspacing="3" cellpadding="2" style="border-collapse: collapse; border: 1px solid #1E90FF;">';
	...
	echo '<tr bgcolor="#AFEEEE">';
		...
		echo '<td>&nbsp;<select name="zdtype">
			...
			<option value="9" '.$sel_type9.'>Социальные сети</option>
			<option value="10" '.$sel_type10.'>YouTube</option>
			<option value="11" '.$sel_type11.'>Работа с капчей</option>
