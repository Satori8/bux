...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) && $_SERVER["HTTP_X_REQUESTED_WITH"]=="XMLHttpRequest") {
	...
	if(isset($_SESSION["userLog"]) && isset($_SESSION["userPas"])) {
		...
		if(isset($_SESSION["captcha_site_sess"])) unset($_SESSION["captcha_site_sess"]);
		...
		if($username==false) {
			...
			$message_text = "Необходимо подтвердить, что Вы не робот!";
			$js_result = ($ajax_json=="json") ? json_encode_cp1251(array("result" => "ERROR", "message" => iconv("CP1251", "UTF-8", $message_text))) : $message_text;
			exit($js_result);
		}elseif($captcha_sess!=$captcha_post) {
			$message_text = "Задача решена не верно!";
			$js_result = ($ajax_json=="json") ? json_encode_cp1251(array("result" => "ERROR", "message" => iconv("CP1251", "UTF-8", $message_text))) : $message_text;
			exit($js_result);
		}else{
			...
			if(mysql_num_rows($sql)>0) {
				...
				$ip = $row["ip"];
				$country_cod = $row["country_cod"];
				$agent = isset($_SERVER["HTTP_USER_AGENT"]) ? $_SERVER["HTTP_USER_AGENT"] : false;
				
				//$sql_country_code = mysql_query("SELECT `country_cod` FROM `tb_users` WHERE `username`='$username' AND `country_cod`='$country_code'");
				...
				if($my_ban_date > 0) {
					...
					exit($js_result);
				/*}elseif($country_cod=="" ) {
					$message_text = "Страна не определена авторизация не возможна!!!";
					$js_result = ($ajax_json=="json") ? json_encode_cp1251(array("result" => "ERROR", "message" => iconv("CP1251", "UTF-8", $message_text))) : $message_text;
					exit($js_result);
				//}elseif(mysql_num_rows($sql_country_code)>0) {
					...
					/*### УВЕДОМЛЕНИЕ НА E-MAIL О ВХОДЕ В АККАУНТ ###
					require_once($_SERVER['DOCUMENT_ROOT'].'/class/email.conf.php');
			    require_once($_SERVER['DOCUMENT_ROOT'].'/class/smtp.class.php');
			    
			    $var = array('{EMAIL}','{LOGIN}','{IP}');
          $zamena = array($email_user, $username, $ip);
	        $msgtext = str_replace($var, $zamena, $email_temp["login"]);
					
					$mail_out = new mailPHP();   
					$mail_error = $mail_out->send($email_user, $username, iconv("CP1251", "UTF-8", 'Вход в аккаунт '.$_SERVER["HTTP_HOST"]), iconv("CP1251", "UTF-8", $msgtext));
