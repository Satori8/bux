<?php
if(!DEFINED("YOUAUTOSERF_AJAX")) {die ("Hacking attempt!");}
...
if($option == "play_pause") {
	...
	if(mysql_num_rows($sql)>0) {
		...
		if($status == 0 | $status == 3) {
			...
			if($limits_now<1) {
				...
				mysql_query("UPDATE `tb_ads_autoyoutube` SET `status`='2' WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
				echo '<span class="adv-play" title="Запустить показ рекламной площадки" onClick="play_pause('.$row["id"].', \'youautoserf\');"></span>';
				exit();
			}
		}elseif($status == 2) {
			mysql_query("UPDATE `tb_ads_autoyoutube` SET `status`='1' WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
			if($limits_now<1) {
				exit("NOLIMIT");
			}else{
				echo '<span class="adv-pause" title="Приостановить показ рекламной площадки" onClick="play_pause('.$row["id"].', \'youautoserf\');"></span>';
				exit();
			}
		}else{
			exit("ERROR");
	...
	if(mysql_num_rows($sql)>0) {
		...
		if($status == 0) {
			mysql_query("DELETE FROM `tb_ads_autoyoutube` WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
			exit("OK");
	...
	if(mysql_num_rows($sql)>0) {
		...
		if(mysql_num_rows($sql_bl)>0 && $black_url!=false) {
			...
			echo 'Не верно указана ссылка на видеоролик youtube!';
			exit();
		...
		//exit();
			...
			echo 'Не заполнено поле Описание видеоролика.';
			exit("");
			...
			if($limits<0) {
				...
				if($timer<$timer_aserf_ot | $timer>$timer_aserf_do) {
					...
					mysql_query("UPDATE `tb_ads_autoyoutube` SET `img_youtube`='$img_youtube',`geo_targ`='$country',`url`='$url', `description`='$description', `timer`='$timer', `limits`='$limits', `limits_now`='$limits', `ip`='$laip'  WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
					exit("OK");
				}
	...
	if(mysql_num_rows($sql)>0) {
		...
		if($status == 0) {
			...
			mysql_query("UPDATE `tb_ads_autoyoutube` SET `outside`='0', `ip`='$laip'  WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
			exit("OK");
		}
	}else{
		exit("У Вас нет рекламной площадки с ID - $id");
...
$cena_timer_aserf = mysql_result($sql,0,0);
		...
		if($plan<$min_hits_aserf) {
			...
			if($status=="0") {
				...
				mysql_query("UPDATE `tb_ads_autoyoutube` SET `status`='1', `method_pay`='10', `date`='".time()."', `plan`='$plan', `totals`='$plan', `ip`='$laip', `money`='$money_pay'  WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
				mysql_query("INSERT INTO `tb_history` (`user`,`date`,`amount`,`method`,`status`,`tipo`) 
				VALUES('$username','".DATE("d.m.Yг. H:i")."','$money_pay','Пополнение баланса видеоролика в авто-серфинг YouTube ID:$id','Списано','rashod')") or die(mysql_error());

				if($limits==0) mysql_query("UPDATE `tb_ads_autoyoutube` SET `limits`=`plan`, `limits_now`=`plan` WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
				...
				mysql_query("UPDATE `tb_users` SET `reiting`=`reiting`+'$reit_add_1', `money_rb`=`money_rb`-'$money_pay', `money_rek`=`money_rek`+'$money_pay'  WHERE `username`='$username'") or die(mysql_error());
				mysql_query("UPDATE `tb_ads_autoyoutube` SET `method_pay`='10', `date`='".time()."', `plan`=`plan`+'$plan', `totals`=`totals`+'$plan', `ip`='$laip', `money`=`money`+'$money_pay'  WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
				mysql_query("INSERT INTO `tb_history` (`user`,`date`,`amount`,`method`,`status`,`tipo`) 
				VALUES('$username','".DATE("d.m.Yг. H:i")."','$money_pay','Пополнение баланса видеоролика в авто-серфинг YouTube ID:$id','Списано','rashod')") or die(mysql_error());

				if($limits==0) mysql_query("UPDATE `tb_ads_autoyoutube` SET `limits`=`plan` WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
				...
				mysql_query("UPDATE `tb_users` SET `reiting`=`reiting`+'$reit_add_1', `money_rb`=`money_rb`-'$money_pay', `money_rek`=`money_rek`+'$money_pay'  WHERE `username`='$username'") or die(mysql_error());
				mysql_query("UPDATE `tb_ads_autoyoutube` SET `status`='1', `method_pay`='10', `date`='".time()."', `plan`=`plan`+'$plan', `totals`=`totals`+'$plan', `ip`='$laip', `money`=`money`+'$money_pay'  WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
				mysql_query("INSERT INTO `tb_history` (`user`,`date`,`amount`,`method`,`status`,`tipo`) 
				VALUES('$username','".DATE("d.m.Yг. H:i")."','$money_pay','Пополнение баланса видеоролика в авто-серфинг YouTube ID:$id','Списано','rashod')") or die(mysql_error());

				if($limits==0) mysql_query("UPDATE `tb_ads_autoyoutube` SET `limits`=`plan` WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());

				@stat_pay('autoserfyou', $money_pay, 2);
				@ads_wmid($wmid_user, $wmr_user, $username, $money_pay);
				@konkurs_ads_new($wmid_user, $username, $money_pay);
