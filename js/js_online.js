...
function ModalStart(title, content, width, closeOnEsc, closeOnOverlayClick, timerAutoClose) {
	...
	if(timerAutoClose && $("div").is(".box-modal")) {setTimeout(function(){$.modalpopup("close");},timerAutoClose*1000);}
...
function InfoMsg(elem, status, message, timer) {
	clearTimeout(TmInfo);

	if (status == "OK") {
		message = '<span class="msg-ok">'+message+'</span>';
	} else if (status == "WARNING") {
		message = '<span class="msg-w">'+message+'</span>';
	} else if (status == "ERROR") {
		message = '<span class="msg-error">'+message+'</span>';
	}

	$(elem).html(message).animate({opacity:"show"}, 550);
	TmInfo = setTimeout(function(){$(elem).animate({opacity:"hide"}, 550, function(){$(elem).html("");});}, timer);
}
...
function PostMess(event){
	...
	if(data.id && data.op && data.mess && data.op == "serf_mess") {
		$("#"+data.id).html('<div style="text-align:center;" class="text-green">'+data.mess+'</div>');
		setTimeout(function(){
			$("#tr"+data.id).fadeOut(300, function () {$(this).remove();});
			$("#info_serf_"+data.id).remove();
			$("#claims_"+data.id).remove();
		}, 15000);
	} else if(data.op == "pv_mess") {
