...
$pagetitle="Автоматический серфинг";
include('header.php');
...
if(!isset($_SESSION["userLog"]) && !isset($_SESSION["userPas"])) {
	...
	require('config.php');
	mysql_query("DELETE FROM `tb_ads_auto_visits` WHERE `date`<'".(time()-72*60*60)."'") or die(mysql_error());
	mysql_query("UPDATE `tb_ads_auto` SET `status`='3' WHERE `status`>'0' AND `totals`<'1' ") or die(mysql_error());

	$sql = mysql_query("SELECT `id` FROM `tb_ads_auto` WHERE `status`='1' AND `totals`>'0' AND `limits_now`>'0' AND `id` NOT IN (SELECT `ident` FROM `tb_ads_auto_visits` WHERE `username`='$username' AND `date`>='".(time()-24*60*60)."') ORDER BY `id` DESC");
	$kolvo_site = mysql_num_rows($sql);

	echo 'Доступно сайтов для просмотра: <b>'.$kolvo_site.'</b><br><br>';
	//if($kolvo_site>0) echo '<span class="proc-btn" target="_blank" onClick="document.location.href=\'http://'.$_SERVER["HTTP_HOST"].'/autoserf/auto.php\' " target="_blank" style="text-align:center;">Начать просмотр</span><br><br>';
	if($kolvo_site>0) echo '<a href="https://'.$_SERVER["HTTP_HOST"].'/autoserf/auto.php" target="_blank"><input type="submit" value="Начать просмотр" class="proc-btn" style="float:none;" /></a><br><br><br>';
	else echo '<br>';
	}else{
