	...
	if(!isset($_SESSION["userLog"]) && !isset($_SESSION["userPas"])) {
		...
		if(mysql_num_rows($sql_id)>0) {
			...
			if($money_user_rb >= $money_pay) {
				...
				VALUES('$username','".DATE("d.m.Yг. H:i")."','$money_pay',  'Оплата пакета рекламы №$packet','Списано','rashod')") or die(mysql_error());
...
$system_pay[21] = "AdvCash";
		...
		echo '<table class="tables">';
			...
			if(isset($my_discount[$packet]) && $my_discount[$packet]>0) echo '<tr><td><b>Скидка от вашего реферера:</b></td><td>'.$my_discount[$packet].'%</td></tr>';
					...
					echo '<tr>';
						echo '<td colspan="2" align="center">';
							echo '<form method="GET" action="'.PHP_SELF.'">';
								...
								echo '<input type="submit" value="Купить пакет рекламы №'.$row["packet"].'" class="proc-btn" style="float:none;" />';
							echo '</form>';
						echo '</td>';
					echo '</tr>';
				echo '</tbody>';
