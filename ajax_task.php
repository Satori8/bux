...
<div class="text-popup" id="js-popup">
	...
	<button><span class="proc-btn-t" style="width: 200px;" onclick="$('#zero-del').submit();return false;">Да</span></button>
	    <form method="post" action="/ajax/ajax_task.php" class="aj-form_z" id="zero-del" data-infa="true,<?=$rid;?>,">
      <input name="func" type="hidden" value="zero_del">
	  <input name="id" type="hidden" value="<?=$rid;?>">
    </form>
	<!--<span class="proc-btn-t" onclick="task_fun($('#task-block<?=$id_z;?>'), '../ajax/ajax_task.php', 'func=color_task&id=<?=$id_z;?>');return false;">Да</span>  -->
		...
		if(mysql_num_rows($sql_p)>0) {
			...
			if(isset($_POST["ask_reply"])) {
				if($row_p["status"]=="" || $row_p["status"]=="dorab") {
					if($ctext==false) {
						$ajax_json="json"; $result_text = "error"; $message_text = '<span class="msg-error">Вы не указали информацию (из описания задания) для проверки задания рекламодателем!</span>';
						exit(my_json_encode($ajax_json, $result_text, $message_text));
					}else{
						if($zdcheck==2) {
...
if(mysql_num_rows($sql_zv)>0) {
	...
	$sql_f = mysql_query("SELECT * FROM `tb_ads_task_fav` WHERE `user_id`='$partnerid' AND `rek_id`='$rek_id'");
	
	if (mysql_num_rows($sql_f) > 0) {
		mysql_query("UPDATE `tb_ads_task_fav` SET `type`='favorite' WHERE `user_id`='$partnerid' AND `rek_id`='$rek_id'") or die(mysql_error());
		$ajax_json="json"; $result_text = "ok"; $message_text = '<span class="msg-ok">Задание добавлено в избранные!</span>';
	    exit(my_json_encode($ajax_json, $result_text, $message_text));
	}else{

		mysql_query("INSERT INTO `tb_ads_task_fav` (`type`,`user_id`,`rek_id`,`date_add`,`ip`) VALUES ('favorite','$partnerid','$rek_id','".time()."','$ip')");
		$ajax_json="json"; $result_text = "ok"; $message_text = '<span class="msg-ok">Задание добавлено в избранные!</span>';
	    exit(my_json_encode($ajax_json, $result_text, $message_text));
	}
...
if(mysql_num_rows($sql_zv)>0) {
	...
	$sql_f = mysql_query("SELECT * FROM `tb_ads_task_fav` WHERE `type`='favorite' AND `user_id`='$partnerid' AND `rek_id`='$rek_id'");
	if (mysql_num_rows($sql_f) > 0) {

		mysql_query("DELETE FROM `tb_ads_task_fav` WHERE `type`='favorite' AND `user_id`='$partnerid' AND `rek_id`='$rek_id'") or die(mysql_error());
...
<div class="text-popup" id="js-popup">
    ...
    <div style="text-align: center;margin-bottom: 10px;">	  
	<span class="proc-btn-t" onclick="task_fun($('#task-block<?=$id_z;?>'), '../ajax/ajax_task.php', 'func=vip_task&id=<?=$id_z;?>');return false;">Да</span>  
    <input type="submit" class="proc-btn-n" value="Нет" onclick="closed_popup();return false;">
    </div>     
</div>
<?
...
<div class="text-popup" id="js-popup">
    ...
    <div style="text-align: center;margin-bottom: 10px;">	  
	<span class="proc-btn-t" onclick="task_fun($('#task-block<?=$id_z;?>'), '../ajax/ajax_task.php', 'func=up_task&id=<?=$id_z;?>');return false;">Да</span>  
    <input type="submit" class="proc-btn-n" value="Нет" onclick="closed_popup();return false;">
    </div>     
</div>
<?
...
<div class="text-popup" id="js-popup">
    ...
    <div style="text-align: center;margin-bottom: 10px;">	  
	<span class="proc-btn-t" onclick="task_fun_del($('#task-block<?=$id_z;?>'), '../ajax/ajax_task.php', 'func=vozvrat_task&id=<?=$id_z;?>');return false;">Да</span>  
    <input type="submit" class="proc-btn-n" value="Нет" onclick="closed_popup();return false;">
    </div>     
</div>
<?
...
<div class="text-popup" id="js-popup">
    ...
    <div style="text-align: center;margin-bottom: 10px;">	  
	<span class="proc-btn-t" onclick="task_fun($('#task-block<?=$id_z;?>'), '../ajax/ajax_task.php', 'func=perevod_task&id=<?=$id_z;?>&plan=<?=$add_plan;?>&z_popoln=<?=$z_popoln;?>');return false;">Да</span>  
    <input type="submit" class="proc-btn-n" value="Нет" onclick="closed_popup();return false;">
    </div>     
</div>
<?
...
<div class="text-popup" id="js-popup">
    ...
    <div style="text-align: center;margin-bottom: 10px;">	  
	<span class="proc-btn-t" onclick="task_fun($('#task-block<?=$id_z;?>'), '../ajax/ajax_task.php', 'func=money_task&id=<?=$id_z;?>&plan=<?=$add_plan;?>&system=<?=$system;?>');return false;">Да</span>  
    <input type="submit" class="proc-btn-n" value="Нет" onclick="closed_popup();return false;">
    </div>     
</div>
<?
