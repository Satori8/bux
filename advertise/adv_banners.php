	...
	if(!isset($_SESSION["userLog"]) && !isset($_SESSION["userPas"])) {
		...
		if(mysql_num_rows($sql_id)>0) {
			...
			if($money_rb>=$money_pay) {
				...
				VALUES('$username','".DATE("d.m.Yг. H:i")."','$money_pay',  'Оплата рекламы [баннер $size_banner], ID:#$id_pay','Списано','rashod')") or die(mysql_error());
		...
		new AjaxUpload(btnUpload, {
			...
			onSubmit: function(file, ext){
				if (! (ext && /^(bmp|jpg|png|jpeg|gif)$/.test(ext))){ 
					...
					status.html('<span class="msg-error">Поддерживаемые форматы bmp, jpg, jpeg, png, gif</span>');
					return false;
				}
			...
			onComplete: function(file, response){
				...
				if(response==="success"){
				  document.getElementById('urlbanner').value="https://<?=$_SERVER["HTTP_HOST"];?>/uploads/"+file;
					...
					$('<li></li>').appendTo('#files').text('Файл не загружен!' + file).addClass('msg-error'); 
				}
			}
		});
...
echo '<tbody>';
	...
	echo '</tr>';
	echo '<tr>';
		echo '<td align="left"><b>URL баннера</b> (картинка на PC:)</td>';
		echo '<td align="left"><div style="position:relative;overflow:hidden;"><input type="text" id="urlbanner" name="urlbanner" maxlength="300" placeholder="http://website.ru/banner.gif" value="" class="ok" ><div class="open" style="position:absolute;top:0;right:0;width:86px;text-align:center;" id="open">Обзор</div><br/> <span align="center" id="status"></span></td>';
	echo '</tr>';
	echo '<tr>';
	...
	//echo '</tr>';
	//echo '<tr>';
			//echo '<td id=""><b>Стоимость показа в сутки</b></td>';
			//echo '<td id="price"></td>';
		//echo '</tr>';
	//	echo '<tr>';
