	...
	if(!isset($_SESSION["userLog"]) && !isset($_SESSION["userPas"])) {
		...
		if(mysql_num_rows($sql_id)>0) {
			...
			if($money_user>=$money_pay) {
				...
				mysql_query("INSERT INTO `tb_history` (`user`,`date`,`amount`,`method`,`status`,`tipo`) VALUES('$username','".DATE("d.m.Yг. H:i")."','$money_pay',  'Оплата рекламы: автосерфинг - $plan шт.','Списано','rashod')") or die(mysql_error());
...
echo '<table class="tables">';
			echo '<tr>';
				...
				echo '<td width="160" align="right"><input type="submit" value="Проверить ссылку" class="proc-btn" style="float:none;" /></td>';
			echo '</tr>';
		}
	echo '</table>';
	echo '</form>';
