...
if(isset($ads) && isset($id) && isset($op) && $ads=="psevdo" && $op=="edit" && $id>0) {
	if(!DEFINED("PSEVDO_EDIT")) DEFINE("PSEVDO_EDIT", true);
	include("psevdo_edit.php");
	include(DOC_ROOT."/footer.php");
	exit();
}

echo '<a name="goto"></a>';
echo '<h5 class="sp" style="margin-top:0px; font-size:14px; font-weight:bold; text-align:center;">Мои псевдо-динамические ссылки</h5>';
...
function pay_adv(id, type) {
	...
	if (plan == '') {
		...
		gebi("imfo-msg-addmoney"+id).style.display = 'block';
		setTimeout(function() {$("#imfo-msg-addmoney"+id).fadeOut('slow')}, 2000); clearTimeout();
	} else if (rplan < <?=$min_psevdo;?>) {
		gebi("imfo-msg-addmoney"+id).innerHTML = "<span class='msg-error'>Количество дней должно быть не менее <?=$min_psevdo;?></span>";
		gebi("imfo-msg-addmoney"+id).style.display = 'block';
		setTimeout(function() {$("#imfo-msg-addmoney"+id).fadeOut('slow')}, 2000); clearTimeout();
	} else {
...
$sql = mysql_query("SELECT * FROM `tb_ads_psevdo` WHERE `username`='$username' ORDER BY `id` ASC");
if(mysql_num_rows($sql)>0) {
	...
	while ($row = mysql_fetch_array($sql)) {
		...
		echo '<tr id="adv_dell'.$row["id"].'">';
			...
			echo '<td align="left" class="noborder2">';
				...
				echo '<span class="info-text">';
					echo '№:&nbsp;'.$row["id"].'&nbsp;&nbsp;Просмотров:&nbsp;<span id="c_stat'.$row["id"].'">'.($row["members"]+$row["outside"]).'</span>';
					if($row["status"]>"0") echo '&nbsp;&nbsp;Заказано до:&nbsp;'.DATE("d.m.Yг. H:i", $row["date_end"]);
				echo '</span>';
				echo '<span class="adv-dell" title="Удалить ссылку" onClick="alert_delete('.$row["id"].', \'psevdo\');"></span>';
				echo '<span class="adv-erase" title="Сброс статистики" onClick="clear_stat('.$row["id"].', \'psevdo\', '.($row["members"]+$row["outside"]).');"></span>';
		...
		echo '<tr id="adv_addmoney'.$row["id"].'" style="display: none">';
			...
			function obsch<?php echo $row["id"];?>(){
				...
				plan<?php echo $row["id"];?> = str_replace("-", "", plan<?php echo $row["id"];?>);
				var cena_link<?php echo $row["id"];?> = <?php echo ($cena_psevdo + $row["color"] * $cena_color_psevdo);?>;
			...
			echo '<td align="center" colspan="3" class="ext-text">';
				...
				echo 'Укажите количество дней, которые вы хотите внести в бюджет рекламной площадки<br>(Минимум '.count_text($min_psevdo, "дней", "день", "дня", "").')';
				...
				echo '<center><span onClick="pay_adv('.$row["id"].', \'psevdo\');" class="sub-green" style="float:none;" title="Пополнить бюджет площадки">Оплатить</span></center>';
	...
	echo '<span class="msg-w">У вас нет своих размещённых псевдо-динамических ссылок</span>';
}

echo '<div align="center"><span class="proc-btn" onClick="document.location.href=\'/advertise.php?ads=psevdo\'">Разместить ссылку</span></div>';
?>