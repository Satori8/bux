...
if(isset($_SESSION["userLog"]) && isset($_SESSION["userPas"])) {
	...
	if(isset($_GET["st"]) && isset($_GET["b"]) && intval($_GET["b"])==1) {
		...
		if($id>0) {
			$sql_id = mysql_query("SELECT * FROM `tb_ads_autoyoutube` WHERE `id`='$id' AND `status`='1' AND (`geo_targ`='' OR `geo_targ` LIKE '%$my_country%') AND `totals`>'0' AND `limits_now`>'0' AND `ytflag`='1'");
			if(mysql_num_rows($sql_id)>0) {
				$sql_v = mysql_query("SELECT * FROM `tb_ads_autoyoutube_visits` WHERE `username`='$username' AND `type`='claims' AND `ident`='$id'") or die(mysql_error());
				if(mysql_num_rows($sql_v)>0) {
					mysql_query("UPDATE `tb_ads_autoyoutube` SET `claims`=`claims`+1 WHERE `id`='$id' AND `status`='1'") or die(mysql_error());
					mysql_query("UPDATE `tb_ads_autoyoutube_visits` SET `date`='".time()."', `ip`='$laip', `money`='0.0000' WHERE `username`='$username' AND `type`='claims' AND `ident`='$id'") or die(mysql_error());
				}else{
					mysql_query("UPDATE `tb_ads_autoyoutube` SET `claims`=`claims`+1 WHERE `id`='$id' AND `status`='1' AND (`geo_targ`='' OR `geo_targ` LIKE '%$my_country%')") or die(mysql_error());
					mysql_query("INSERT INTO `tb_ads_autoyoutube_visits` (`username`, `type`, `ident`, `date`, `ip`, `money`) VALUES('$username', 'claims', '".$id."', '".time()."', '$laip', '0.0000')") or die(mysql_error());
				}
			}
		}
	}
	...
	if(isset($_GET["st"]) && !isset($_GET["b"])) {
		...
		if($id>0 && isset($_SESSION["endtmr_auto"]) && intval($_SESSION["endtmr_auto"]) <= time()) {
			...
			if(mysql_num_rows($sql_id)>0) {
				...
				if ($ytflag=='1') {
$pos = strrpos($url, "/");
if ($pos === false) { $visedurl="viewvyou.php?v=".$url; }
else { $visedurl="viewvyou.php?v=".substr($url,$pos); }
} else { $visedurl=$url; }
				...
				$sql_v = mysql_query("SELECT * FROM `tb_ads_autoyoutube_visits` WHERE `username`='$username' AND `type`='visit' AND `ident`='$id'") or die(mysql_error());
				if(mysql_num_rows($sql_v)>0) {
					...
					if($crok1 >= $crok2) {
						mysql_query("UPDATE `tb_ads_autoyoutube` SET `limits_now`=`limits_now`-'1', `totals`=`totals`-'1', `members`=`members`+'1', `outside`=`outside`+'1' WHERE `id`='$id' AND `status`='1' AND (`geo_targ`='' OR `geo_targ` LIKE '%$my_country%') AND `totals`>'0' AND `limits_now`>'0'") or die(mysql_error());
						mysql_query("UPDATE `tb_ads_autoyoutube_visits` SET `date`='".time()."', `ip`='$laip', `money`='$cena_click' WHERE `username`='$username' AND `type`='visit' AND `ident`='$id'") or die(mysql_error());
						...
						stats_users($username, strtolower(DATE("D")), 'autoyoutube_ser');
						### END СБОР СТАТИСТИКИ ######
					...
					mysql_query("UPDATE `tb_ads_autoyoutube` SET `limits_now`=`limits_now`-'1', `totals`=`totals`-'1', `members`=`members`+'1', `outside`=`outside`+'1' WHERE `id`='$id' AND `status`='1' AND (`geo_targ`='' OR `geo_targ` LIKE '%$my_country%') AND `totals`>'0' AND `limits_now`>'0'") or die(mysql_error());
					mysql_query("INSERT INTO `tb_ads_autoyoutube_visits` (`username`, `type`, `ident`, `date`, `ip`, `money`) VALUES('$username', 'visit', '".$id."', '".time()."', '$laip', '$cena_click')") or die(mysql_error());
					mysql_query("UPDATE `tb_users` SET `visits_a`=`visits_a`+'1', `reiting`=`reiting`+'$reit_as', `money`=`money`+'$cena_click', `money_a`=`money_a`+'$cena_click' WHERE `username`='$username'") or die(mysql_error());
	...
	$sql = mysql_query("SELECT `id`,`timer`,`url`,`description`,`ytflag` FROM `tb_ads_autoyoutube` WHERE `status`='1' AND (`geo_targ`='' OR `geo_targ` LIKE '%$my_country%') AND `totals`>'0' AND `limits_now`>'0' AND `id` NOT IN (SELECT `ident` FROM `tb_ads_autoyoutube_visits` WHERE `username`='$username' AND `date`>='".(time()-24*60*60)."') ORDER BY `id` DESC");
...
if ($pos === false) { $visedurl="/viewvyou.php?v=".$url; }
		...
		<th width="260" height="15px" align="center" bgcolor="#D1EEEE" style="border-bottom: 1 px solid; #94FFB8">Статистика</th>
		<th height="15px" align="center" bgcolor="#D1EEEE" style="border-bottom: 1 px solid; #94FFB8">Информация о видеоролике</th>
		<th width="220" height="15px" align="center" bgcolor="#D1EEEE" style="border-bottom: 1 px solid; #94FFB8">Действия</th>
		</tr>
		
		<tr>
		<td align="center">Ждите <span id="tmr" style="color: #FF0000;"></span> секунд <span id="begin" style="color: #ff0000;"></span></td>
			
			<td style="font-size: 80%; padding: 0 5px; 0 5px;">Стоимость просмотра: <b style="color: #000;"><?php echo $cena_click;?></b> руб.<br>Просмотрено видеороликов <?php echo DATE("d.m.Yг.");?>: <b style="color: #000;"><?php echo $kolvo_s;?></b></td>
			<td style="font-size: 80%; padding: 0 5px; 0 5px;"><b>Описание видеоролика:</b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $description;?><br><b>Ссылка на видеоролик:</b>&nbsp;&nbsp;<a href="<?php echo $row["url"];?>" target="_blank"><?php echo $url;?></a></td>
			<td style="font-size: 80%; padding: 0 5px; 0 5px;"><a href="<?php echo $url;?>" target="_blank">Открыть видеоролик в новом окне</a><br><a href="auto_s.php?st=<?php echo $id;?>&amp;b=1" onmouseover="window.status=''; return true" onmouseout="window.status=''; return false">Заблокировать видеоролик</a></td>
		</tr>
		
		</table>

		<script type="text/javascript" language="JavaScript">
			window.parent.document.title="<?php echo strtoupper($_SERVER["HTTP_HOST"]);?> | Автоматический серфинг YouTube | <?php echo $description;?>";
			window.parent.frames['site'].location.replace("<?php echo $visedurl;?>");
	...
	echo '<br><center><b style="color: #FF0000;">Ошибка! Для просмотра ссылок в авто-серфинге YouTube необходимо авторизоваться.</b></center>';
	echo '<script type="text/javascript" language="JavaScript">window.parent.frames["site"].location.replace("auto_no.php");</script>';
} 
?>
