...
if(count($_GET)>0) {
	...
	if($option=="edit_s") {
		...
		$sql_s = mysql_query("SELECT * FROM `tb_forum_s` WHERE `id_status`='$id_s'");
		if(mysql_num_rows($sql_s)>0) {
			...
			if(count($_POST)>0) {
				...
				mysql_query("UPDATE `tb_forum_s` SET 
					...
					`del_post`='$del_post', 
					`edit_post`='$edit_post' 
				WHERE `id_status`='$id_s'") or die(mysql_error());
...
$sql_s = mysql_query("SELECT * FROM `tb_forum_s` ORDER BY `id` ASC");
if(mysql_num_rows($sql_s)>0) {
