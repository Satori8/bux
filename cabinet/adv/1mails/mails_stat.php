...
if(mysql_num_rows($sql)>0) {
	...
	$perpage = 30;
	$sql_p = mysql_query("SELECT `id` FROM `tb_ads_mails_visits` WHERE `ident`='$id' AND `money`>'0'");
	...
	$sql = mysql_query("SELECT * FROM `tb_ads_mails_visits` WHERE `ident`='$id' AND `money`>'0' ORDER BY `time` DESC LIMIT $start_pos,$perpage");
	if(mysql_num_rows($sql)>0) {
