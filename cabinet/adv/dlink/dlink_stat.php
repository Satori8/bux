<?php
if(!DEFINED("DLINK_STAT")) {die ("Hacking attempt!");}

echo '<a name="goto"></a>';
echo '<h5 class="sp" style="margin-top:0px; font-size:14px; font-weight:bold; text-align:center;">Статистика просмотров серфинга № '.$id.' за последние 24 часа</h5>';
...
if(mysql_num_rows($sql)>0) {
	...
	$perpage = 30;
	$sql_p = mysql_query("SELECT `id` FROM `tb_ads_dlink_visits` WHERE `ident`='$id' AND `money`>'0'");
	...
	$sql = mysql_query("SELECT * FROM `tb_ads_dlink_visits` WHERE `ident`='$id' AND `money`>'0' ORDER BY `time_next` DESC LIMIT $start_pos,$perpage");
	if(mysql_num_rows($sql)>0) {
