...
$perpage = 20;
$sql_p = mysql_query("SELECT `id` FROM `tb_ads_dlink` WHERE `status`>'0' AND `type_serf`!='10' $WHERE_ADD");
...
if(isset($_POST["id"])) {
	...
	/*}elseif(($type_serf==2 | $type_serf==4) && is_url($url)!="true") {
		...
		$sql_totals = mysql_query("SELECT `plan`,`totals` FROM `tb_ads_dlink` WHERE `id`='$id' AND `type_serf`!='10'");
...
if(isset($_GET["id"])) {
	...
	if ($option=="edit"){
		$sql = mysql_query("SELECT * FROM `tb_ads_dlink` WHERE `id`='$id' AND `status`>'0' AND `type_serf`!='10'");
...
$sql_ads = mysql_query("SELECT * FROM `tb_ads_dlink` WHERE `status`>'0' AND `type_serf`!='10' $WHERE_ADD ORDER BY `id` DESC LIMIT $start_pos, $perpage") or die(mysql_error());
