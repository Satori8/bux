<?php
if(!DEFINED("LINK_AJAX")) {die ("Hacking attempt!");}
...
if($option == "delete") {
	...
	if(mysql_num_rows($sql)>0) {
		...
		if($status == 0) {
			mysql_query("DELETE FROM `tb_ads_link` WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
			cache_link();
	...
	if(mysql_num_rows($sql)>0) {
		...
		if(mysql_num_rows($sql_bl)>0 && $black_url!=false) {
			...
			if($status == 0) {
				...
				mysql_query("UPDATE `tb_ads_link` SET `color`='$color', `url`='$url', `description`='$description', `ip`='$laip'  WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
				cache_link();

				exit("OK");

			}elseif($status == 1) {
				mysql_query("UPDATE `tb_ads_link` SET `color`='$color_tab', `url`='$url', `description`='$description', `ip`='$laip'  WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
				cache_link();
	...
	if(mysql_num_rows($sql)>0) {
		...
		if($status == 0) {
			...
			mysql_query("UPDATE `tb_ads_link` SET `view`='0', `ip`='$laip'  WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
			cache_link();
	...
	if(mysql_num_rows($sql)>0) {
		...
		$cena = ($color * $cena_color_link + $cena_link) * (100-$cab_skidka)/100;
		...
		if($money_user_rb<$money_pay) {
			...
			if($status=="0" | $status=="3") {
				...
				mysql_query("UPDATE `tb_ads_link` SET `status`='1', `method_pay`='10', `date`='".time()."', `ip`='$laip'  WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
				mysql_query("INSERT INTO `tb_history` (`user`,`date`,`amount`,`method`,`status`,`tipo`) 
				VALUES('$username','".DATE("d.m.Yг. H:i")."','$money_pay','Оплата ссылки в блоке платная строка ID:$id','Списано','rashod')") or die(mysql_error());

				@stat_pay('link', $money_pay);
