...
$sql_id = mysql_query("SELECT * FROM `tb_ads_packet` WHERE `status`='1' AND `merch_tran_id`='$merch_tran_id'");
if(mysql_num_rows($sql_id)>0) {
	...
	if(mysql_num_rows($sql_p)>0) {
		...
		mysql_query("DELETE FROM `tb_ads_packet` WHERE `status`='1' AND `merch_tran_id`='$merch_tran_id'");
	}else{
		exit();
	}
}
