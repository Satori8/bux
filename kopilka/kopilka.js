...
function money_plus_us() {
	$.ajax({
		...
		success: function(data) { 
			$('#loading').hide();
			if(data=="OK") {
				ModalStart("Информация", StatusMsg("OK", "Вы успешно получили бонус!"), 500, true, false, false);
				setTimeout(function(){if($("div").is(".box-modal")) {$("#LoadModal").modalpopup("close"); window.location = '';}}, 1500);
			}else{
				ModalStart("Информация", StatusMsg("ERROR", data), 500, true, false, false);
...
function money_add_kopilka() {
	...
	if(moneyadd == '') {
		document.getElementById("info-msg-kop").innerHTML = "<span class='msg-error'>Не указана сумма</span>";
		document.getElementById('info-msg-kop').style.display = 'block';
		tm = setTimeout(function() {
			hidemsg()
		}, 1000);
	} else if(isNaN(moneyadd)) {
		document.getElementById("info-msg-kop").innerHTML = "<span class='msg-error'>Неверно указана сумма</span>";
		document.getElementById('info-msg-kop').style.display = 'block';
		tm = setTimeout(function() {
			hidemsg()
		}, 1000);
	} else if(moneyadd < 0.01) {
		document.getElementById("info-msg-kop").innerHTML = "<span class='msg-error'>Минимальная сумма 0.01 руб.</span>";
		document.getElementById('info-msg-kop').style.display = 'block';
		tm = setTimeout(function() {
			hidemsg()
		}, 1000);
	} else if( (methodadd != '1') && (methodadd != '2') ) {
		document.getElementById("info-msg-kop").innerHTML = "<span class='msg-error'>Не указан тип счета</span>";
		document.getElementById('info-msg-kop').style.display = 'block';
		tm = setTimeout(function() {
			hidemsg()
		}, 1000);
		...
		function setstate() {
			if((myReq.readyState == 4)&&(myReq.status == 200)) {
				...
				document.getElementById('info-msg-kop').innerHTML = "<span id='loading' title='Подождите пожалуйста...'></span>";
				document.getElementById('info-msg-kop').style.display = 'block';
			}
			return true;
		} 
...
function ViewMoneyIn() {
	$.ajax({
		...
		data: { 'op': 'ViewMoneyIn' }, 
		success: function(data) {
			ModalStart("Пополнили копилку", data, 700, true, false, false);
		}
	});
}
...
function InfoKop() {
	$.ajax({
		...
		data: { 'op': 'InfoKop' }, 
		success: function(data) {
			ModalStart("Копилка нашего проекта", data, 600, true, false, false);
		}
	});
}
