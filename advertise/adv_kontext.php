	...
	if(!isset($_SESSION["userLog"]) && !isset($_SESSION["userPas"])) {
		...
		if(mysql_num_rows($sql_id)>0) {
			...
			if($money_user>=$money_pay) {
				...
				mysql_query("INSERT INTO `tb_history` (`user`,`date`,`amount`,`method`,`status`,`tipo`) VALUES('$username','".DATE("d.m.Yг. H:i")."','$money_pay',  'Оплата рекламы: контекстная ссылка ($plan переходов)','Списано','rashod')") or die(mysql_error());
