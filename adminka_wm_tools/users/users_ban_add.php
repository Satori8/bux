...
if(count($_POST)>0) {
	...
	if($user_name_ban==false) {
		...
		$sql_id = mysql_query("SELECT `username`,`wmid` FROM `tb_users` WHERE `username`='$user_name_ban'");
		if(mysql_num_rows($sql_id)>0) {
			$row = mysql_fetch_array($sql_id);
			$user_name_ban = $row["username"];
			$user_wmid_ban = $row["wmid"];

			$sql_с = mysql_query("SELECT `id` FROM `tb_black_users` WHERE `name`='$user_name_ban'");
			if(mysql_num_rows($sql_с)>0) {
				...
				VALUES('$user_name_ban','$prichina','','".DATE("d.m.Y H:i")."','".time()."','$username')") or die(mysql_error());

				mysql_query("UPDATE `tb_users` SET `ban_date`='".time()."' WHERE `username`='$user_name_ban' AND `ban_date`='0'") or die(mysql_error());

				$sql_ch2 = mysql_query("SELECT `id` FROM `tb_black_wmid` WHERE `wmid`='$user_wmid_ban'") or die(mysql_error());
				if(mysql_num_rows($sql_ch2)<1) {
					mysql_query("INSERT INTO `tb_black_wmid` (`wmid`,`reason`,`date`,`ip`) 
					VALUES ('$user_wmid_ban','$prichina','".time()."','')") or die(mysql_error());
				}
