...
if(count($_POST)>0 && isset($_POST["id_pay"])) {
	...
	if(mysql_num_rows($sql_bl)>0 && $black_url!=false) {
		...
		if(mysql_num_rows($check_wmid)>0) {
			mysql_query("UPDATE `tb_ads_kontext` SET `wmid`='$wmid_user', `username`='$username', `date`='".time()."', `plan`='$plan', `totals`='$plan', `url`='$url', `title`='$title', `description`='$description', `color`='$color', `ip`='$laip', `money`='0' WHERE `status`='0' AND `session_ident`='".session_id()."'") or die(mysql_error());
		}else{
			...
			VALUES('0','".session_id()."','0','$method_pay','$wmid_user','$username','".time()."','$plan','$plan','$url','$title','$description','$color','$laip','0')") or die(mysql_error());
		}
