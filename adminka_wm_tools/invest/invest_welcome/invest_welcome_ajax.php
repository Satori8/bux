	...
	if(isset($_SESSION["userLog_a"]) && isset($_SESSION["userPas_a"])) {
		...
		if($option=="AddWelcome") {
			...
			if($title==false) {
				...
				$sql = mysql_query("SELECT `id` FROM `tb_invest_welcome`") or die(mysql_error());
				if(mysql_num_rows($sql)>0) {
					mysql_query("UPDATE `tb_invest_welcome` SET `title_text`='$title', `desc_text`='$description'") or die(mysql_error());

					exit("OK");
				}else{
					mysql_query("INSERT INTO `tb_invest_welcome` (`title_text`,`desc_text`) 
					VALUES('$title','$description')") or die(mysql_error());
			...
			mysql_query("TRUNCATE TABLE `tb_invest_welcome`") or die(mysql_error());
			exit("OK");
