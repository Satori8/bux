...
$perpage = 30;
$sql_p = mysql_query("SELECT `id` FROM `tb_users` WHERE `referer`='' AND `user_status`='0' AND `ban_date`='0'");
...
if(isset($_POST["option"]) && limpiar($_POST["option"])=="not_get_ref") {
	...
	mysql_query("UPDATE `tb_users` SET `not_get_ref`='1' WHERE `id`='$id' AND `referer`='' AND `user_status`='0' AND `not_get_ref`='0' AND `ban_date`='0'") or die(mysql_error());
}

if(isset($_POST["option"]) && limpiar($_POST["option"])=="get_ref") {
	$id = (isset($_POST["id"]) && preg_match("|^[\d]{1,11}$|", trim($_POST["id"]))) ? intval(limpiar(trim($_POST["id"]))) : false;

	mysql_query("UPDATE `tb_users` SET `not_get_ref`='0' WHERE `id`='$id' AND `referer`='' AND `user_status`='0' AND `not_get_ref`='1' AND `ban_date`='0'") or die(mysql_error());
}


$sql = mysql_query("SELECT * FROM `tb_users` WHERE `referer`='' AND `user_status`='0' AND `ban_date`='0' ORDER BY $s_tab $u_tab LIMIT $start_pos,$perpage");
