...
if(isset($_SESSION["userLog"]) && isset($_SESSION["userPas"])) {
	$sql_user = mysql_query("SELECT `wmid`,`wm_purse`,`money_rb`,`test_drive`,`reiting` FROM `tb_users` WHERE `username`='$username'");
	if(mysql_num_rows($sql_user)>0) {
		...
		if($test_drive==1) {
			...
			exit();
                ...
                echo '<span class="msg-error">Для использовали функцию TEST DRIVE у вас должен быть статус <b>Студент</b> или выше!</span>';
                        include('footer.php');
                        exit();
                }
...
if(count($_POST)>0 && isset($_POST["id_pay"])) {
	if(isset($_SESSION["userLog"]) && isset($_SESSION["userPas"])) {
		...
		if(mysql_num_rows($sql_id)>0) {
			...
			mysql_query("UPDATE `tb_users` SET `test_drive`='1' WHERE `username`='$username'") or die(mysql_error());
...
if(count($_POST)>0) {
	...
	echo '<table class="tables">';
		...
		echo '</tr>';
		echo '<tr>';
			echo '<td colspan="2" align="center"><input type="submit" value="Оформить заказ" class="proc-btn" style="float:none;" /></td>';
		echo '</tr>';
	echo '</tbody>';
	echo '</table>';
	echo '</form>';
