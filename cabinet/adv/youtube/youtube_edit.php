<?php
if(!DEFINED("YOUTUBE_EDIT")) {die ("Hacking attempt!");}

echo '<a name="goto"></a>';
echo '<h5 class="sp" style="margin-top:0px; font-size:14px; font-weight:bold; text-align:center;">Редактирование видеоролика в серфинге YouTube № '.$id.'</h5>';

$sql = mysql_query("SELECT * FROM `tb_ads_youtube` WHERE `id`='$id' AND ytflag='1' AND`username`='$username' AND `type_serf`!='10'");
if(mysql_num_rows($sql)>0) {
	...
	if($row["type_serf"]==1) {
		$cena_hits = $youtube_cena_hits;
	}elseif($row["type_serf"]==2) {
		$cena_hits = $youtube_cena_hits_bs;
		...
		/*} else if ((type_serf==1 | type_serf==3) && description == '') {
			...
			if (status == 0 | status == 3) {
				if (timer == "" | timer == 0) {
					...
					focus_bg("timer");
					return false;
				} else if (rtimer < timer_ot | rtimer > <?=$youtube_timer_do;?>) {
					gebi("info-msg-cab").innerHTML = '<span class="msg-error">Время просмотра должно быть от ' + timer_ot + ' до <?=$youtube_timer_do;?> сек.</span>';
					gebi("info-msg-cab").style.display = "";
					setTimeout(function() {$('#info-msg-cab').fadeOut('slow')}, 2000); clearTimeout();
					focus_bg("timer");
					return false;
	...
	function obsch() {
		...
		var cena_timer = <?=$youtube_cena_timer;?>;
		var cena_color = <?=$youtube_cena_color;?>;
		var cena_active = <?=$youtube_cena_active;?>;
		var cena_nac = <?=(100+$youtube_nacenka)/100;?>;
		...
		if(gebi('timer_serf')) {
			if(type_serf==3 | type_serf==4) {
				gebi('timer_serf').innerHTML = '(<?php echo "от 20 сек. до $youtube_timer_do сек.";?>)';
			} else {
				gebi('timer_serf').innerHTML = '(<?php echo "от $youtube_timer_ot сек. до $youtube_timer_do сек.";?>)';
			}
		}
		...
		if(revisit==1) {var cena_revisit = <?=$youtube_cena_revisit[1];?>;}
		if(revisit==2) {var cena_revisit = <?=$youtube_cena_revisit[2];?>;}
		if(unic_ip==0) {var cena_unic_ip = 0;}
		if(unic_ip==1) {var cena_unic_ip = <?php echo $youtube_cena_unic_ip[1];?>;}
		if(unic_ip==2) {var cena_unic_ip = <?php echo $youtube_cena_unic_ip[2];?>;}

		if(nolimit==1) {
			var price = <?=$youtube_cena_nolimit[1];?>;
		}else if(nolimit==2) {
			var price = <?=$youtube_cena_nolimit[2];?>;
		}else if(nolimit==3) {
			var price = <?=$youtube_cena_nolimit[3];?>;
		}else if(nolimit==4) {
			var price = <?=$youtube_cena_nolimit[4];?>;
		}else{
			nolimit = 0;
		}
		...
		if(nolimit>0) {
			...
			gebi('bl61').innerHTML = '<b>Каждые 24 часа</b>';
			gebi('bl7').style.display = 'none';
			gebi('bl71').innerHTML = '<b>Нет</b>';
			...
			var price = cena_nac * (cena_hits + (cena_timer * (timer-<?=$youtube_timer_ot;?>)) + (active * cena_active)) + ((color * cena_color) + cena_revisit + cena_unic_ip);
			...
			echo '<td>';
				...
				if($row["status"]==0 | $row["status"]==3) {
					echo '<select id="type_serf" onChange="obsch();" onClick="obsch();">';
						echo '<option value="1" '.("1"==$row["type_serf"] ? 'selected="selected"' : false).'>YouTube серфинг</option>';
						//echo '<option value="2" '.("2"==$row["type_serf"] ? 'selected="selected"' : false).'>Баннерный серфинг</option>';
						echo '<option value="3" '.("3"==$row["type_serf"] ? 'selected="selected"' : false).'>YouTube серфинг - VIP (вы сможете смотреть вип серфинг YouTube)</option>';
					...
					if($row["type_serf"]==1 | $row["type_serf"]==5) {
						echo '<b>YouTube серфинг</b>';
					}elseif($row["type_serf"]==2) {
						echo '<b>Баннерный серфинг</b>';
					}elseif($row["type_serf"]==3) {
						echo '<b>YouTube серфинг - VIP (вы сможете смотреть вип серфинг YouTube)</b>';
					}elseif($row["type_serf"]==4) {
			...
			echo '<td>';
				if($row["status"]==0 | $row["status"]==3) {
					echo '<select id="nolimit" onChange="obsch();" onClick="obsch();">';
						...
						echo '<option value="1">1&nbsp;неделя&nbsp;('.number_format($youtube_cena_nolimit[1],2,".","`").' руб.)</option>';
						echo '<option value="2">2&nbsp;недели&nbsp;('.number_format($youtube_cena_nolimit[2],2,".","`").' руб.)</option>';
						echo '<option value="3">3&nbsp;недели&nbsp;('.number_format($youtube_cena_nolimit[3],2,".","`").' руб.)</option>';
						echo '<option value="4">1&nbsp;месяц&nbsp;&nbsp;('.number_format($youtube_cena_nolimit[4],2,".","`").' руб.)</option>';
					echo '</select>';
				}else{
					if($row["nolimit"]!=0) {
	...
	echo '<div id="adv-block1" style="display:none; margin:0; padding:0;">';
				...
				echo '<td style="height:31px;"><div id="bl3">';
					if($row["status"]==0 | $row["status"]==3) {
						echo '<select id="color" onChange="obsch();" onClick="obsch();">';
							...
							echo '<option value="1" '.("1"==$row["color"] ? 'selected="selected"' : false).'>Да (+'.round(number_format($youtube_cena_color,6,".",""),6).' руб./визит)</option>';
						echo '</select>';
					}elseif($row["color"]==0) {
						echo '<b>НЕТ</b>';
				...
				echo '<td style="height:31px;"><div id="bl5">';
					if($row["status"]==0 | $row["status"]==3) {
						echo '<select id="active" onChange="obsch();" onClick="obsch();">';
							...
							echo '<option value="1" '.("1"==$row["active"] ? 'selected="selected"' : false).'>Да (+'.round(number_format(($youtube_cena_active*(100+$nacenka_hits_bs)/100),6,".",""),6).' руб./визит)</option>';
						echo '</select>';
					}elseif($row["active"]==0) {
						echo '<b>НЕТ</b>';
				...
				echo '<td><div id="bl6">';
					if($row["status"]==0 | $row["status"]==3) {
						echo '<select id="revisit" onChange="obsch();" onClick="obsch();">';
							...
							echo '<option value="1"'.("1"==$row["revisit"] ? 'selected="selected"' : false).'>Каждые 48 часов (+'.round(number_format($youtube_cena_revisit[1], 6, ".", ""),6).' руб./визит)</option>';
							echo '<option value="2"'.("2"==$row["revisit"] ? 'selected="selected"' : false).'>1 раз (+'.round(number_format($youtube_cena_revisit[2], 6, ".", ""),6).' руб./визит)</option>';
						echo '</select>';
				...
				echo '<td style="height:31px;"><div id="bl7">';
					if($row["status"]==0 | $row["status"]==3) {
						echo '<select id="unic_ip" onChange="obsch();" onClick="obsch();">';
							...
							echo '<option value="1" '.("".$row["unic_ip"]."" == 1 ? 'selected="selected"' : false).'>Да (100% совпадение) &mdash; (+'.round(number_format($youtube_cena_unic_ip[1], 6, ".", ""),6).' руб./визит)</option>';
							echo '<option value="2" '.("".$row["unic_ip"]."" == 2 ? 'selected="selected"' : false).'>Усиленный по маске до 2 чисел (255.255.X.X) &mdash; (+'.round(number_format($youtube_cena_unic_ip[1], 6, ".", ""),6).' руб./визит)</option>';
						echo '</select>';
		...
		echo '<tbody>';
			echo '<tr>';
				echo '<td width="220">Ограничение показов в сутки</td>';
				if($row["limit_d"]==$row["plan"]) $row["limit_d"] = 0;
				echo '<td>';
					if($row["status"]==0 | $row["status"]==3) {
						echo '<input type="text" id="limit_d" maxlength="7" value="'.$row["limit_d"].'" class="ok12" style="text-align:right;" onChange="obsch();" onKeyUp="obsch();" onkeydown="this.style.background=\'#FFFFFF\';">&nbsp;&nbsp;&nbsp;(0 - без ограничений)';
					}else{
						if($row["nolimit"]!=0) {
				...
				echo '</td>';
				echo '<td rowspan="2" align="center">(0 - без ограничений)</td>';
			echo '</tr>';

			echo '<tr>';
				echo '<td width="220">Ограничение показов в час</td>';
				if($row["limit_h"]==$row["plan"]) $row["limit_h"] = 0;
				echo '<td>';
					if($row["status"]==0 | $row["status"]==3) {
						echo '<input type="text" id="limit_h" maxlength="7" value="'.$row["limit_h"].'" class="ok12" style="text-align:right;" onChange="obsch();" onKeyUp="obsch();" onkeydown="this.style.background=\'#FFFFFF\';">&nbsp;&nbsp;&nbsp;(0 - без ограничений)';
					}else{
						if($row["nolimit"]!=0) {
	...
	if($row["status"]!=0) {
		echo '<div align="center"><span onClick="save_ads('.$row["id"].', \'youtube\', '.$row["status"].', 1);" class="proc-btn" style="float:none; width:160px;">Сохранить</span></div>';
	}else{
		echo '<div align="center"><span onClick="save_ads('.$row["id"].', \'youtube\', '.$row["status"].', 0);" class="proc-btn" style="float:none; width:160px;">Сохранить</span></div>';
	}
