...
<head>
	...
	<link type="text/css" rel="stylesheet" href="/style/style-serf.css?v=1.06">
...
if(isset($_SESSION["userLog"]) && isset($_SESSION["userPas"])) {
	...
	mysql_query("UPDATE `tb_ads_youtebe` SET `status`='3', `date`='".time()."' WHERE `status`>'0' AND `status`<'3' AND ( (`totals`<'1' AND `nolimit`='0') OR ( `nolimit`>'0' AND `nolimit`<='".time()."') ) ");
	...
	$token_now = strrev(md5(strtolower($user_name).$id_adv_get.$my_lastiplog)).strrev(md5(strtolower($_SERVER["HTTP_HOST"]).strtolower($_SERVER["HTTP_USER_AGENT"]).$my_lastiplog."### SPARTAK ###"));
	...
	if(mysql_num_rows($sql_user)>0) {
		...
		$sql_link = mysql_query("SELECT `id`,`status`,`active`,`timer`,`url`,`ytflag` FROM `tb_ads_youtube` WHERE `id`='$id_adv_get' AND `status`>'0' 
		...
		if(mysql_num_rows($sql_link)>0) {
			...
			$url = $row_link["url"];
			$ytflag = $row_link["ytflag"];
			
if ($ytflag=='1') {
$pos = strrpos($url, "/");
if ($pos === false) { $visedurl="viewv.php?v=".$url; }
else { $visedurl="viewv.php?v=".substr($url,$pos); }
} else { $visedurl=$url; }

			$_SESSION["id_adv_serf"] = $id_adv_get;
			$_SESSION["time_end_serf"] = (time()+$timer);
			...
			if($status==2) {
				...
				echo '</script>'."\r\n";
				echo '<script type="text/javascript" src="/js/js_surfing_verify_youtube.js?v='.mt_rand(11111,99999).'"></script>'."\r\n";
				...
				if($my_frm_position==1) {
					...
					echo '<iframe id="framesite" onLoad="OnLoadFrm();" width="100%" hspace="0" vspace="0" marginheight="0" marginwidth="0" frameborder="0" scrolling="auto" src="'.$visedurl.'"></iframe>'."\r\n";
				}
				...
				echo '</div>';
				if($my_frm_position!=1) {
					echo "\r\n".'<iframe id="framesite" onLoad="OnLoadFrm();" width="100%" hspace="0" vspace="0" marginheight="0" marginwidth="0" frameborder="0" scrolling="auto" src="'.$visedurl.'"></iframe>';
