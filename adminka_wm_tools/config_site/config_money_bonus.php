...
if(count($_POST)>0) {
	...
	$bon_pay_status = isset($_POST["bon_pay_status"]) ? $_POST["bon_pay_status"] : 0;
		...
		mysql_query("UPDATE `tb_config` SET `price`='$bon_pay_summa' WHERE `item`='bon_pay_summa'") or die(mysql_error());
		
		mysql_query("UPDATE `tb_config` SET `price`='$bon_pay_reiting' WHERE `item`='bon_pay_reiting'") or die(mysql_error());
		
		mysql_query("UPDATE `tb_config` SET `price`='$bon_pay_money' WHERE `item`='bon_pay_money'") or die(mysql_error());
		
		mysql_query("UPDATE `tb_config` SET `price`='$bon_pay_status' WHERE `item`='bon_pay_status'") or die(mysql_error());
...
$sql = mysql_query("SELECT `price` FROM `tb_config` WHERE `item`='bon_pay_status'");
$bon_pay_status = (mysql_num_rows($sql)>0 && mysql_result($sql,0,0)>0 ) ? mysql_result($sql,0,0) : 0;

$sql = mysql_query("SELECT `price` FROM `tb_config` WHERE `item`='bon_pay_summa'");
$bon_pay_summa = (mysql_num_rows($sql)>0 && mysql_result($sql,0,0)>0 ) ? mysql_result($sql,0,0) : 0;

$sql = mysql_query("SELECT `price` FROM `tb_config` WHERE `item`='bon_pay_money'");
$bon_pay_money = (mysql_num_rows($sql)>0 && mysql_result($sql,0,0)>0 ) ? mysql_result($sql,0,0) : 0;

$sql = mysql_query("SELECT `price` FROM `tb_config` WHERE `item`='bon_pay_reiting'");
$bon_pay_reiting = (mysql_num_rows($sql)>0 && mysql_result($sql,0,0)>0 ) ? mysql_result($sql,0,0) : 0;
