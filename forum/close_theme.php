...
if(isset($_SESSION["userLog"]) && isset($_SESSION["userPas"])) {
	...
	if(mysql_num_rows($sql)>0) {
		...
		$sql_s = mysql_query("SELECT `id` FROM `tb_forum_s` WHERE `id_status`='$user_f_status' AND `close_theme`='1'");
		if(mysql_num_rows($sql_s)>0) {

			mysql_query("UPDATE `tb_forum_t` SET `status`='1' WHERE `id`='$id_t'") or die(mysql_error());
