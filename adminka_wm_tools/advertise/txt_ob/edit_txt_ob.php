...
$perpage = 25;
$sql_p = mysql_query("SELECT `id` FROM `tb_ads_txt` WHERE `status`>'0' $WHERE_ADD");
	...
	if($option=="edit") {
		...
		if(count($_POST)>0) {
			...
			if(mysql_num_rows($sql_bl)>0 && $black_url!=false) {
				...
				mysql_query("UPDATE `tb_ads_txt` SET `wmid`='$wmid', `username`='$username', `date_end`=`date`+'".($plan*24*60*60)."', `plan`='$plan', `url`='$url', `description`='$description' WHERE `id`='$id'") or die(mysql_error());
		...
		if(!isset($save)) {
			...
			$sql = mysql_query("SELECT * FROM `tb_ads_txt` WHERE `id`='$id' ORDER BY `id` ASC");
...
$sql_ads = mysql_query("SELECT * FROM `tb_ads_txt` WHERE `status`>'0' $WHERE_ADD ORDER BY `id` DESC LIMIT $start_pos, $perpage") or die(mysql_error());
