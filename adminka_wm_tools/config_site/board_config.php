...
if(count($_POST)>0) {
	...
	mysql_query("UPDATE `tb_config` SET `price`='$cena_board_add' WHERE `item`='cena_board_add' AND `howmany`='1'") or die(mysql_error());
	mysql_query("UPDATE `tb_config` SET `price`='$cena_board_comm' WHERE `item`='cena_board_comm' AND `howmany`='1'") or die(mysql_error());
	mysql_query("UPDATE `tb_config` SET `price`='$cena_board_reit' WHERE `item`='cena_board_reit' AND `howmany`='1'") or die(mysql_error());
	mysql_query("UPDATE `tb_config` SET `price`='$cena_board_comis' WHERE `item`='cena_board_comis' AND `howmany`='1'") or die(mysql_error());
...
$cena_board_add = number_format(mysql_result($sql,0,0), 0, ".", "");

$sql = mysql_query("SELECT `price` FROM `tb_config` WHERE `item`='cena_board_comm'");
$cena_board_comm = round(number_format(mysql_result($sql,0,0), 2, ".", ""),2);
