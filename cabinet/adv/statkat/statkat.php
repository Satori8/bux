...
if(isset($ads) && isset($id) && isset($op) && $ads=="statkat" && $op=="edit" && $id>0) {
	if(!DEFINED("STATKAT_EDIT")) DEFINE("STATKAT_EDIT", true);
	include("statkat_edit.php");
	include(DOC_ROOT."/footer.php");
	exit();
}

echo '<a name="goto"></a>';
echo '<h5 class="sp" style="margin-top:0px; font-size:14px; font-weight:bold; text-align:center;">Мои статические ссылки</h5>';
...
function pay_adv(id, type) {
	...
	if (plan == '') {
		...
		gebi("imfo-msg-addmoney"+id).style.display = 'block';
		setTimeout(function() {$("#imfo-msg-addmoney"+id).fadeOut('slow')}, 2000); clearTimeout();
	} else if (rplan < <?=$kat_min;?>) {
		gebi("imfo-msg-addmoney"+id).innerHTML = "<span class='msg-error'>Количество дней должно быть не менее <?=$kat_min;?></span>";
		gebi("imfo-msg-addmoney"+id).style.display = 'block';
		setTimeout(function() {$("#imfo-msg-addmoney"+id).fadeOut('slow')}, 2000); clearTimeout();
	} else {
...
$sql = mysql_query("SELECT * FROM `tb_ads_kat` WHERE `username`='$username' ORDER BY `id` ASC");
if(mysql_num_rows($sql)>0) {
	...
	while ($row = mysql_fetch_array($sql)) {
		...
		echo '<tr id="adv_dell'.$row["id"].'">';
			...
			echo '<td align="left" class="noborder2">';
				...
				echo '</span>';
				echo '<span class="adv-dell" title="Удалить ссылку" onClick="alert_delete('.$row["id"].', \'statkat\');"></span>';
			...
			echo '<td align="center" width="60" nowrap="nowrap">';
				if($row["status"]=="0") {
					...
					//echo '<a class="add-money-no" title="Пополнить рекламный бюджет" onClick="show_money_add(\'adv_addmoney'.$row["id"].'\');">'.count_text(ceil(($row["date_end"]-time())/86400), "дней", "день", "дня", "Пополнить").'</a>';
					...
					//echo '<a class="add-money" title="Пополнить рекламный бюджет" onClick="show_money_add(\'adv_addmoney'.$row["id"].'\');">'.count_text(ceil(($row["date_end"]-time())/86400), "дней", "день", "дня", "Пополнить").'</a>';
		...
		echo '<tr id="adv_addmoney'.$row["id"].'" style="display: none">';
			...
			function obsch<?php echo $row["id"];?>(){
				...
				plan<?php echo $row["id"];?> = str_replace("-", "", plan<?php echo $row["id"];?>);
				var cena_kat<?php echo $row["id"];?> = <?php echo ($cena_kat + $row["color"] * $cena_kat_color);?>;

				gebi('price<?php echo $row["id"];?>_1').innerHTML = number_format((plan<?php echo $row["id"];?> * cena_kat<?php echo $row["id"];?>), 2, '.', ' ') + ' руб.';
				gebi('price<?php echo $row["id"];?>_2').innerHTML = number_format((plan<?php echo $row["id"];?> * cena_kat<?php echo $row["id"];?> * <?php echo ((100-$cab_skidka)/100);?>), 2, '.', ' ') + ' руб.';
			}
			</script><?php

			echo '<td align="center" colspan="3" class="ext-text">';

				echo 'Укажите количество дней, которые вы хотите внести в бюджет рекламной площадки<br>(Минимум '.count_text($kat_min, "дней", "день", "дня", "").')';
				...
				echo '<center><span onClick="pay_adv('.$row["id"].', \'statkat\');" class="sub-green" style="float:none;" title="Пополнить бюджет площадки">Оплатить</span></center>';
	...
	echo '<span class="msg-w">У вас нет своих размещённых статических ссылок</span>';
}

echo '<div align="center"><span class="proc-btn" onClick="document.location.href=\'/advertise.php?ads=kat_links\'">Разместить ссылку</span></div>';
?>