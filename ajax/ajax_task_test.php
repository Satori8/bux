...
if( isset($_SERVER["HTTP_X_REQUESTED_WITH"]) && $_SERVER["HTTP_X_REQUESTED_WITH"] == "XMLHttpRequest" ) {
    ...
    if( isset($_SESSION["userLog"]) && isset($_SESSION["userPas"]) ) {
        ...
        if($sql_user->num_rows>0)  {
            ...
            if($type_ads == "task") {
                $mysqli->query("UPDATE `tb_ads_task` SET `status`='pause' WHERE `status` NOT IN ('wait','block') AND `totals`<='0'") or die( my_json_encode($ajax_json, "ERROR", $mysqli->error) );
                if($option == "get_url") {
                    $sql = $mysqli->query("SELECT `id` FROM `tb_ads_task` WHERE `id`='" . $id . "' AND `username`='" . $user_name . "'") or die( my_json_encode($ajax_json, "ERROR", $mysqli->error) );
                    if($sql->num_rows>0) {
                        $sql->free();
                        $result_text = "OK";
                ...
                if( $option == "PlayPause" ) {
                    $sql = $mysqli->query("SELECT `id`,`status`,`plan`,`totals`,`wait`,`zdurl` FROM `tb_ads_task` WHERE `id`='" . $id . "' AND `username`='" . $user_name . "'") or die( my_json_encode($ajax_json, "ERROR", $mysqli->error) );
                    if($sql->num_rows>0) {
                        $row = $sql->fetch_assoc();
                        $id = $row["id"];
                        ...
                        if($adv_status == "pause" && $cnt_plan>0 && $cnt_totals>0) {
                            $mysqli->query("UPDATE `tb_ads_task` SET `status`='pay', `ip`='" . $my_lastiplog . "' WHERE `id`='" . $id . "'") or die( my_json_encode($ajax_json, "ERROR", $mysqli->error) );
                            $result_text = "OK";
                            $message_text = false;
                        ...
                        if( $adv_status == "pay" && $cnt_wait <= 0 ) {
                            $mysqli->query("UPDATE `tb_ads_task` SET `status`='pause', `ip`='" . $my_lastiplog . "' WHERE `id`='" . $id . "'") or die( my_json_encode($ajax_json, "ERROR", $mysqli->error) );
                            $result_text = "OK";
                            $message_text = false;
                            $status_text = "<span class=\"adv-play\" title=\"Запустить задание\" onClick=\"FuncAdv(" . $id . ", 'task', 'PlayPause', '" . $token_check . "');\"></span>";
                            exit( my_json_encode($ajax_json, $result_text, $message_text, $status_text) );
                ...
                if( $option == "confirm_del" ) {
                    $sql = $mysqli->query("SELECT `id`,`status`,`zdprice`,`totals` FROM `tb_ads_task` WHERE `id`='" . $id . "' AND `username`='" . $user_name . "'") or die( my_json_encode($ajax_json, "ERROR", $mysqli->error) );
                    if($sql->num_rows>0) {
                        $row = $sql->fetch_assoc();
                        $id = $row["id"];
                        $adv_status = $row["status"];
                ...
                if( $option == "yes_del" ) {
                    $sql = $mysqli->query("SELECT `id`,`status`,`zdprice`,`totals`,`wait`,`cnt_start`,`cnt_claims`,`date_act` FROM `tb_ads_task` WHERE `id`='" . $id . "' AND `username`='" . $user_name . "'") or die( my_json_encode($ajax_json, "ERROR", $mysqli->error) );
                    if($sql->num_rows ) {
                        $row = $sql->fetch_assoc();
                        $id = $row["id"];
                        $adv_status = $row["status"];
                        ...
                        if($cnt_start>0 | $cnt_wait>0 | ($cnt_claims>0 && time() - 5 * 24 * 60 * 60 < $adv_date_act) ) {
                            $result_text = "ERROR";
                            if($cnt_start>0) {
                                ...
                                if($cnt_wait>0) {
                                    ...
                                    if($cnt_claims>0 && time() - 5 * 24 * 60 * 60 < $adv_date_act ) {
                        ...
                        if( $adv_status == "wait" | ($cnt_totals == 0 && $cnt_start == 0 && $cnt_wait == 0 && $cnt_claims == 0) )  {
                            $mysqli->query("DELETE FROM `tb_ads_task` WHERE `id`='" . $id . "'") or die( my_json_encode($ajax_json, "ERROR", $mysqli->error) );
                            $result_text = "OK";
                        ...
                        if($cnt_totals>0) {
                            ...
                            if( 1 <= $money_back ) {
                                ...
                                $mysqli->query("INSERT INTO `tb_history` (`status_pay`,`user`,`user_id`,`date`,`time`,`amount`,`method`,`status`,`tipo`) VALUES('1','" . $user_name . "','" . $user_id . "','" . DATE("d.m.Y H:i") . "','" . time() . "','" . $money_back . "','Возврат средств от удаления задания №" . $id . "','Списано','rashod')") or exit( my_json_encode($ajax_json, "ERROR", $mysqli->error) );
                ...
                if( $option == "form_balance" ) {
                    $sql = $mysqli->query("SELECT `id`,`status`,`zdprice` FROM `tb_ads_task` WHERE `id`='" . $id . "' AND `username`='" . $user_name . "'") or die( my_json_encode($ajax_json, "ERROR", $mysqli->error) );
                    if( 0 < $sql->num_rows ) {
                        $row = $sql->fetch_assoc();
                        $id = $row["id"];
                        $adv_status = $row["status"];
                ...
                if( $option == "pay_balance" ) {
                    $sql = $mysqli->query("SELECT `id`,`status`,`zdprice`,`zdurl`,`totals`,`bads`,`goods`,`wait`,`cnt_start` FROM `tb_ads_task` WHERE `id`='" . $id . "' AND `username`='" . $user_name . "'") or die( my_json_encode($ajax_json, "ERROR", $mysqli->error) );
                    if($sql->num_rows>0) {
                        $row = $sql->fetch_assoc();
                        $id = $row["id"];
                        $adv_status = $row["status"];
                        ...
                        $mysqli->query("UPDATE `tb_ads_task` SET `status`='pay', `plan`=`plan`+'" . $plan_add . "', `totals`=`totals`+'" . $plan_add . "', `ip`='" . $my_lastiplog . "' WHERE `id`='" . $id . "' AND `username`='" . $user_name . "'") or die( my_json_encode($ajax_json, "ERROR", $mysqli->error) );
                        ...
                        $mysqli->query("INSERT INTO `tb_history` (`status_pay`,`user`,`user_id`,`date`,`time`,`amount`,`method`,`status`,`tipo`) VALUES('1','" . $user_name . "','" . $user_id . "','" . DATE("d.m.Y H:i") . "','" . time() . "','" . $money_pay . "','Пополнение баланса задания №" . $id . " на " . count_text($plan_add, "выполнение", "выполнения", "выполнений") . "','Списано','rashod')") or die( my_json_encode($ajax_json, "ERROR", $mysqli->error) );
                        ...
                        $sql_pos->free();
                        if( isset($position_up) && $position_up>0 && $position_up < 100 ) {
                            $pos_class = "adv-down";
                ...
                if( $option == "form_up" ) {
                    ...
                    if($sql->num_rows>0) {
                        ...
                        if($adv_status == "pay" && $cnt_autoup>0 && $time_next>0) {
                    ...
                    if( $option == "pay_aup" ) {
                        ...
                        if($sql->num_rows>0) {
                            ...
                            $task_cena_up = sqltask("task_cena_up", 2);
                            $task_sale_up = sqltask("task_sale_up", 0);
                            if( 100 <= $cnt_autoup && $task_sale_up>0) {
                                $task_cena_up = ($task_cena_up * (100 - $task_sale_up)) / 100;
                            }
                    ...
                    if( $option == "pay_vip" ) {
                        $sql = $mysqli->query("SELECT `id`,`status`,`vip`,`zdprice`,`zdurl`,`totals` FROM `tb_ads_task` WHERE `id`='" . $id . "' AND `username`='" . $user_name . "'") or die( my_json_encode($ajax_json, "ERROR", $mysqli->error) );
                        if($sql->num_rows>0) {
                            $row = $sql->fetch_assoc();
                            $id = $row["id"];
                            $adv_status = $row["status"];
                            ...
                            if( $adv_status == "pay" && 0 < $adv_date_vip && 0 < $cnt_totals ) {
                                ...
                                $position_vip = ($sql_pos->num_rows>0 ? $sql_pos->fetch_object()->position + 1 : false);
                                $sql_pos->free();
                            }else{
                                unset($position_vip);
                            }
                            ...
                            $mysqli->query("UPDATE `tb_ads_task` SET `vip`='" . time() . "', `ip`='" . $my_lastiplog . "' WHERE `id`='" . $id . "' AND `username`='" . $user_name . "'") or die( my_json_encode($ajax_json, "ERROR", $mysqli->error) );
                            $mysqli->query("UPDATE `tb_users` SET `money_rb`=`money_rb`-'" . $task_cena_vip . "', `money_rek`=`money_rek`+'" . $task_cena_vip . "' WHERE `username`='" . $user_name . "'") or die( my_json_encode($ajax_json, "ERROR", $mysqli->error) );
                            $mysqli->query("INSERT INTO `tb_history` (`status_pay`,`user`,`user_id`,`date`,`time`,`amount`,`method`,`status`,`tipo`) \tVALUES('1','" . $user_name . "','" . $user_id . "','" . DATE("d.m.Y H:i") . "','" . time() . "','" . $task_cena_vip . "','Поднятие задания №" . $id . " в VIP блок','Списано','rashod')") or die( my_json_encode($ajax_json, "ERROR", $mysqli->error) );
