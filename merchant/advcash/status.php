		...
		if(mysql_num_rows($sql)) {
			...
			/////////////////////	
				if($bon_pay_status==1){
					...
					VALUES('$username', '$merch_amount1', '".DATE("d.m.Yг. H:i")."', 'Пополнение рекламного баланса через AdvCash','Зачислено', 'popoln')") or die(mysql_error());
		...
		/////////////////////////////////////////////////////
			...
			/////////////////////	
				if($bon_inv_status==1){
					...
					mysql_query("INSERT INTO `tb_history` (`user`, `amount`, `date`, `method`, `status`, `tipo`) 
					VALUES('$username', '$merch_amount', '".DATE("d.m.Yг. H:i")."', 'Пополнение баланса инвестора через AdvCash','Зачислено', 'popoln')") or die(mysql_error());
