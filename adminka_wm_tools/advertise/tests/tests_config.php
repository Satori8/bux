...
if(count($_POST)>0) {
	...
	if($tests_cena_hit==0) {
		...
		mysql_query("UPDATE `tb_config` SET `price`='$tests_cena_hit' WHERE `item`='tests_cena_hit' AND `howmany`='1'") or die(mysql_error());
		mysql_query("UPDATE `tb_config` SET `price`='$tests_comis_del' WHERE `item`='tests_comis_del' AND `howmany`='1'") or die(mysql_error());
		mysql_query("UPDATE `tb_config` SET `price`='$tests_nacenka' WHERE `item`='tests_nacenka' AND `howmany`='1'") or die(mysql_error());
		mysql_query("UPDATE `tb_config` SET `price`='$tests_min_pay' WHERE `item`='tests_min_pay' AND `howmany`='1'") or die(mysql_error());
		mysql_query("UPDATE `tb_config` SET `price`='$tests_cena_quest' WHERE `item`='tests_cena_quest' AND `howmany`='1'") or die(mysql_error());
		mysql_query("UPDATE `tb_config` SET `price`='$tests_cena_color' WHERE `item`='tests_cena_color' AND `howmany`='1'") or die(mysql_error());

		for($i=1; $i<=4; $i++) {
			mysql_query("UPDATE `tb_config` SET `price`='$tests_cena_revisit[$i]' WHERE `item`='tests_cena_revisit' AND `howmany`='$i'") or die(mysql_error());
		}

		for($i=1; $i<=2; $i++) {
			mysql_query("UPDATE `tb_config` SET `price`='$tests_cena_unic_ip[$i]' WHERE `item`='tests_cena_unic_ip' AND `howmany`='$i'") or die(mysql_error());
		}

		$sql_test = mysql_query("SELECT `id`,`questions`,`revisit`,`color`,`unic_ip_user` FROM `tb_ads_tests` ORDER BY `id` DESC");
		if(mysql_num_rows($sql_test)>0) {
			while ($row = mysql_fetch_assoc($sql_test)) {
				$id = $row["id"];
				...
				mysql_query("UPDATE `tb_ads_tests` SET `cena_user`='$cena_user',`cena_advs`='$cena_advs' WHERE `id`='$id'") or die(mysql_error());
			}
		}
