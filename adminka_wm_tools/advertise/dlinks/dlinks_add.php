...
if(count($_POST)>0) {
	...
	/*}elseif(($type_serf==2 | $type_serf==4) && is_url($url)!="true") {
		...
		mysql_query("INSERT INTO `tb_ads_dlink` (`status`,`session_ident`,`merch_tran_id`,`method_pay`,`type_serf`,`date`,`wmid`,`username`,`geo_targ`,`content`,`active`,`revisit`,`color`,`timer`,`nolimit`,`limit_d`,`limit_h`,`limit_d_now`,`limit_h_now`,`new_users`,`unic_ip`,`no_ref`,`sex_adv`,`to_ref`,`url`,`title`,`description`,`urlbanner_load`,`plan`,`totals`,`ip`,`money`) 
		VALUES('1','".session_id()."','0','$method_pay','$type_serf','".time()."','$wmid_user','$username','$country','$content','$active','$revisit','$color','$timer','$nolimitdate','$limit_d','$limit_h','$limit_d','$limit_h','$new_users','$unic_ip','$no_ref','$sex_adv','$to_ref','$url','$title','$description','$urlbanner_load','$plan','$plan','$laip','0')") or die(mysql_error());

		if($type_serf==3) {
			mysql_query("UPDATE `tb_users` SET `vip_serf`=`vip_serf`+'200' WHERE `username`='$username'") or die(mysql_error());
