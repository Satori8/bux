...
if(count($_POST)>0) {
	...
	mysql_query("UPDATE `tb_config` SET `price`='$cena_hide_free_users' WHERE `item`='cena_hide_free_users' AND `howmany`='1'") or die(mysql_error());
...
$sql = mysql_query("SELECT `price` FROM `tb_config` WHERE `item`='cena_hide_free_users'");
