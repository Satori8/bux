	...
	if(!isset($_SESSION["userLog"]) && !isset($_SESSION["userPas"])) {
		...
		if(mysql_num_rows($sql_id)>0) {
			...
			if($money_user>=$money_pay) {
				...
				VALUES('$username','".DATE("d.m.Yг. H:i")."','$money_pay',  'Оплата рекламы [авто-серфинг], ID:#$id_pay','Списано','rashod')") or die(mysql_error());
	...
	function SbmFormB() {
		...
		for (var i=0;i<arrayElem.length;i++){
			...
			if ((document.forms["formzakaz"].plan.value == '')|(document.forms["formzakaz"].plan.value < <?=$min_hits_aserf;?> )) {
				alert('Минимальное количество визитов <?=$min_hits_aserf;?>');
	...
	echo '<table class="tables">';
		...
		if(isset($_SESSION["checkurl"]) | isset($_SESSION["checkurl_ok"])) {
			...
			echo '<tr>';
				...
				echo '<td><input type="text" name="plan" id="plan" maxlength="7" value="" class="ok12" style="text-align:right;" onChange="obsch();" onKeyUp="obsch();" onkeydown="this.style.background=\'#FFFFFF\';">&nbsp;&nbsp;&nbsp;(минимум - '.$min_hits_aserf.')</td>';
			echo '</tr>';
			echo '<tr>';
				echo '<td><b>Время просмотра:</b></td>';
				echo '<td><input type="text" name="timer" id="timer" maxlength="3" value="'.$timer_aserf_ot.'" class="ok12" style="text-align:right;" onChange="obsch();" onKeyUp="obsch();" onkeydown="this.style.background=\'#FFFFFF\';">&nbsp;&nbsp;&nbsp;(от '.$timer_aserf_ot.' до '.$timer_aserf_do.' сек.)</td>';
			echo '</tr>';
			echo '<tr>';
            ...
            echo '</div></td></tr>';
			echo '<tr>';
				echo '<td align="left"><b>Выберите способ оплаты:</b></td>';
				echo '<td align="left">';
