...
if($method_pay==1) {
	...
	echo '<table class="tables"><tr><td align="center" colspan="2" '.($shp_item==21 ? 'style="border:none; padding:0; margin:0;"' : false).'>';
		echo '<form id="newform" name="payment" method="post" action="https://sci.interkassa.com/" enctype="utf-8">';
			...
			echo '<input type="submit" name="process" value="Оплатить" class="proc-btn" style="float:none;">';
		echo '</form>';
	echo '</td></tr></table>';
	...
	echo '<table class="tables"><tr><td align="center" colspan="2" '.($shp_item==21 ? 'style="border:none; padding:0; margin:0;"' : false).'>';
		echo '<form id="newform" method="POST" action="https://payeer.com/api/merchant/m.php">';
			...
			echo '<input type="submit" name="m_process" value="Оплатить" class="proc-btn" style="float:none;">';
		echo '</form>';
	echo '</td></tr></table>';
	...
	echo '<table class="tables"><tr><td align="center" colspan="2" '.($shp_item==21 ? 'style="border:none; padding:0; margin:0;"' : false).'>';
		echo '<form id="newform" name="payform" method="post" action="https://money.yandex.ru/quickpay/confirm.xml">';
			...
			echo '<input type="hidden" name="formcomment" value="'.$inv_desc.'" />';
			echo '<input type="hidden" name="short-dest" value="'.$inv_desc.'" />';
	...
	require_once(ROOT_DIR."/merchant/megakassa/mk_config.php");
	$mk_client_email = "";
	$mk_order_id = "$shp_item:$merch_tran_id";
	$mk_desc = $inv_desc_utf8;
	$mk_signature = md5($mk_secret_key.md5(join(':', array($mk_shop_id, floatval($money_add), $mk_currency, $mk_desc, $mk_order_id, $mk_method_id, $mk_client_email, $mk_debug, $mk_secret_key))));

	echo '<table class="tables"><tr><td align="center" colspan="2" '.($shp_item==21 ? 'style="border:none; padding:0; margin:0;"' : false).'>';
		echo '<form id="newform" method="POST" action="https://megakassa.ru/merchant/">';
			echo '<input type="hidden" name="shop_id" 	value="'.$mk_shop_id.'">';
			echo '<input type="hidden" name="amount" 	value="'.floatval($money_add).'">';
			echo '<input type="hidden" name="currency" 	value="'.$mk_currency.'">';
			echo '<input type="hidden" name="description" 	value="'.$mk_desc.'">';
			echo '<input type="hidden" name="order_id" 	value="'.$mk_order_id.'">';
			echo '<input type="hidden" name="method_id" 	value="'.$mk_method_id.'">';
			echo '<input type="hidden" name="client_email" 	value="'.$mk_client_email.'">';
			echo '<input type="hidden" name="debug" 	value="'.$mk_debug.'">';
			echo '<input type="hidden" name="language" 	value="'.$mk_culture.'">';
			echo '<input type="hidden" name="signature" 	value="'.$mk_signature.'">';
			echo '<input type="submit" class="proc-btn" 	value="Оплатить" style="float:none;">';
		echo '</form>';
	echo '</td></tr></table>';
	...
	echo '<table class="tables"><tr><td align="center" colspan="2" '.($shp_item==21 ? 'style="border:none; padding:0; margin:0;"' : false).'>';
		echo '<form id="newform" method="GET" accept-charset="windows-1251" action="http://www.free-kassa.ru/merchant/cash.php">';
			...
			echo '<input type="submit" name="pay" 	value="Оплатить" class="proc-btn" style="float:none;">';
		echo '</form>';
	echo '</td></tr></table>';
