...
$sql = mysql_query("SELECT `price` FROM `tb_config` WHERE `item`='cena_sent_emails' AND `howmany`='1'");
...
$sql_all_sent = mysql_query("SELECT `id` FROM `tb_users` WHERE `email_ok`='1' AND `email_sent`='1'");
$all_sent_users = number_format(mysql_num_rows($sql_all_sent), 0, ".", "`");
...
echo '<div align="center"><span id="Save" onClick="SaveAds(0, \'sent_emails\');" class="proc-btn" style="float:none; width:160px;">Добавить рассылку</span></div>';
