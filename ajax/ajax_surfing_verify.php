...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) && $_SERVER["HTTP_X_REQUESTED_WITH"]=="XMLHttpRequest") {
	if(isset($_SESSION["userLog"]) && isset($_SESSION["userPas"])) {
		...
		mysql_query("UPDATE `tb_ads_dlink` SET `status`='3', `date`='".time()."' WHERE `status`>'0' AND `status`<'3' AND ( (`totals`<'1' AND `nolimit`='0') OR ( `nolimit`>'0' AND `nolimit`<='".time()."') ) ") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
		...
		if(mysql_num_rows($sql_user)>0) {
			...
			$ref_bonus_get_2 = $row_user["ref_bonus_get_2"];
			$my_visits = $row_user["visits"];
			$my_visits_bs = $row_user["visits_bs"];
		...
		$sql_link = mysql_query("SELECT `id`,`status`,`username`,`type_serf`,`nolimit`,`active`,`timer`,`revisit`,`geo_targ`,`new_users`,`no_ref`,`to_ref`,`sex_adv` FROM `tb_ads_dlink` 
		...
		if(mysql_num_rows($sql_link)>0) {
			...
			$new_users = $row_link["new_users"];
			$no_ref = $row_link["no_ref"];
			$to_ref = $row_link["to_ref"];
			$sex_adv = $row_link["sex_adv"];
			...
			}elseif($type_serf==3 && $my_vip_serf<=0) {
		...
		$sql_visit = mysql_query("SELECT `id` FROM `tb_ads_dlink_visits` WHERE `username`='$username' AND `ident`='$id_adv_post' AND `time_next`>='".time()."' ORDER BY `id` DESC") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
		if(mysql_num_rows($sql_visit)>0) {
			...
			mysql_query("DELETE FROM `tb_ads_dlink_visits` WHERE `username`='$username' AND `ident`='$id_adv_post' AND `time_next`<'".time()."'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
		}
		...
		if($id_adv_post==false | $id_adv_sess==false | $captcha_type==false | $code_post==false | $code_sess==false) {
			...
			$sql_up_ins = mysql_query("SELECT `id` FROM `tb_ads_dlink_visits` WHERE `username`='$username' AND `ident`='$id_adv_post' ORDER BY `id` DESC") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
			if(mysql_num_rows($sql_up_ins)>0) {
				mysql_query("UPDATE `tb_ads_dlink_visits` SET `status`='0',`time`='".time()."',`time_next`='$time_next',`money`='0',`ip`='$my_lastiplog' WHERE `username`='$username' AND `ident`='$id_adv_post'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
			}else{
				mysql_query("INSERT INTO `tb_ads_dlink_visits` (`status`,`ident`,`time`,`time_next`,`username`,`ip`, `money`) 
			...
			$ref_proc_1 = 0; $ref_proc_2 = 0; $ref_proc_3 = 0;
			...
			/*if($my_referer_1 != false && $type_serf != 10) {
				...
				if(mysql_num_rows($sql_ref_1)>0) {
					...
					($sql_rang_1 = mysql_query("SELECT `c_1` FROM `tb_config_rang` WHERE `r_ot`<='$reit_ref_1' AND `r_do`>='" . floor($reit_ref_1) . "'")) || exit(my_json_encode($ajax_json, "ERROR", mysql_error()));
					$ref_proc_1 = (0 < mysql_num_rows($sql_rang_1) ? mysql_result($sql_rang_1, 0, 0) : 0);

						
					if($my_referer_2 != false) {
						$sql_ref_2 = mysql_query("SELECT `id`,`reiting`,`money_rb`,`referer` FROM `tb_users` WHERE `username`='$my_referer_2'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
						
						if(mysql_num_rows($sql_ref_2)>0) {
							$row_ref_2 = mysql_fetch_assoc($sql_ref_2);
							$user_id_ref_2 = $row_ref_2["id"];
							$reit_ref_2 = $row_ref_2["reiting"];
							$money_ref_2 = $row_ref_2["money_rb"];
							$my_referer_3 = $row_ref_2["referer"];
						    ($sql_rang_2 = mysql_query("SELECT `c_2` FROM `tb_config_rang` WHERE `r_ot`<='$reit_ref_2' AND `r_do`>='" . floor($reit_ref_2) . "'")) || exit(my_json_encode($ajax_json, "ERROR", mysql_error()));
					...
					if($my_referer_3 != false) {
						...
						if(mysql_num_rows($sql_ref_3)>0) {
						    ...
						    ($sql_rang_3 = mysql_query("SELECT `c_3` FROM `tb_config_rang` WHERE `r_ot`<='$reit_ref_3' AND `r_do`>='" . floor($reit_ref_3) . "'")) || exit(my_json_encode($ajax_json, "ERROR", mysql_error()));
			...
			if($my_referer_1 != false && $type_serf != 10) {
				...
				if(mysql_num_rows($sql_ref_1)>0) {
					...
					$sql_rang_1 = mysql_query("SELECT `c_1` FROM `tb_config_rang` WHERE `r_ot`<='$reit_ref_1' AND `r_do`>='".floor($reit_ref_1)."'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
					...
					if($my_referer_2 != false) {
						...
						if(mysql_num_rows($sql_ref_2)>0) {
							...
							$sql_rang_2 = mysql_query("SELECT `c_2` FROM `tb_config_rang` WHERE `r_ot`<='$reit_ref_2' AND `r_do`>='".floor($reit_ref_2)."'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
							...
							if($my_referer_3 != false) {
								...
								if(mysql_num_rows($sql_ref_3)>0) {
									...
									$sql_rang_3 = mysql_query("SELECT `c_3` FROM `tb_config_rang` WHERE `r_ot`<='$reit_ref_3' AND `r_do`>='".floor($reit_ref_3)."'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
			...
			$sql = mysql_query("SELECT `price` FROM `tb_config` WHERE `item`='reit_ds' AND `howmany`='1'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
			$reiting_serf = mysql_result($sql,0,0);

			$sql = mysql_query("SELECT `price` FROM `tb_config` WHERE `item`='dlink_nacenka' AND `howmany`='1'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
			$dlink_nacenka = mysql_result($sql,0,0);

			$sql = mysql_query("SELECT `price` FROM `tb_config` WHERE `item`='".($type_serf==2 | $type_serf==4 ? "dlink_cena_hits_bs" : "dlink_cena_hits")."' AND `howmany`='1'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
			$dlink_cena_hits = mysql_result($sql,0,0);

			$sql = mysql_query("SELECT `price` FROM `tb_config` WHERE `item`='dlink_cena_active' AND `howmany`='1'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
			$dlink_cena_active = mysql_result($sql,0,0);

			$sql = mysql_query("SELECT `price` FROM `tb_config` WHERE `item`='dlink_timer_ot' AND `howmany`='1'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
			$dlink_timer_ot = mysql_result($sql,0,0);

			$sql = mysql_query("SELECT `price` FROM `tb_config` WHERE `item`='dlink_cena_timer' AND `howmany`='1'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
			...
			if($my_ref_back > 0) 	{ $dlink_click_us_see = "(".round(p_floor($dlink_click_us, 4), 4)." + ".round(p_floor($my_ref_back, 4), 4)." рефбек)"; }
			else 			{ $dlink_click_us_see = round(p_floor($dlink_click_us, 4), 4); }

			$dlink_click_us = round(p_floor(($dlink_click_us + $my_ref_back),4),4);
			$add_money_r_1 = round(p_floor(($dlink_click_us * ($ref_proc_1/100) - $my_ref_back), 4),4);
			$add_money_r_2 = round(p_floor(($dlink_click_us * ($ref_proc_2/100)), 4), 4);
			$add_money_r_3 = round(p_floor(($dlink_click_us * ($ref_proc_3/100)), 4), 4);
			//$add_money_r_4 = round(p_floor($dlink_click_us * ($ref_proc_4 / 100), 4), 4);
			//$add_money_r_5 = round(p_floor($dlink_click_us * ($ref_proc_5 / 100), 4), 4);
			
			$dlink_click_ref = ($dlink_cena_hits + $active * $dlink_cena_active + abs($timer-$dlink_timer_ot) * $dlink_cena_timer);

			$dlink_click_ref = round(p_floor(($dlink_click_ref),4),4);
			$add_money_ref_1 = round(p_floor(($dlink_click_ref * ($ref_proc_1/100)), 4),4);
			$add_money_ref_2 = round(p_floor(($dlink_click_ref * ($ref_proc_2/100)), 4), 4);
			$add_money_ref_3 = round(p_floor(($dlink_click_ref * ($ref_proc_3/100)), 4), 4);
			//$add_money_ref_4 = round(p_floor(($dlink_click_ref * ($ref_proc_4/100)), 4), 4);
			//$add_money_ref_5 = round(p_floor(($dlink_click_ref * ($ref_proc_5/100)), 4), 4);

			$sql_up_ins = mysql_query("SELECT `id` FROM `tb_ads_dlink_visits` WHERE `username`='$username' AND `ident`='$id_adv_post' ORDER BY `id` DESC") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
			if(mysql_num_rows($sql_up_ins)>0) {
				mysql_query("UPDATE `tb_ads_dlink_visits` SET `status`='1',`time`='".time()."',`time_next`='$time_next',`money`='$dlink_click_us',`ip`='$my_lastiplog' WHERE `username`='$username' AND `ident`='$id_adv_post'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
			}else{
				mysql_query("INSERT INTO `tb_ads_dlink_visits` (`status`,`ident`,`time`,`time_next`,`username`,`ip`, `money`) 
				VALUES('1','$id_adv_post','".time()."','$time_next','$username','$my_lastiplog','$dlink_click_us')") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
			}

			if($type_serf==10) {
				$dlink_click_us = 0; $reiting_serf = 0;
				$add_money_r_1 = 0; $add_money_r_2 = 0; $add_money_r_3 = 0; $add_money_r_4 = 0; $add_money_r_5 = 0;
				$dlink_click_us_see = 0;
				$totals_tab = ($nolimit==0 | $nolimit<time()) ? "`totals`=`totals`-'1', " : false;
				
				mysql_query("UPDATE `tb_ads_dlink` SET $totals_tab `members`=`members`+'1' WHERE `id`='$id_adv_post' AND `status`='1'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
				...
				$totals_tab = ($nolimit==0 | $nolimit<time()) ? "`totals`=`totals`-'1', " : false;
				$pay_users_tab = ($dlink_click_us + $add_money_r_1 + $add_money_r_2 + $add_money_r_3);

				//mysql_query("UPDATE `tb_ads_dlink`  SET `totals`=`totals`-'1',`members`=`members`+'1',`outside`=`outside`+'1', `pay_users`=`pay_users`+'$pay_users_tab' WHERE `id`='$id_adv_post' AND `status`='1'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
				mysql_query("UPDATE `tb_ads_dlink` SET $totals_tab `members`=`members`+'1',`outside`=`outside`+'1', `pay_users`=`pay_users`+'$pay_users_tab' WHERE `id`='$id_adv_post' AND `status`='1'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));

				mysql_query("UPDATE `tb_ads_dlink`  SET `limit_h_now`=`limit_h_now`-'1' WHERE `id`='$id_adv_post' AND `status`='1' AND `limit_h`>'0'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
				mysql_query("UPDATE `tb_ads_dlink`  SET `limit_d_now`=`limit_d_now`-'1' WHERE `id`='$id_adv_post' AND `status`='1' AND `limit_d`>'0'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
				mysql_query("UPDATE `tb_statistics` SET `hitov`=`hitov`+'1' WHERE `id`='1'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));

				if($type_serf==3 && $my_vip_serf>0) {
					$vip_serf_tab = ", `vip_serf`=`vip_serf`-'1'";
				}elseif($type_serf==4 && $my_ban_serf>0) {
				...
				if($type_serf==2 | $type_serf==4) {
					mysql_query("UPDATE `tb_users` SET `reiting`=`reiting`+'$reiting_serf', `visits_bs`=`visits_bs`+'1', `money`=`money`+'$dlink_click_us', `money_bs`=`money_bs`+'$dlink_click_us' $vip_serf_tab WHERE `username`='$username'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
					...
					### START СБОР СТАТИСТИКИ ####
					stats_copilka($username, strtolower(DATE("D")), "serf_copilka");
					### END СБОР СТАТИСТИКИ ######
					
					### START СБОР СТАТИСТИКИ ####
					stats_users_pay($username, strtolower(DATE("D")), "serf_pay");
					### END СБОР СТАТИСТИКИ ######
					
				}else{
					mysql_query("UPDATE `tb_users` SET `reiting`=`reiting`+'$reiting_serf', `visits`=`visits`+'1', `money`=`money`+'$dlink_click_us', `money_ds`=`money_ds`+'$dlink_click_us' $vip_serf_tab WHERE `username`='$username'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
				
				    ### START СБОР СТАТИСТИКИ ####
				stats_users($username, strtolower(DATE("D")), "serf");
				### END СБОР СТАТИСТИКИ ######
				...
				### АДМИН-КОНКУРС КЛИКЕРОВ №1 ###
				if($geo_targ == false && strtolower($rek_name) != strtolower($username) && $new_users == 0 && $no_ref == 0 && $to_ref == 0 && $sex_adv == 0 && $my_ban_date == 0) {
					$sql = mysql_query("SELECT `price` FROM `tb_konkurs_conf` WHERE `type`='click' AND `item`='status'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
					$konk_click_status = mysql_result($sql,0,0);

					if($konk_click_status==1) {
						$sql = mysql_query("SELECT `price` FROM `tb_konkurs_conf` WHERE `type`='click' AND `item`='date_start'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
						$konk_click_date_start = mysql_result($sql,0,0);

						$sql = mysql_query("SELECT `price` FROM `tb_konkurs_conf` WHERE `type`='click' AND `item`='date_end'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
						$konk_click_date_end = mysql_result($sql,0,0);

						if($konk_click_date_end>=time() && $konk_click_date_start<=time()) {
							mysql_query("UPDATE `tb_users` SET `konkurs_click`=`konkurs_click`+'1' WHERE `username`='$username' AND `username` NOT IN (SELECT `user_name` FROM `tb_konkurs_exp`)") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
						}
					}
				}
				### АДМИН-КОНКУРС КЛИКЕРОВ №1 ###
				...
				if($geo_targ == false && strtolower($rek_name) != strtolower($username) && $my_ban_date == 0) {
					...
					if($konk_complex_status==1) {
						...
						$sql = mysql_query("SELECT `price` FROM `tb_konkurs_conf` WHERE `type`='complex' AND `item`='point_serf'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
				...
				### БОНУС ЗА РЕГИСТРАЦИЮ РЕФЕРАЛА ОТ АДМИНА ###
				$sql_bonus_ref = mysql_query("SELECT `price` FROM `tb_config` WHERE `item`='bon_reg_ref' AND `howmany`='1'");
						$bonus_reg_ref = mysql_num_rows($sql_bonus_ref)>0 ? mysql_result($sql_bonus_ref,0,0) : 0;
						
						if($my_referer_1 != false /*&& $statusref != 6*/ && isset($bonus_reg_ref) && $bonus_reg_ref>0) {
						    ...
						    if($bonus_reg_ref && ($my_visits+$my_visits_bs+1)==$konk_ref_min_click_ref) {
							mysql_query("UPDATE `tb_users` SET `money`=`money`+'$bonus_reg_ref' WHERE `username`='$my_referer_1'");
							mysql_query("INSERT INTO `tb_history` (`status_pay`,`user`,`date`,`time`,`amount`,`method`,`status`,`tipo`) 
							VALUES('1','$my_referer_1','".DATE("d.m.Y H:i", time())."','".time()."','$bonus_reg_ref','Бонус от Администрации за привлечение реферала $username','Зачислено','prihod')") or die(($ajax_json=="json") ? json_encode_cp1251(array("result" => "ERROR", "message" => iconv("CP1251", "UTF-8", mysql_error()))) : mysql_error());
							mysql_query("INSERT INTO `tb_mail_in`(`namein`,`nameout`,`subject`,`message`,`status`,`date`,`ip`) 
							VALUES('$my_referer_1','Система','Бонус от Администрации','Бонус от Администрации за привлечение реферала $username в размере $bonus_reg_ref руб. Зачислен!','0','".time()."','0.0.0.0')");
						}
					}
						### БОНУС ЗА РЕГИСТРАЦИЮ РЕФЕРАЛА ОТ АДМИНА ###
						
						### БОНУС ЗА РЕГИСТРАЦИЮ РЕФЕРАЛА ОТ РЕФЕРЕРА ###
				if($my_referer_1 != false && $my_referer_2 != false) {
					list($bonus_reg_id_ref,$type_bon_ref,$count_nado_ref,$ref_description,$bonus_reg_ref_ref)=mysql_fetch_array(mysql_query("SELECT `id`,`type_bon`,`count_nado`,`description`,`bonus` FROM `tb_refbonus` WHERE `status`='1' AND `type_bon`='2' AND `username`='$my_referer_2' ORDER BY `id` DESC LIMIT 1"));
					list($ref_bonus_add)=mysql_fetch_array(mysql_query("SELECT `ref_bonus_add` FROM `tb_users` WHERE `username`='$my_referer_1'"));

	                if (isset($ref_bonus_add) && $ref_bonus_add == 1) {
				        if ($count_nado_ref=="") {$count_nado_ref=0;}
				        if($count_nado_ref>0 && $bonus_reg_ref_ref>0) {
							if($bonus_reg_ref_ref && $ref_bonus_get_2==0 && ($my_visits+$my_visits_bs+1)==$count_nado_ref) {
						        ...
						        $money_urefererref_nado = ($bonus_reg_ref_ref * ($comis_sys_bon+100)/100);
							    $money_urefererref_nado = round($money_urefererref_nado, 2);

							    if ($money_ref_2>=$money_urefererref_nado) {
								    mysql_query("UPDATE `tb_refbonus` SET `count_bon`=`count_bon`+'1' WHERE `id`='$bonus_reg_id_ref'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
									mysql_query("INSERT INTO `tb_refbonus_stat` (`status`,`username`,`stat_info`,`ident`,`type`,`date`,`money`) VALUES('1','$my_referer_1','$user_id','$bonus_reg_id_ref','$type_bon_ref','".time()."','$bonus_reg_ref_ref')") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
					                ...
					                mysql_query("UPDATE `tb_users` SET `money`=`money`+'$bonus_reg_ref_ref' WHERE `username`='$my_referer_1'");
									mysql_query("UPDATE `tb_users` SET `money_rb`=`money_rb`-'$money_urefererref_nado' WHERE `username`='$my_referer_2'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));

					                mysql_query("INSERT INTO `tb_history` (`status_pay`,`user`,`date`,`time`,`amount`,`method`,`status`,`tipo`) VALUES('1','$my_referer_1','".DATE("d.m.Y H:i", time())."','".time()."','$bonus_reg_ref_ref','Бонус от реферера за привлечение реферала $username','Зачислено','prihod')") or die(($ajax_json=="json") ? json_encode_cp1251(array("result" => "ERROR", "message" => iconv("CP1251", "UTF-8", mysql_error()))) : mysql_error());
									mysql_query("INSERT INTO `tb_history` (`user`,`user_id`,`date`,`time`,`amount`,`method`,`status`,`tipo`) 
									VALUES('$my_referer_2','$user_id_ref_2','".DATE("d.m.Y H:i")."','".time()."','$money_urefererref_nado','Реф-Бонус рефералу $my_referer_1 за привлечение реферала $username','Списано','rashod')") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));

					                if(trim($ref_description)!=false) {
										mysql_query("INSERT INTO `tb_mail_in`(`namein`,`nameout`,`subject`,`message`,`status`,`date`,`ip`) 
										VALUES('$username','Система','Бонус от реферера','$ref_description','0','".time()."','0.0.0.0')");
									}
								} else {
								    mysql_query("INSERT INTO `tb_refbonus_stat` (`status`,`username`,`stat_info`,`ident`,`type`,`date`,`money`) VALUES('0','$my_referer_1','$user_id','$bonus_reg_id_ref','$type_bon_ref','".time()."','$bonus_reg_ref_ref')") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
								}
					        }
				        }
					}
				...
				if($my_referer_1 != false && $my_ban_date == 0) {
					...
					### АДМИН-КОНКУРС АКТИВНЫХ РЕФЕРАЛОВ NEW ###
					$sql = mysql_query("SELECT `price` FROM `tb_konkurs_conf` WHERE `type`='ref' AND `item`='status'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
					$konk_ref_status = mysql_result($sql,0,0);

					if($konk_ref_status==1) {
						$sql = mysql_query("SELECT `price` FROM `tb_konkurs_conf` WHERE `type`='ref' AND `item`='date_start'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
						$konk_ref_date_start = mysql_result($sql,0,0);

						$sql = mysql_query("SELECT `price` FROM `tb_konkurs_conf` WHERE `type`='ref' AND `item`='date_end'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
						$konk_ref_date_end = mysql_result($sql,0,0);

						$sql = mysql_query("SELECT `price` FROM `tb_konkurs_conf` WHERE `type`='ref' AND `item`='min_click'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
						$konk_ref_min_click = mysql_result($sql,0,0);

						if( $my_joindate>=$konk_ref_date_start && ($my_visits+$my_visits_bs+1)==$konk_ref_min_click && $konk_ref_date_end>=time() && $konk_ref_date_start<=time() ) {
							mysql_query("UPDATE `tb_users` SET `konkurs_ref`=`konkurs_ref`+'1' WHERE `username`='$my_referer_1' AND `username` NOT IN (SELECT `user_name` FROM `tb_konkurs_exp`)") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
						}
					}
					### АДМИН-КОНКУРС АКТИВНЫХ РЕФЕРАЛОВ NEW ###
					...
					if($geo_targ == false && strtolower($rek_name) != strtolower($username)) {
						$sql_t = mysql_query("SELECT * FROM `tb_refkonkurs` WHERE `status`='1' AND `username`='$my_referer_1' AND `type_kon`='2' AND `date_s`<'".time()."' AND `date_e`>'".time()."' ORDER BY `id` DESC LIMIT 1");
				...
				//}
					...
					if($my_referer_1 != false && $my_referer_2 != false) {
						$sql_t = mysql_query("SELECT * FROM `tb_refkonkurs` WHERE `status`='1' AND `username`='$my_referer_2' AND `type_kon`='3' AND `date_s`<'".time()."' AND `date_e`>'".time()."' AND `date_s`<='$my_joindate' ORDER BY `id` DESC LIMIT 1") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
						if(mysql_num_rows($sql_t)>0) {
							$row_t = mysql_fetch_assoc($sql_t);

							if($row_t["limit2_kon"]!=0 && ($my_visits+$my_visits_bs+1)==$row_t["limit2_kon"]) {
								$sql_kon = mysql_query("SELECT `id` FROM `tb_refkonkurs_stat` WHERE `username`='$my_referer_1' AND `referer`='1' AND `ident`='".$row_t["id"]."' AND `type`='".$row_t["type_kon"]."'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
								if(mysql_num_rows($sql_kon) > 0) {
									mysql_query("UPDATE `tb_refkonkurs_stat` SET `kolvo`=`kolvo`+'1' WHERE `username`='$my_referer_1' AND `referer`='1' AND `ident`='".$row_t["id"]."' AND `type`='".$row_t["type_kon"]."'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
								}else{
									mysql_query("INSERT INTO `tb_refkonkurs_stat` (`username`,`referer`,`ident`,`type`,`kolvo`) 
									VALUES('$my_referer_1','1','".$row_t["id"]."','".$row_t["type_kon"]."','1')") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
								}
							}
						}

						if($my_referer_3 != false) {
							$sql_t = mysql_query("SELECT * FROM `tb_refkonkurs` WHERE `status`='1' AND `username`='$my_referer_3' AND `type_kon`='3' AND `glubina`='1' AND `date_s`<'".time()."' AND `date_e`>'".time()."' AND `date_s`<='$my_joindate' ORDER BY `id` DESC LIMIT 1") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
							if(mysql_num_rows($sql_t)>0) {
								$row_t = mysql_fetch_assoc($sql_t);

								if($row_t["limit2_kon"]!=0 && ($my_visits+$my_visits_bs+1)==$row_t["limit2_kon"]) {
									$sql_kon = mysql_query("SELECT `id` FROM `tb_refkonkurs_stat` WHERE `username`='$my_referer_1' AND `referer`='2' AND `ident`='".$row_t["id"]."' AND `type`='".$row_t["type_kon"]."'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
									if(mysql_num_rows($sql_kon) > 0) {
										mysql_query("UPDATE `tb_refkonkurs_stat` SET `kolvo`=`kolvo`+'1' WHERE `username`='$my_referer_1' AND `referer`='2' AND `ident`='".$row_t["id"]."' AND `type`='".$row_t["type_kon"]."'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
									}else{
										mysql_query("INSERT INTO `tb_refkonkurs_stat` (`username`,`referer`,`ident`,`type`,`kolvo`) 
										VALUES('$my_referer_1','2','".$row_t["id"]."','".$row_t["type_kon"]."','1')") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
									}
								}
							}
						}
					...
					$sql_r_b_1 = mysql_query("SELECT * FROM `tb_refbonus` WHERE `status`='1' AND `username`='$my_referer_1' AND `type_bon`='3' ORDER BY `id` DESC LIMIT 1");
					...
					if($my_referer_2 != false) {
						...
						if($geo_targ == false && strtolower($rek_name) != strtolower($username)) {
							$sql_t = mysql_query("SELECT * FROM `tb_refkonkurs` WHERE `status`='1' AND `username`='$my_referer_2' AND `type_kon`='2' AND `glubina`='1' AND `date_s`<'".time()."' AND `date_e`>'".time()."' ORDER BY `id` DESC LIMIT 1") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
							if(mysql_num_rows($sql_t)>0) {
								$row_t = mysql_fetch_assoc($sql_t);
					...
					//}
						...
						if($my_referer_3 != false) {
							...
							if($geo_targ == false && strtolower($rek_name) != strtolower($username)) {
								$sql_t = mysql_query("SELECT * FROM `tb_refkonkurs` WHERE `status`='1' AND `username`='$my_referer_3' AND `type_kon`='2' AND `glubina`='1' AND `date_s`<'".time()."' AND `date_e`>'".time()."' ORDER BY `id` DESC LIMIT 1") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
								if(mysql_num_rows($sql_t)>0) {
									$row_t = mysql_fetch_assoc($sql_t);
			...
			$result_text = "OK";
            $message_text = "<script>parent.opener.postMessage('{\"op\":\"serf_mess\", \"id\":\"" . $id_adv_post . "\", \"mess\":\"За просмотр ссылки на Ваш баланс начислено <b>" . $dlink_click_us . "</b> руб.\"}', 'https://" . $_SERVER["HTTP_HOST"] . "');</script>";			
			$message_text .= '<div class="block-success">Спасибо за посещение!<span>Оплата за просмотр <b>'.$dlink_click_us_see.' руб.</b> зачислена.</span></div>';
			exit(my_json_encode($ajax_json, $result_text, $message_text));
		}else{
