<?php
if(!DEFINED("BEGSTROKA_AJAX")) {die ("Hacking attempt!");}
...
if($option == "delete") {
	...
	if(mysql_num_rows($sql)>0) {
		...
		if($status == 0) {
			mysql_query("DELETE FROM `tb_ads_beg_stroka` WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
			exit("OK");
		}elseif($status == 1) {
	...
	$color = (isset($_POST["color"]) && preg_match("|^[\d]{1,2}$|", intval(limpiarez($_POST["color"]))) && (intval($_POST["color"])==0 | intval($_POST["color"])==1)) ? intval(limpiarez($_POST["color"])) : 0;
	...
	if(mysql_num_rows($sql)>0) {
		...
		$status = $row["status"];
		$date_end = $row["date_end"];
		$color = ($status == 1 | $status == 2) ? $row["color"] : $color;
		...
		if(mysql_num_rows($sql_bl)>0 && $black_url!=false) {
			...
			mysql_query("UPDATE `tb_ads_beg_stroka` SET `color`='$color', `url`='$url', `description`='$description', `ip`='$laip'  WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
			exit("OK");
		}
	}else{
		exit("У Вас нет рекламной площадки с ID - $id");
	...
	if(mysql_num_rows($sql)>0) {
		...
		$cena = $plan * ($beg_stroka_cena + $color * $beg_stroka_cena_color) * (100-$cab_skidka)/100;
		$money_pay = number_format($cena, 2, ".", "");

		if($plan<$beg_stroka_min) {
			exit("ERROR1");
			...
			if($status=="0" | $status=="3") {
				...
				mysql_query("UPDATE `tb_ads_beg_stroka` SET `status`='1', `wm_sent`='0', `method_pay`='10', `plan`='$plan', `date`='".time()."', `date_end`='".(time()+$plan*24*60*60)."', `ip`='$laip', `money`='$money_pay'  WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
				mysql_query("INSERT INTO `tb_history` (`user`,`date`,`amount`,`method`,`status`,`tipo`) 
				VALUES('$username','".DATE("d.m.Yг. H:i")."','$money_pay','Пополнение баланса ссылки в бегущей строке ID:$id','Списано','rashod')") or die(mysql_error());

				mysql_query("UPDATE `tb_ads_beg_stroka` SET `plan`=(`date_end`-`date`)/86400  WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
				...
				mysql_query("UPDATE `tb_users` SET `reiting`=`reiting`+'$reit_add_1', `money_rb`=`money_rb`-'$money_pay', `money_rek`=`money_rek`+'$money_pay'  WHERE `username`='$username'") or die(mysql_error());
				mysql_query("UPDATE `tb_ads_beg_stroka` SET `status`='1', `method_pay`='10', `plan`=`plan`+'$plan', `date_end`=`date_end`+'".($plan*24*60*60)."', `ip`='$laip', `money`=`money`+'$money_pay'  WHERE `id`='$id' AND `username`='$username'") or die(mysql_error());
				mysql_query("INSERT INTO `tb_history` (`user`,`date`,`amount`,`method`,`status`,`tipo`) 
				VALUES('$username','".DATE("d.m.Yг. H:i")."','$money_pay','Пополнение баланса ссылки в бегущей строке ID:$id','Списано','rashod')") or die(mysql_error());
