    ...
    if(status_form == '0'){
      ...
      $.ajax({      	
        url: url,
      	type: 'POST',
      	data: $(this).serialize(),
	beforeSend: function () { $(".job-reply-btns").html("<img src=\"img/load-offers1.gif\" style=\"display:block;margin:0 auto 20px auto;\">"); }, 	
        error: function (infa){
		status_form = 0;
...
//console.log(infa);
        ...
        complete: function () { $(".job-reply-btns").html(""); }
      });
    }
  ...
  $('body').on('submit', '.aj-form', function() {
    var url = $(this).attr('action'),
    arrInfa = $(this).data('infa').split(/\s*,\s*/), dpost = $(this).serializeArray();
    if(status_form == '0'){
      status_form = 1;
      $.ajax({      	
        url: url,
        ...
        beforeSend: function () { $("#zaiavca-"+dpost['2'].value+" .job-reply-btns").html("<img src=\"img/load-offers1.gif\" style=\"display:block;margin:0 auto 20px auto;\">"); },
        error: function (infa){
		status_form = 0; 
        ...
        complete: function () { $("#zaiavca-"+dpost['2'].value+" .job-reply-btns").html(""); } 
      });
    }    
    return false;
  });
...
function task_fu(e, link, zapros, type_in = 'script', elem = '') {
  e_js_post = $(e);
