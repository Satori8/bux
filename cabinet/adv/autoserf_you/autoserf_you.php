...
if(isset($ads) && isset($id) && isset($op) && $ads=="youautoserf" && $op=="edit" && $id>0) {
	if(!DEFINED("YOUAUTOSERF_EDIT")) DEFINE("YOUAUTOSERF_EDIT", true);
	include("autoserf_edit_you.php");
	include(DOC_ROOT."/footer.php");
	exit();
}

if(isset($ads) && isset($id) && isset($op) && $ads=="youautoserf" && $op=="statistics" && $id>0) {
	if(!DEFINED("YOUAUTOSERF_STAT")) DEFINE("YOUAUTOSERF_STAT", true);
	...
	include("autoserf_stat_you.php");
	include(DOC_ROOT."/footer.php");
	exit();
}

echo '<a name="goto"></a>';
echo '<h5 class="sp" style="margin-top:0px; font-size:14px; font-weight:bold; text-align:center;">Мои видеоролики в авто-серфинге <span style="color: #3F3F3F;">You</span><span style="border-radius: 5px; background-color: #E62117; padding: 2px; color: #FFFFFF;">Tube</span></h5>';
...
$sql = mysql_query("SELECT * FROM `tb_ads_autoyoutube` WHERE `username`='$username' AND ytflag='1' ORDER BY `id` ASC");
if(mysql_num_rows($sql)>0) {
	...
	while ($row = mysql_fetch_array($sql)) {
		...
		echo '<tr id="adv_dell'.$row["id"].'">';
			...
			echo '<td align="left" class="noborder2">';
				...
				echo '</span>';
				echo '<span class="adv-dell" title="Удалить ссылку" onClick="alert_delete('.$row["id"].', \'youautoserf\');"></span>';
				echo '<a class="adv-statistics" href="?ads='.$ads.'&op=statistics&id='.$row["id"].'#goto" title="Статистика просмотра"></a>';
				echo '<span class="adv-erase" title="Сброс статистики" onClick="clear_stat('.$row["id"].', \'youautoserf\', '.$row["outside"].');"></span>';
				///if($row["status"]=="1") {
					//echo '<span class="adv-edit" title="Редактировать ссылку" onClick="alert_edit();"></span>';
					//}else{
		...
		echo '<tr id="adv_addmoney'.$row["id"].'" style="display: none">';
			...
			echo '<td align="center" colspan="3" class="ext-text">';
				...
				echo '<center><span onClick="pay_adv('.$row["id"].', \'youautoserf\');" class="sub-green" style="float:none;" title="Пополнить бюджет площадки">Оплатить</span></center>';
	...
	echo '<span class="msg-w">У вас нет своих размещённых видеороликов в авто-серфинге <span style="color: #3F3F3F;">You<span style="border-radius: 5px; background-color: #E62117; padding: 2px; color: #FFFFFF;">Tube</span></span></span>';
}

echo '<div align="center"><span class="proc-btn" onClick="document.location.href=\'/advertise.php?ads=auto_serf_you\'">Разместить видеоролик</span></div>';
?>