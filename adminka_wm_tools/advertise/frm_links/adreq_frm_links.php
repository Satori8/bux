...
if(isset($_GET["id"])) {
	...
	if($option=="add") {
		$sql = mysql_query("SELECT * FROM `tb_ads_frm` WHERE `id`='$id' AND `status`='0'");
		if(mysql_num_rows($sql)>0) {
			mysql_query("UPDATE `tb_ads_frm` SET `status`='1', `date`='".time()."', `date_end`=`plan`*'".(24*60*60)."'+'".time()."' WHERE `id`='$id'") or die(mysql_error());
...
$sql = mysql_query("SELECT * FROM `tb_ads_frm` WHERE `status`='0' ORDER BY `id` ASC");
if(mysql_num_rows($sql)>0) {
