...
if(count($_POST)>0 && isset($_POST["id_pay"])) {
	if(!isset($_SESSION["userLog"]) && !isset($_SESSION["userPas"])) {
		...
		$sql_id = mysql_query("SELECT `money` FROM `tb_ads_rc` WHERE `id`='$id_pay' AND `status`='0' AND `username`='$username' ORDER BY `id` DESC LIMIT 1");
		if(mysql_num_rows($sql_id)>0) {
			...
			if($money_user>=$money_pay) {
				...
				mysql_query("UPDATE `tb_ads_rc` SET `status`='1', `date`='".time()."', `wmid`='$wmid_user' WHERE `id`='$id_pay' AND `status`='0' AND `username`='$username' ORDER BY `id` DESC LIMIT 1") or die(mysql_error());
				mysql_query("INSERT INTO `tb_history` (`user`,`date`,`amount`,`method`,`status`,`tipo`) VALUES('$username','".DATE("d.m.Yг. H:i")."','$money_pay',  'Оплата рекламы: - Рекламная цепочка','Списано','rashod')") or die(mysql_error());
...
if(count($_POST)>0) {
	...
	$sql_bl = mysql_query("SELECT * FROM `tb_black_sites` WHERE `domen` LIKE '%$black_url%'");
...
$system_pay[21] = "AdvCash";
		...
		mysql_query("DELETE FROM `tb_ads_rc` WHERE `status`='0' AND `date`<'".(time()-(24*60*60))."'") or die(mysql_error());
		...
		$check_wmid = mysql_query("SELECT `id` FROM `tb_ads_rc` WHERE `status`='0' AND `session_ident`='".session_id()."' ORDER BY `id` DESC LIMIT 1");
		if(mysql_num_rows($check_wmid)>0) {
			mysql_query("UPDATE `tb_ads_rc` SET `merch_tran_id`='$merch_tran_id',`method_pay`='$method_pay',`date`='".time()."',`wmid`='$wmid_user',`username`='$username',`color`='$color',`url`='$url',`description`='$description',`ip`='$laip',`money`='$summa' WHERE `status`='0' AND `session_ident`='".session_id()."'") or die(mysql_error());
		}else{
			mysql_query("INSERT INTO `tb_ads_rc` (`status`,`session_ident`,`merch_tran_id`,`method_pay`,`date`,`wmid`,`username`,`color`,`url`,`description`,`ip`,`money`) 
			VALUES('0','".session_id()."','$merch_tran_id','$method_pay','".time()."','$wmid_user','$username','$color','$url','$description','$laip','$summa')") or die(mysql_error());
		}

		$sql_id = mysql_query("SELECT `id` FROM `tb_ads_rc` WHERE `status`='0' AND `session_ident`='".session_id()."' ORDER BY `id` DESC LIMIT 1");
