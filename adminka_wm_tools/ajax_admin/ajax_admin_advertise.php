...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) && $_SERVER["HTTP_X_REQUESTED_WITH"]=="XMLHttpRequest") {
	...
	if(isset($_SESSION["userLog_a"]) && isset($_SESSION["userPas_a"])) {
		$sql_user = mysql_query("SELECT `id`,`username`,`wmid`,`user_status` FROM `tb_users` WHERE `username`='$user_name' AND md5(`password`)='$user_pass'") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
