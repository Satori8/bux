...
echo '<thead><tr>';
	...
	echo '<th class="top">Отчёт исполнителя:<br>Причина</th>';
	echo '<th class="top">IP</th>';
echo '</tr></thead>';

$sql = mysql_query("SELECT * FROM `tb_ads_task_pay` WHERE `ident`='$is' AND `type`='task' AND `status`='bad' ORDER BY `id` DESC");
if(mysql_num_rows($sql)>0) {
	...
	while($stat=mysql_fetch_array($sql1)) {
	 ...
	 echo ' <span style="color: #cd5b45;">ID: '.$stat["id"].'</span>';
		...
		echo'</td>';
		echo '<td width="15%">'.$row["ctext"].'<br>'.$row["why"].'</td>';
