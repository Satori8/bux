		...
		$message_text.= '<div id="ToPaySys" style="display:none;"><table class="tables">';
			...
			$message_text.= '<tr><td align="center" colspan="2">';
				if($method_pay == 10 && $user_name != false) {
					$message_text.= '<span onClick="PayAds(\''.$id_zakaz.'\', \''.$type_ads.'\');" class="proc-btn" style="float:none;">Оплатить</span>';
	...
	if($user_name==false) {
		...
		if(mysql_num_rows($sql_id)>0) {
			...
			if($user_money_rb >= $money_pay) {
				...
				VALUES('1','$user_name','$user_id','".DATE("d.m.Y H:i")."','".time()."','$money_pay','Оплата рекламы (рассылка на e-mail, ID:$id)','Оплачено','reklama')") or die(my_json_encode($ajax_json, "ERROR", mysql_error()));
