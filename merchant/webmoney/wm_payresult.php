...
$secret_key = "hgvbvbvbtuihbnoiru84";
		...
		if(mysql_num_rows($sql)) {
			...
			/////////////////////	
				if($bon_pay_status==1){
					...
					mysql_query("INSERT INTO `tb_history` (`user`, `amount`, `date`, `method`, `status`, `tipo`) VALUES('$username', '$merch_amount1', '".DATE("d.m.Yг. H:i")."', 'Пополнение рекламного баланса через WebMoney','Зачислено', 'popoln')") or die(mysql_error());
		...
		/////////////////////////////////////////////////////
			...
			/////////////////////	
				if($bon_inv_status==1){
					...
					VALUES('$username', '$merch_amount', '".DATE("d.m.Yг. H:i")."', 'Пополнение баланса инвестора через WebMoney','Зачислено', 'popoln')") or die(mysql_error());
				...
				mysql_query("UPDATE `tb_users` SET `money_rek`=`money_rek`+'$merch_amount' WHERE `username`='$username' ORDER BY `id` ASC LIMIT 1") or die(mysql_error());
				...
				if($shp_item==21 | $shp_item==32) {
					mysql_query("UPDATE `".$TABLE_ARR[$shp_item]."` SET `status`='1', `wmid`='$merch_user_wmid', `date`='".time()."',`money`='$merch_amount',`balance`='$merch_amount'  WHERE `status`='0' AND `merch_tran_id`='$merch_tran_id' ORDER BY `id` DESC LIMIT 1") or die(mysql_error());
				}elseif($shp_item==30) {
