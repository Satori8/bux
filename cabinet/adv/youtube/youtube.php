...
if(isset($ads) && isset($id) && isset($op) && $ads=="youtube" && $op=="edit" && $id>0) {
	if(!DEFINED("YOUTUBE_EDIT")) DEFINE("YOUTUBE_EDIT", true);
	include("youtube_edit.php");
	include(DOC_ROOT."/footer.php");
	exit();
}

if(isset($ads) && isset($id) && isset($op) && $ads=="youtube" && $op=="statistics" && $id>0) {
	if(!DEFINED("YOUTUBE_STAT")) DEFINE("YOUTUBE_STAT", true);
	...
	include("youtube_stat.php");
	include(DOC_ROOT."/footer.php");
	exit();
}

echo '<a name="goto"></a>';
echo '<h5 class="sp" style="margin-top:0px; font-size:14px; font-weight:bold; text-align:center;">Мои рекламные видеоролики в серфинге YouTube</h5>';
...
function pay_adv(id, type, bezlimit, type_serf) {
	...
	plan = plan.replace(",", ".");
	rplan = parseInt(plan);
	if (type_serf == 3 | type_serf == 4) var min_hits = <?=$youtube_min_hits_vip;?>;
	else var min_hits = <?=$youtube_min_hits;?>;
...
$sql = mysql_query("SELECT * FROM `tb_ads_youtube` WHERE `username`='$username' AND ytflag='1' AND `type_serf`!='10' ORDER BY `id` ASC");
if(mysql_num_rows($sql)>0) {
	echo '<table class="adv-cab">';

	while ($row = mysql_fetch_array($sql)) {
	$img_youtube=$row['img_youtube'];
		if($row["type_serf"]==1) {
			$cena_hits = $youtube_cena_hits;
			$min_hits = $youtube_min_hits;
		}elseif($row["type_serf"]==2) {
			$cena_hits = $youtube_cena_hits_bs;
			$min_hits = $youtube_min_hits;
		}elseif($row["type_serf"]==3) {
			$cena_hits = $youtube_cena_hits;
			$min_hits = $youtube_min_hits_vip;
		...
		if($row["status"]==1 && $row["up_list"]>0) {
			$position = mysql_result(mysql_query( "SELECT COUNT(*) FROM `tb_ads_youtube` WHERE `status`='1' AND `up_list`>(SELECT `up_list` FROM `tb_ads_youtube` WHERE `id`='".$row["id"]."')" ),0,0)+1;
		}else{
			unset($position);
		}

		$cena_one = ($cena_hits + ($youtube_cena_timer * ($row["timer"]-$youtube_timer_ot)) + ($row["active"] * $youtube_cena_active)) * ($youtube_nacenka + 100)/100 + ( ($row["color"] * $youtube_cena_color) + $youtube_cena_revisit[$row["revisit"]] + $youtube_cena_unic_ip[$row["unic_ip"]] );
		...
		echo '<tr id="adv_dell'.$row["id"].'">';
			...
			echo '<td align="left" class="noborder2">';
				...
				echo '<img style="cursor: pointer;" src="'.$img_youtube.'" border="0" width="80"><br>';
				...
				if($row["claims"] > 0 ) {
					echo '&nbsp;&nbsp;<span id="claims-'.$row["id"].'" class="info-text" onClick="ViewClaims(\''.$row["id"].'\', \'youtube\', \'ViewClaims\');" title="Просмотреть жалобы, поданные пользователями проекта" style="color:#FF0000; cursor:pointer;">Жалобы:&nbsp;<b>'.number_format($row["claims"], 0, ".", "`").'</b></span>';
				}

				echo '<span class="adv-dell" title="Удалить ссылку" onClick="alert_delete('.$row["id"].', \'youtube\');"></span>';
				echo '<a class="adv-edit" href="?ads='.$ads.'&op=edit&id='.$row["id"].'#goto" title="Редактировать ссылку"></a>';

				if($row["status"]==1) {
					if(isset($position) && $position>0 && $position<100) echo '<span class="adv-down" title="Позиция видеоролика в списке серфинга YouTube: '.$position.'" onClick="show_up_list(\'advup'.$row["id"].'\');">'.$position.'</span>';
			...
			if($row["type_serf"]==3) {
				...
				echo '<span class="adv-erase" title="Сброс статистики" onClick="clear_stat('.$row["id"].', \'youtube\', '.$row["outside"].');"></span>';
		...
		echo '<tr id="advup'.$row["id"].'" style="display: none">';
			echo '<td align="center" colspan="3" class="ext-text">';
				...
				echo 'Вы можете разместить свой видеоролик выше остальных в списке серфинга YouTube. Видеоролик будет сохранять свою позицию до тех пор, пока другой пользователь не поднимет свой видеоролик. ';
				echo 'В таком случае ваш видеоролик будет опускаться ниже. В связи с этим, невозможно однозначно сказать, как долго ваш видеоролик продержится на первых позициях. Стоимость поднятия составляет <b>'.$youtube_cena_uplist.'</b> руб. Эта сумма будет снята с рекламного счета.';
				echo '</div>';
				echo '<center><span onClick="pay_adv_up('.$row["id"].', \'youtube\');" class="sub-red" style="float:none;" title="Поднять">Поднять</span></center>';
			echo '</td>';
		echo '</tr>';
		...
		echo '<tr id="adv_addmoney'.$row["id"].'" style="display: none">';
			...
			function obsch<?php echo $row["id"];?>(bezlimit){
				...
				if (bezlimit == 1) {
					if(plan<?php echo $row["id"];?> == 1) {
						var cena_link<?php echo $row["id"];?> = <?php echo $youtube_cena_nolimit[1];?>;
					} else if(plan<?php echo $row["id"];?> == 2) {
						var cena_link<?php echo $row["id"];?> = <?php echo $youtube_cena_nolimit[2];?>;
					} else if(plan<?php echo $row["id"];?> == 3) {
						var cena_link<?php echo $row["id"];?> = <?php echo $youtube_cena_nolimit[3];?>;
					} else if(plan<?php echo $row["id"];?> == 4) {
						var cena_link<?php echo $row["id"];?> = <?php echo $youtube_cena_nolimit[4];?>;
					} else {
			...
			echo '<td align="center" colspan="3" class="ext-text">';
				if($row["nolimit"]!=0) {
					...
					echo '<center><span onClick="pay_adv('.$row["id"].', \'youtube\', 1, '.$row["type_serf"].');" class="sub-green" style="float:none;" title="Пополнить бюджет площадки">Оплатить</span></center>';
					...
					echo '<span id="price'.$row["id"].'_1" class="payadvrez">0.00 руб.</span>';
					if($cab_status==1 && $cab_skidka>0) echo '<div style="color:#708090;">(с учетом вашей скидки '.$cab_skidka.'% это составит <span id="price'.$row["id"].'_2">0.00 руб.</span>)</div>';
					echo '<center><span onClick="pay_adv('.$row["id"].', \'youtube\', 0, '.$row["type_serf"].');" class="sub-green" style="float:none;" title="Пополнить бюджет площадки">Оплатить</span></center>';
					echo '<div id="imfo-msg-addmoney'.$row["id"].'" style="display: none"></div>';
				}
			echo '</td>';
	...
	echo '<span class="msg-w">У вас нет своих размещённых видеороликов в серфинге YouTube</span>';
}

echo '<div align="center"><span class="proc-btn" onClick="document.location.href=\'/advertise.php?ads=youtube\'">Разместить видеоролик</span></div>';
?>