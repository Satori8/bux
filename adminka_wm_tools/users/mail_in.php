...
<span id="lot"><h3>Входящая почта пользователей</h3>
...
if($_GET["option"]=="dellmail")
{
   ...
   $res = "DELETE FROM tb_mail_in WHERE status=1 and date<$timemailout";
...
if(isset($_GET["maildeldialog"]))
{
    ...
    $res=mysql_query("select nameout,namein,subject from tb_mail_in where id='$maildell'");
	  $res=mysql_fetch_array($res);
	  $nameout=$res["nameout"];
	  $namein=$res["namein"];
	  $subject=$res["subject"];
    $qw="DELETE from tb_mail_in where subject='$subject' and (( nameout='$nameout' and namein='$namein' ) or (nameout='$namein' and namein='$nameout') )";
    $res=mysql_query($qw);
...
<table class="history" width="40%" ><thead>
  ...
  <tr>
   <form action="index.php?op=mail_in&option=dellmail" method="post" >
...
<?
  while($row=mysql_fetch_array($res))
  {
	...
	$restema=mysql_query("select count(id) as kolvo from tb_mail_in where subject='$mail_subject' and (( nameout='$mail_sender' and namein='$mail_namein' ) or (nameout='$mail_namein' and namein='$mail_sender') ) ");
    ...
    <tr>
      ...
      ?> <td><a href="index.php?op=mail_in&mail=<?=$mail_id?>&page=<?=$page?>"><?=$mail_subject?></a><div style="float: right; display: block; height: 16px; "><span class="mail-dell"  style="cursor: pointer;" title="Удалить всю переписку" 
      onclick="javascript:
      if (confirm('Вы уверены что хотите удалить диалог?'))
           { 
            top.location = 'index.php?op=mail_in&page=<?=$page?>&maildeldialog=<?=$mail_id?>'; 
           };">
     </span></div></td> <?
...
if(isset($_GET["mail"]))
{
    ...
    $res=mysql_query("select subject,nameout,namein from tb_mail_in where id='$mail'");
	  $res=mysql_fetch_array($res);
	  $subject=$res["subject"];
	  $nameout=$res["nameout"];
	  $namein=$res["namein"];
    ...
    $qw="select nameout,namein,message,date from tb_mail_in where subject='$subject' and (( nameout='$nameout' and namein='$namein' ) or (nameout='$namein' and namein='$nameout') )  order by date  desc ";
    $res=mysql_query($qw);
    //echo "$mail";
