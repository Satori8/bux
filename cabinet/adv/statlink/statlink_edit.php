<?php
if(!DEFINED("STATLINK_EDIT")) {die ("Hacking attempt!");}

	echo '<a name="goto"></a>';
	echo '<h5 class="sp" style="margin-top:0px; font-size:14px; font-weight:bold; text-align:center;">Редактирование статической ссылки № '.$id.'</h5>';
	...
	if(mysql_num_rows($sql)>0) {
		...
		function obsch(){
			...
			var cena_link = <?php echo $cena_slink;?>;
				...
				echo '<td>';
					if($row["status"]=="0" || $row["status"]=="3") {
						echo '<select id="color" onChange="obsch();" onClick="obsch();">';
							...
							echo '<option value="1" '.("".$row["color"]."" == "1" ? 'selected="selected"' : false).'>Да (+'.number_format($cena_slink_color,2,".","'").' руб./сутки)</option>';
		...
		echo '<div align="center"><span onClick="save_ads('.$row["id"].', \'statlink\');" class="proc-btn" style="float:none; width:160px;">Сохранить</span></div>';
