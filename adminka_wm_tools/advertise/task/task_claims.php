...
//mysql_query("UPDATE `tb_ads_task` SET `status`='3', `date`='".time()."' WHERE `status`>'0' AND `status`<'3' AND ( (`totals`<'1' )") or die(mysql_error());
...
$perpage = 20;
$sql_p = mysql_query("SELECT `id` FROM `tb_ads_task` WHERE `status`>'0' AND `id` IN (SELECT `ident` FROM `tb_ads_claims` WHERE `type`='task') ");
...
$sql = mysql_query("SELECT * FROM `tb_ads_task` WHERE `status`>'0' AND `id` IN (SELECT `ident` FROM `tb_ads_claims` WHERE `type`='task') ORDER BY `id` DESC LIMIT $start_pos,$perpage ");
