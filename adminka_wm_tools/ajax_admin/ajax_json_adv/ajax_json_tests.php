	...
	if(mysql_num_rows($sql)>0) {
		...
		if($status==1) {
			...
			if($cashback == 1 && $balance >= 1 && $rek_name != false) {
				...
				VALUES('1','$rek_name','".DATE("d.m.Y H:i")."','".time()."','$money_back','Возврат средств с рекламной площадки (Тесты, ID:$id)', 'Зачислено', 'reklama')") or die(json_encode_cp1251(array("result" => "ERROR", "message" => iconv("CP1251", "UTF-8", mysql_error()))));
