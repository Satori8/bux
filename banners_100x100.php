...
if(!isset($banners100x100_array) && is_file(banners100x100_file) ) {
	...
	if($banners_count>0) {
		...
		for($i=0; $i<$banners_count; $i++){
			...
			echo '</tr>';
			echo '<tr align="center">';
				echo '<td>Переходов:&nbsp;&nbsp;'.$banners100x100_array[$i]['count_visit'].'<br>Показ заказан до: '.DATE("d.m.Y H:i:s", $banners100x100_array[$i]['datE']).'<br><br></td>';
			echo '</tr>';
		}
		echo '</table>';
