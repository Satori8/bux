...
if(isset($_SESSION["userLog"]) && isset($_SESSION["userPas"])) {
	...
	if(isset($_SESSION["userLog"]) && isset($_SESSION["userPas"]) && isset($my_referer)) {
		if($my_referer!=false) {
			$sql_p = mysql_query("SELECT `p_psd`,`discount_partner` FROM `tb_users_partner` WHERE `username`='$my_referer' AND `discount_partner`>'0'");
	...
	if(!isset($_SESSION["userLog"]) && !isset($_SESSION["userPas"])) {
		...
		if(mysql_num_rows($sql_id)>0) {
			...
			if($money_user>=$money_pay) {
				...
				mysql_query("INSERT INTO `tb_history` (`user`,`date`,`amount`,`method`,`status`,`tipo`) VALUES('$username','".DATE("d.m.Yг. H:i")."','$money_pay',  'Оплата рекламы: - Псевдо-динамические ссылки $plan дн.','Списано','rashod')") or die(mysql_error());
				...
				mysql_query("UPDATE `tb_users` SET `p_psd`='$add_percent_user' WHERE `username`='$username'") or die(mysql_error());

				if($my_referer!=false) {
					$sql_up = mysql_query("SELECT `p_psd` FROM `tb_users` WHERE `username`='$my_referer' AND `p_psd`>'0'");
					if(mysql_num_rows($sql_up)>0) {
						...
						mysql_query("INSERT INTO `tb_partner` (`time`, `username`, `referer`, `type`, `money`, `percent`) VALUES('".time()."','$username','$my_referer','p_psd','$money_add_referer', '$ref_p_psd')") or die("Error".mysql_error());
					}
				}
...
$system_pay[21] = "AdvCash";
		...
		if(mysql_num_rows($check_wmid)>0) {
			mysql_query("UPDATE `tb_ads_psevdo` SET `merch_tran_id`='$merch_tran_id',`method_pay`='$method_pay',`wmid`='$wmid_user',`username`='$username',`plan`='$plan',`date`='".time()."',`date_end`='".(time() + $plan*24*60*60)."',`color`='$color',`url`='$url',`description`='$description',`ip`='$laip',`money`='$summa' WHERE `status`='0' AND `session_ident`='".session_id()."'") or die(mysql_error());
		}else{
			mysql_query("INSERT INTO `tb_ads_psevdo` (`status`,`session_ident`,`merch_tran_id`,`method_pay`,`wmid`,`username`,`plan`,`date`,`date_end`,`color`,`url`,`description`,`ip`,`money`) 
			VALUES ('0','".session_id()."','$merch_tran_id','$method_pay','$wmid_user','$username','$plan','".time()."','".(time() + $plan*24*60*60)."','$color','$url','$description','$laip','$summa')") or die(mysql_error());
		}
