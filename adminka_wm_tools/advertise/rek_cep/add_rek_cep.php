...
if(count($_POST)>0 && isset($_POST["id_pay"])) {
	...
	$sql_id = mysql_query("SELECT * FROM `tb_ads_rc` WHERE `id`='$id_pay' AND `status`='0' AND `username`='$username' ORDER BY `id` DESC LIMIT 1");
	if(mysql_num_rows($sql_id)>0) {
		$row = mysql_fetch_array($sql_id);

		mysql_query("UPDATE `tb_ads_rc` SET `status`='1', `date`='".time()."' WHERE `id`='$id_pay' AND `status`='0' AND `username`='$username' ORDER BY `id` DESC LIMIT 1") or die(mysql_error());
	...
	if(mysql_num_rows($sql_bl)>0 && $black_url!=false) {
		...
		if(mysql_num_rows($check_wmid)>0) {
			mysql_query("UPDATE `tb_ads_rc` SET `wmid`='$wmid_user', `username`='$username', `date`='".time()."', `url`='$url', `description`='$description', `color`='$color', `ip`='$laip', `money`='0' WHERE `status`='0' AND `session_ident`='".session_id()."'") or die(mysql_error());
		}else{
			mysql_query("INSERT INTO `tb_ads_rc` (`status`,`session_ident`,`merch_tran_id`,`method_pay`,`wmid`,`username`,`date`,`url`,`description`,`color`,`ip`,`money`) 
			VALUES('0','".session_id()."','0','$method_pay','$wmid_user','$username','".time()."','$url','$description','$color','$laip','0')") or die(mysql_error());
		}
