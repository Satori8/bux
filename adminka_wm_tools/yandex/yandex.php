...
if (isset($_POST['auth']))
{
  ...
  else
  {
    ...
    ?>
    <div style="padding: 30px;" align="center">—ейчас ¬ы будете перенаправлены на сайт яндекс.ƒеньги дл€ авторизации ¬ашего приложени€.</div>
    <script>
...
<form method="post" action="" id='MainForm'>
 <table class="adn">
  <tr class="lineb">
   ...
   <td align="left"><table class="adn"><tr><td align="left" style="border: none; padding: 0">ќписание</td><td align="right" style="border: none; padding: 0"><a href="#" onclick="document.getElementById('MainForm').submit(); return false" class="liv">—охранить</a></td></tr></table></td>
  </tr>
  <tr class="liney">
   <td class="settab listsr"><input type=text value='<? echo $row["yandex"]; ?>' name='yandex'></td>
   <td class="listsl"><b>яндекс.ƒеньги</b><br>—чЄт в яндекс.ƒеньги</td>
  </tr>
  <tr  class="liney ell">
   <td class="settab listsr"><input type=text value='<? echo $row["client_id"]; ?>' name='client_id' ></td>
   <td class="listsl"><b>»дентификатор</b><br>»дентификатор магазина в яƒ</td>
  </tr>
  <tr class="liney">
   <td class="settab listsr"><input type=text value='<? echo $row["redirect_url"]; ?>' name='redirect_url'></td>
   <td class="listsl"><b>Redirect_url</b><br>јдрес указанный в магазине яƒ</td>
  </tr>
 </table>
 <br />
 <br />
...
<form action="" method="post">
 <table align="center">
  <thead>
   <tr>
    <td>
     <span style="color: #FF0000; font-weight: bold;">¬нимание!</span> ƒл€ работы с API яндекс.ƒенег нужно авторизовать пользовател€ в системе.
    </td>
   </tr>
  </thead>
  <tfoot>
   <tr>
    <td><input type="hidden" name="auth" value="1" /><input type="submit" value="јвторизовать в системе яндекс.ƒеньги" /></td>
   </tr>
  </tfoot>
 </table>
</form>
