...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) && $_SERVER["HTTP_X_REQUESTED_WITH"]=="XMLHttpRequest") {
	if(isset($_SESSION["userLog"]) && isset($_SESSION["userPas"])) {
		...
		if(mysql_num_rows($sql_user)>0) {
			...
			$sql_link = mysql_query("SELECT `id` FROM `tb_ads_dlink` WHERE `id`='$id_adv_post' AND `status`='1' 
				...
				AND (`to_ref`='0' 
					...
					OR (`to_ref`='2' AND `username`!='' AND (`username`='$username' OR `username`='$my_referer_1' OR `username`='$my_referer_2' OR `username`='$my_referer_3') ) 
				) 
				AND `id` NOT IN (
			...
			if(mysql_num_rows($sql_link)>0) {
				...
				$token = strrev(md5(strtolower($username).$id_adv_post.$my_lastiplog)).strrev(md5(strtolower($_SERVER["HTTP_HOST"]).strtolower($_SERVER["HTTP_USER_AGENT"]).$my_lastiplog."### SCORPION ###"));
				$url_link = "http://".$_SERVER["HTTP_HOST"]."/surfing_new.php?id=$id_adv_post&token=$token";
