...
if(!isset($banners_array) && is_file($banners_file) ) {
	...
	echo '<div align="center" style="display:inline-block; width:'.$width.'px; height:'.$height.'px; margin:0 auto; border:3px solid #CCC;"><div id="ImgBan_'.$banner_type.'"></div></div>';

	echo '<script type="text/javascript">RotBan_'.$banner_type.'(); setInterval("RotBan_'.$banner_type.'()", '.$timer_refresh.');</script>';

}

unset($banners_array, $banners_file, $UrlBan_arr, $ImgBan_arr);
