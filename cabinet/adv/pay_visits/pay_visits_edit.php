<?php
if(!DEFINED("CABINET") | !DEFINED("PAY_VISITS_EDIT")) exit("Access denied!");

$sql = $mysqli->query("SELECT * FROM `tb_ads_pay_vis` USE INDEX (`username_id`) WHERE `id`='$id' AND `username`='$username'") or die($mysqli->error);
if($sql->num_rows > 0) {
	$row = $sql->fetch_assoc();
	$id = $row["id"];
	$status = $row["status"];
...
$token_edit = strtolower(md5($id.strtolower($username).$_SERVER["HTTP_HOST"]."token-edit-adv".$security_key));
...
echo '<div id="newform" class="form-edit-adv ec.c" onKeyPress="CtrlEnter(event);"><form id="form-edit-'.$ads.'-'.$id.'" action="" method="POST" onSubmit="FuncAdvCab('.$id.', \'edit-adv\', \''.$ads.'\', $(this).attr(\'id\'), \''.$token_edit.'\', \'modal\'); return false;">';
	...
	echo '<div id="adv-block-dopset" style="display:block;">';
		...
		echo '<tbody>';
			...
			echo '<tr>';
				...
				echo '<td><input type="checkbox" id="content" name="content" value="1" '.($row["content"]==1 ? 'checked="checked"' : false).'> - на моем сайте присутствуют материалы для взрослых</td>';
	...
	echo '<tbody>';
		echo '<tr>';
			echo '<td align="center"><input id="subAdv" type="submit" value="Cохранить изменения" class="proc-btn"></td>';
		echo '</tr>';
	echo '</tbody>';
	echo '</table>';
