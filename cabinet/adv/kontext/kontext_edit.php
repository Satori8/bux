<?php
if(!DEFINED("KONTEXT_EDIT")) {die ("Hacking attempt!");}

	echo '<a name="goto"></a>';
	echo '<h5 class="sp" style="margin-top:0px; font-size:14px; font-weight:bold; text-align:center;">Редактирование контекстной рекламы № '.$id.'</h5>';
	...
	if(mysql_num_rows($sql)>0) {
		...
		function save_ads(id, type) {
			...
			if ((url == '') | (url == 'http://') | (url == 'https://')) {
				...
				gebi("title").style.background = "#FFDBDB";
				gebi("title").focus();
				...
				$.ajax({
					type: "POST", url: "/cabinet/ajax/ajax_adv.php", data: {'op':'save', 'type':type, 'id':id, 'url':url, 'title':title, 'description':description, 'color':color}, 
		...
		function obsch(){
			...
			var cena_link = <?php echo $cena_kontext;?>;
			var cena_color = <?php echo $cena_kontext_color;?>;

			var price = (cena_link + color * cena_color);
			gebi('price').innerHTML = '<span style="color:#228B22;"><b>' + number_format(price, 2, '.', ' ') + '</b> руб./переход</span>';
		}
		</script><?php
		...
		echo '<tbody>';
			...
			echo '<tr>';
				...
				echo '<td><input type="text" id="title" maxlength="30" value="'.$row["title"].'" class="ok" onkeydown="this.style.background=\'#FFFFFF\';"></td>';
			echo '</tr>';
				...
				echo '<td>';
					if($row["status"]=="0" | $row["status"]=="3") {
						echo '<select id="color" onChange="obsch();" onClick="obsch();">';
							...
							echo '<option value="1" '.("".$row["color"]."" == "1" ? 'selected="selected"' : false).'>Да (+'.number_format($cena_kontext_color,2,".","'").' руб./переход)</option>';
		...
		echo '<div align="center"><span onClick="save_ads('.$row["id"].', \'kontext\');" class="proc-btn" style="float:none; width:160px;">Сохранить</span></div>';
