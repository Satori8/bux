...
if(count($_POST)>0 && isset($_POST["id_pay"])) {
	if(!isset($_SESSION["userLog"]) && !isset($_SESSION["userPas"])) {
		...
		$sql_id = mysql_query("SELECT * FROM `tb_ads_autoyoutube` WHERE `id`='$id_pay' AND `status`='0' AND `username`='$username' ORDER BY `id` DESC LIMIT 1");
		if(mysql_num_rows($sql_id)>0) {
			...
			if($money_user>=$money_pay) {
				...
				mysql_query("INSERT INTO `tb_history` (`user`,`date`,`amount`,`method`,`status`,`tipo`) VALUES('$username','".DATE("d.m.Yг. H:i")."','$money_pay',  'Оплата рекламы: автосерфинг YouTube - $plan шт.','Списано','rashod')") or die(mysql_error());
				...
				echo '<span class="msg-ok">Ваша видеоролик успешно размещен!<br>Спасибо, что пользуетесь услугами нашего сервиса</span>';
				include('footer.php');
				exit();
			}else{
...
if(count($_POST)>0) {
	...
	//	echo '<span class="msg-error">Ограничение количества показов в сутки не может быть больше чем заказанное количество визитов ('.$plan.').</span><br></div></div>';
		...
		if(mysql_num_rows($check_wmid)>0) {
			mysql_query("UPDATE `tb_ads_autoyoutube` SET `img_youtube`='$img_youtube', `merch_tran_id`='$merch_tran_id',`method_pay`='$method_pay',`wmid`='$wmid_user',`username`='$username',`geo_targ`='$country',`timer`='$timer', `date`='".time()."', `plan`='$plan', `totals`='$plan', `limits`='$limits_table',`limits_now`='$limits_table', `url`='$url', `description`='$description', `check_url`='1', `ip`='$laip', `money`='$summa' WHERE `status`='0' AND `session_ident`='".session_id()."'") or die(mysql_error());
		}else{
			mysql_query("INSERT INTO `tb_ads_autoyoutube` (`status`,`session_ident`,`merch_tran_id`,`method_pay`,`wmid`,`username`,`geo_targ`,`timer`,`date`,`plan`,`totals`,`limits`,`limits_now`,`members`,`url`,`description`,`check_url`,`claims`,`ip`,`money`, `ytflag`, `img_youtube`) 
			VALUES('0','".session_id()."','$merch_tran_id','$method_pay','$wmid_user','$username','$country','$timer','".time()."','$plan','$plan','$limits_table','$limits_table','0','$url','$description','1','0','$laip','$summa','$ytflag', '$img_youtube')") or die(mysql_error());
		}
	...
	echo '<div id="adv-block-rules" style="display:block; padding:5px 7px 10px 7px; text-align:center; background-color:#FFFFFF;">';
		echo '<span class="warning-info" style="margin-bottom:0; font-weight:normal;">';
                        ...
                        echo 'выбрать копировать ссылку на видеоролик и вставить в поле URL видеоролика!<br>';
		echo '</span>';
	//echo '<br/>';
	echo '<br><br>';
	...
	echo '<table class="tables">';
		...
		//if(isset($_SESSION["checkurl"]) | isset($_SESSION["checkurl_ok"])) {
			...
			echo '</tr>';echo '<tr><td colspan=2><span id="adv-title2" class="adv-title-close" onclick="ShowHideBlock(2);">Настройки географического таргетинга</span></td></tr>';
